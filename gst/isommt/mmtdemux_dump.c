...
gboolean
mmtdemux_dump_mvhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version = 0;
...
gboolean
mmtdemux_dump_tkhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_elst (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_mdhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version = 0;
  guint64 duration, ctime, mtime;
...
gboolean
mmtdemux_dump_hdlr (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_vmhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_dref (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
static gboolean
mmtdemux_dump_stsd_avc1 (GstMMTDemux * mmtdemux, GstByteReader * data, guint size,
    int depth)
{
  guint32 fourcc;
  ...
  GST_LOG_OBJECT (mmtdemux, "%*s    data reference:%d", depth, "",
      GET_UINT16 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    version/rev.:  %08x", depth, "",
      GET_UINT32 (data));
  fourcc = GET_FOURCC (data);
  GST_LOG_OBJECT (mmtdemux, "%*s    vendor:        %" GST_FOURCC_FORMAT, depth,
      "", GST_FOURCC_ARGS (fourcc));
  GST_LOG_OBJECT (mmtdemux, "%*s    temporal qual: %u", depth, "",
      GET_UINT32 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    spatial qual:  %u", depth, "",
      GET_UINT32 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    width:         %u", depth, "",
      GET_UINT16 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    height:        %u", depth, "",
      GET_UINT16 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    horiz. resol:  %g", depth, "",
      GET_FP32 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    vert. resol.:  %g", depth, "",
      GET_FP32 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    data size:     %u", depth, "",
      GET_UINT32 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    frame count:   %u", depth, "",
      GET_UINT16 (data));
  ...
  GST_LOG_OBJECT (mmtdemux, "%*s    compressor:    (skipped)", depth, "");
  GST_LOG_OBJECT (mmtdemux, "%*s    depth:         %u", depth, "",
      GET_UINT16 (data));
  GST_LOG_OBJECT (mmtdemux, "%*s    color table ID:%u", depth, "",
      GET_UINT16 (data));
...
gboolean
mmtdemux_dump_stsd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    GST_LOG_OBJECT (mmtdemux, "%*s    size:          %u", depth, "", size);
    GST_LOG_OBJECT (mmtdemux, "%*s    type:          %" GST_FOURCC_FORMAT, depth,
        "", GST_FOURCC_ARGS (fourcc));
    ...
    switch (fourcc) {
      case FOURCC_avc1:
        if (!mmtdemux_dump_stsd_avc1 (mmtdemux, &sub, size, depth + 1))
          return FALSE;
        break;
      case FOURCC_fLaC:
      ...
      case FOURCC_mp4s:
        if (!gst_byte_reader_get_uint32_be (&sub, &ver_flags) ||
          ...
          return FALSE;
        if (!mmtdemux_dump_unknown (mmtdemux, &sub, depth + 1))
          return FALSE;
        break;
      default:
...
gboolean
mmtdemux_dump_stts (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_stps (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_stss (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_stsc (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_stsz (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_stco (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_ctts (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_cslg (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_co64 (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags = 0, num_entries = 0, i;
...
gboolean
mmtdemux_dump_dcom (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_cmvd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  if (!qt_atom_parser_has_remaining (data, 4))
    return FALSE;
...
gboolean
mmtdemux_dump_mfro (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  if (!qt_atom_parser_has_remaining (data, 4))
    return FALSE;
...
gboolean
mmtdemux_dump_mfhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  if (!qt_atom_parser_has_remaining (data, 4))
    return FALSE;
...
gboolean
mmtdemux_dump_tfra (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_tfhd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_trun (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_trex (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  if (!qt_atom_parser_has_remaining (data, 4 + 4 + 4 + 4 + 4 + 4))
    return FALSE;
...
gboolean
mmtdemux_dump_mehd (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version = 0;
...
gboolean
mmtdemux_dump_tfdt (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version = 0;
  guint64 decode_time;
  guint value_size;
...
gboolean
mmtdemux_dump_sdtp (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version;
  guint8 val;
  guint i = 1;
...
gboolean
mmtdemux_dump_svmi (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 version;
...
gboolean
mmtdemux_dump_dfLa (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_fLaC (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
...
gboolean
mmtdemux_dump_gmin (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  guint32 ver_flags;
...
gboolean
mmtdemux_dump_unknown (GstMMTDemux * mmtdemux, GstByteReader * data, int depth)
{
  int len;
  ...
  GST_MEMDUMP_OBJECT (mmtdemux, "unknown atom data",
      gst_byte_reader_peek_data_unchecked (data), len);
  return TRUE;
}

static gboolean
mmtdemux_node_dump_foreach (GNode * node, gpointer mmtdemux)
{
  ...
  if (type->dump) {
    ...
    ret = type->dump (GST_QTDEMUX_CAST (mmtdemux), &parser, depth);
...
gboolean
mmtdemux_node_dump (GstMMTDemux * mmtdemux, GNode * node)
{
#ifndef GST_DISABLE_GST_DEBUG
  /* Only traverse/dump if we know it will be outputted in the end */
  if (mmtdemux_debug->threshold < GST_LEVEL_LOG)
    return TRUE;

  g_node_traverse (node, G_PRE_ORDER, G_TRAVERSE_ALL, -1,
      mmtdemux_node_dump_foreach, mmtdemux);
#endif
  return TRUE;
}
