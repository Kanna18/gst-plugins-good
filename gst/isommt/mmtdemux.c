 ...
 * |[
 * gst-launch-1.0 filesrc location=test.mov ! mmtdemux name=demux  demux.audio_0 ! queue ! decodebin ! audioconvert ! audioresample ! autoaudiosink   demux.video_0 ! queue ! decodebin ! videoconvert ! videoscale ! autovideosink
 * ]| Play (parse and decode) a .mov file and try to output it to
 * an automatically detected soundcard and videosink. If the MOV file contains
 * compressed audio or video data, this will only work if you have the
 * right decoder elements/plugins installed.
 * </refsect2>
 * CODE MODIFICATION FOR MMTDEMUX
...
#include "mmtdemux_types.h"
#include "mmtdemux_dump.h"
#include "fourcc.h"
#include "descriptors.h"
#include "mmtdemux_lang.h"
#include "mmtdemux.h"
#include "qtpalette.h"
//#include "parser.h"
#include "MMTPacket.h" 
...
#define ADVANCE_TIME 2000000000

GST_DEBUG_CATEGORY (mmtdemux_debug);
#define GST_CAT_DEFAULT mmtdemux_debug
...
guint64   sample_pts_offset[100];

/* Macros for converting to/from timescale */
#define QT_GSTTIME(stream, value) (gst_util_uint64_scale((value), GST_SECOND, (stream)->timescale))
...
#define QTTIME_TO_GSTTIME(mmtdemux, value) (gst_util_uint64_scale((value), GST_SECOND, (mmtdemux)->timescale))
#define GSTTIME_TO_QTTIME(mmtdemux, value) (gst_util_uint64_scale((value), (mmtdemux)->timescale, GST_SECOND))
...
guint64 skip_bytes(unsigned char *buffer, guint64 Rsize);
static GstFlowReturn
gst_mmtdemux_loop_state_mmt_header(GstMMTDemux * mmtdemux);

static GNode *mmtdemux_tree_get_child_by_type (GNode * node, guint32 fourcc);
static GNode *mmtdemux_tree_get_child_by_type_full (GNode * node,
    guint32 fourcc, GstByteReader * parser);
static GNode *mmtdemux_tree_get_sibling_by_type (GNode * node, guint32 fourcc);
static GNode *mmtdemux_tree_get_sibling_by_type_full (GNode * node,
    guint32 fourcc, GstByteReader * parser);

static GstFlowReturn mmtdemux_add_fragmented_samples (GstMMTDemux * mmtdemux);

static GstStaticPadTemplate gst_mmtdemux_sink_template =
    GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/xmmt; video/h265; audio/x-m4a; "
        "application/x-3gp")
    );

static GstStaticPadTemplate gst_mmtdemux_videosrc_template =
GST_STATIC_PAD_TEMPLATE ("video_%u",
    GST_PAD_SRC,
    GST_PAD_SOMETIMES,
    GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate gst_mmtdemux_audiosrc_template =
GST_STATIC_PAD_TEMPLATE ("audio_%u",
    GST_PAD_SRC,
    GST_PAD_SOMETIMES,
    GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate gst_mmtdemux_subsrc_template =
GST_STATIC_PAD_TEMPLATE ("subtitle_%u",
    GST_PAD_SRC,
    GST_PAD_SOMETIMES,
    GST_STATIC_CAPS_ANY);

#define gst_mmtdemux_parent_class parent_class
G_DEFINE_TYPE (GstMMTDemux, gst_mmtdemux, GST_TYPE_ELEMENT);

static void gst_mmtdemux_dispose (GObject * object);

static guint32
gst_mmtdemux_find_index_linear (GstMMTDemux * mmtdemux, QtDemuxStream * str,
    GstClockTime media_time);
static guint32
gst_mmtdemux_find_index_for_given_media_offset_linear (GstMMTDemux * mmtdemux,
    QtDemuxStream * str, gint64 media_offset);

#if 0
static void gst_mmtdemux_set_index (GstElement * element, GstIndex * index);
static GstIndex *gst_mmtdemux_get_index (GstElement * element);
#endif
static GstStateChangeReturn gst_mmtdemux_change_state (GstElement * element,
    GstStateChange transition);
static gboolean mmtdemux_sink_activate (GstPad * sinkpad, GstObject * parent);
static gboolean mmtdemux_sink_activate_mode (GstPad * sinkpad,
    GstObject * parent, GstPadMode mode, gboolean active);

static void gst_mmtdemux_loop (GstPad * pad);
static GstFlowReturn gst_mmtdemux_chain (GstPad * sinkpad, GstObject * parent,
    GstBuffer * inbuf);
static gboolean gst_mmtdemux_handle_sink_event (GstPad * pad, GstObject * parent,
    GstEvent * event);
static gboolean gst_mmtdemux_setcaps (GstMMTDemux * mmtdemux, GstCaps * caps);
static gboolean gst_mmtdemux_configure_stream (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream);
static void gst_mmtdemux_stream_check_and_change_stsd_index (GstMMTDemux * demux,
    QtDemuxStream * stream);
static GstFlowReturn gst_mmtdemux_process_adapter (GstMMTDemux * demux,
    gboolean force);
static GstFlowReturn bkpgst_mmtdemux_process_adapter (GstMMTDemux * demux,
    gboolean force);

//gboolean mmtdemux_parse_moov (GstMMTDemux * mmtdemux,
  //  const guint8 * buffer, guint length);
static gboolean mmtdemux_parse_node (GstMMTDemux * mmtdemux, GNode * node,
    const guint8 * buffer, guint length);
//gboolean mmtdemux_parse_tree (GstMMTDemux * mmtdemux);
static void mmtdemux_parse_udta (GstMMTDemux * mmtdemux, GstTagList * taglist,
    GNode * udta);

static void gst_mmtdemux_handle_esds (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, QtDemuxStreamStsdEntry * entry, GNode * esds,
    GstTagList * list);
static GstCaps *mmtdemux_video_caps (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, QtDemuxStreamStsdEntry * entry, guint32 fourcc,
    const guint8 * stsd_entry_data, gchar ** codec_name);
static GstCaps *mmtdemux_audio_caps (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, QtDemuxStreamStsdEntry * entry, guint32 fourcc,
    const guint8 * data, int len, gchar ** codec_name);
static GstCaps *mmtdemux_sub_caps (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    QtDemuxStreamStsdEntry * entry, guint32 fourcc, const guint8 * data,
    gchar ** codec_name);
static GstCaps *mmtdemux_generic_caps (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, QtDemuxStreamStsdEntry * entry, guint32 fourcc,
    const guint8 * stsd_entry_data, gchar ** codec_name);

static gboolean mmtdemux_parse_samples (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, guint32 n);
//static GstFlowReturn mmtdemux_expose_streams (GstMMTDemux * mmtdemux);
static void gst_mmtdemux_stream_free (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream);
static void gst_mmtdemux_stream_clear (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream);
static void gst_mmtdemux_remove_stream (GstMMTDemux * mmtdemux, int index);
//static GstFlowReturn mmtdemux_prepare_streams (GstMMTDemux * mmtdemux);
static void mmtdemux_do_allocation (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream);
static gboolean gst_mmtdemux_activate_segment (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, guint32 seg_idx, GstClockTime offset);
static gboolean gst_mmtdemux_stream_update_segment (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, gint seg_idx, GstClockTime offset,
    GstClockTime * _start, GstClockTime * _stop);
static void gst_mmtdemux_send_gap_for_segment (GstMMTDemux * demux,
    QtDemuxStream * stream, gint segment_index, GstClockTime pos);

static gboolean mmtdemux_pull_mfro_mfra (GstMMTDemux * mmtdemux);
static void check_update_duration (GstMMTDemux * mmtdemux, GstClockTime duration);

static gchar *mmtdemux_uuid_bytes_to_string (gconstpointer uuid_bytes);

static GstStructure *mmtdemux_get_cenc_sample_properties (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, guint sample_index);
static void gst_mmtdemux_append_protection_system_id (GstMMTDemux * mmtdemux,
    const gchar * id);
static void mmtdemux_gst_structure_free (GstStructure * gststructure);

static void
gst_mmtdemux_class_init (GstMMTDemuxClass * klass)
{
  ...
  gobject_class->dispose = gst_mmtdemux_dispose;

  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_mmtdemux_change_state);
#if 0
  gstelement_class->set_index = GST_DEBUG_FUNCPTR (gst_mmtdemux_set_index);
  gstelement_class->get_index = GST_DEBUG_FUNCPTR (gst_mmtdemux_get_index);
#endif

  gst_tag_register_musicbrainz_tags ();

  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_mmtdemux_sink_template);
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_mmtdemux_videosrc_template);
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_mmtdemux_audiosrc_template);
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_mmtdemux_subsrc_template);
  gst_element_class_set_static_metadata (gstelement_class, "MMT demuxer",
      "Codec/Demuxer",
      "Demultiplex a QuickTime file into audio and video streams",
      "David Schleef <ds@schleef.org>, Wim Taymans <wim@fluendo.com>, Mohammed Safwan Sada <safwan_sada@yahoo.com>");

  GST_DEBUG_CATEGORY_INIT (mmtdemux_debug, "mmtdemux", 0, "mmtdemux plugin");

}

static void
gst_mmtdemux_init (GstMMTDemux * mmtdemux)
{
  mmtdemux->sinkpad =
      gst_pad_new_from_static_template (&gst_mmtdemux_sink_template, "sink");
  gst_pad_set_activate_function (mmtdemux->sinkpad, mmtdemux_sink_activate);
  gst_pad_set_activatemode_function (mmtdemux->sinkpad,
      mmtdemux_sink_activate_mode);
  gst_pad_set_chain_function (mmtdemux->sinkpad, gst_mmtdemux_chain);
  gst_pad_set_event_function (mmtdemux->sinkpad, gst_mmtdemux_handle_sink_event);
  gst_element_add_pad (GST_ELEMENT_CAST (mmtdemux), mmtdemux->sinkpad);

  mmtdemux->state = QTDEMUX_STATE_INITIAL;
  //mmtdemux->state = QTDEMUX_STATE_META;
  mmtdemux->ismmth=0;
  mmtdemux->seek_position=0;
  mmtdemux->sample_reset = 0;
  mmtdemux->stream_n=0;
  mmtdemux->pullbased = FALSE;
  mmtdemux->posted_redirect = FALSE;
  mmtdemux->neededbytes = 16;
  mmtdemux->todrop = 0;
  mmtdemux->adapter = gst_adapter_new ();
  mmtdemux->offset = 0;
  mmtdemux->first_mdat = -1;
  mmtdemux->got_moov = FALSE;
  mmtdemux->moov_node = NULL;
  mmtdemux->moov_detected = 0;
  mmtdemux->mdatoffset = -1;
  mmtdemux->mmdatbuffer[0] = NULL;
  mmtdemux->mmdatbuffer[1] = NULL;
  mmtdemux->mdatbuffer = NULL;
  mmtdemux->mainbuf = NULL;
  mmtdemux->restoredata_buffer = NULL;
  mmtdemux->restoredata_offset = -1;
  mmtdemux->fragment_start = -1;
  mmtdemux->fragment_start_offset = -1;
  mmtdemux->media_caps = NULL;
  mmtdemux->exposed = FALSE;
  mmtdemux->mss_mode = FALSE;
  mmtdemux->pending_newsegment = NULL;
  mmtdemux->upstream_format_is_time = FALSE;
  mmtdemux->have_group_id = FALSE;
  mmtdemux->group_id = G_MAXUINT;
  mmtdemux->cenc_aux_info_offset = 0;
  mmtdemux->cenc_aux_info_sizes = NULL;
  mmtdemux->cenc_aux_sample_count = 0;
  mmtdemux->protection_system_ids = NULL;
  mmtdemux->audio_packet_id = GST_SIGNALS_NOT_DETECTED;
  mmtdemux->video_packet_id = GST_SIGNALS_NOT_DETECTED;
  g_queue_init (&mmtdemux->protection_event_queue);
  gst_segment_init (&mmtdemux->segment, GST_FORMAT_TIME);
  mmtdemux->tag_list = gst_tag_list_new_empty ();
  gst_tag_list_set_scope (mmtdemux->tag_list, GST_TAG_SCOPE_GLOBAL);
  mmtdemux->flowcombiner = gst_flow_combiner_new ();

  GST_OBJECT_FLAG_SET (mmtdemux, GST_ELEMENT_FLAG_INDEXABLE);
}

static void
gst_mmtdemux_dispose (GObject * object)
{
  GstMMTDemux *mmtdemux = GST_QTDEMUX (object);

  if (mmtdemux->adapter) {
    g_object_unref (G_OBJECT (mmtdemux->adapter));
    mmtdemux->adapter = NULL;
  }
  gst_tag_list_unref (mmtdemux->tag_list);
  gst_flow_combiner_free (mmtdemux->flowcombiner);
  g_queue_foreach (&mmtdemux->protection_event_queue, (GFunc) gst_event_unref,
      NULL);
  g_queue_clear (&mmtdemux->protection_event_queue);

  g_free (mmtdemux->cenc_aux_info_sizes);
  mmtdemux->cenc_aux_info_sizes = NULL;
...
static void
gst_mmtdemux_post_no_playable_stream_error (GstMMTDemux * mmtdemux)
{
  if (mmtdemux->posted_redirect) {
    GST_ELEMENT_ERROR (mmtdemux, STREAM, DEMUX,
    ...
    GstBuffer ** buf)
{
  ...
  if (G_UNLIKELY (size == 0)) {
    ...
    ret = gst_mmtdemux_pull_atom (mmtdemux, offset, sizeof (guint32), &tmp);
    if (ret != GST_FLOW_OK)
      return ret;

    gst_buffer_map (tmp, &map, GST_MAP_READ);
    size = QT_UINT32 (map.data);
    GST_DEBUG_OBJECT (mmtdemux, "size 0x%08" G_GINT64_MODIFIER "x", size);
  ...
  if (G_UNLIKELY (size > QTDEMUX_MAX_ATOM_SIZE)) {
    if (mmtdemux->state != QTDEMUX_STATE_MOVIE && mmtdemux->got_moov) {
      /* we're pulling header but already got most interesting bits,
       * so never mind the rest (e.g. tags) (that much) */
      GST_WARNING_OBJECT (mmtdemux, "atom has bogus size %" G_GUINT64_FORMAT,
          size);
      return GST_FLOW_EOS;
    } else {
      GST_ELEMENT_ERROR (mmtdemux, STREAM, DEMUX,
  ...
  flow = gst_pad_pull_range (mmtdemux->sinkpad, offset, size, buf);
...
#if 1
static gboolean
gst_mmtdemux_src_convert (GstMMTDemux * mmtdemux, GstPad * pad,
  ...
  switch (src_format) {
    case GST_FORMAT_TIME:
      switch (dest_format) {
        case GST_FORMAT_BYTES:{
          index = gst_mmtdemux_find_index_linear (mmtdemux, stream, src_value);
          if (-1 == index) {
            res = FALSE;
            goto done;
          }
          ...
          *dest_value = stream->samples[index].offset;
          ...
          GST_DEBUG_OBJECT (mmtdemux, "Format Conversion Time->Offset :%"
    ...
    case GST_FORMAT_BYTES:
      switch (dest_format) {
        case GST_FORMAT_TIME:{
          index =
              gst_mmtdemux_find_index_for_given_media_offset_linear (mmtdemux,
              stream, src_value);
...
static gboolean
gst_mmtdemux_get_duration (GstMMTDemux * mmtdemux, GstClockTime * duration)
{
  gboolean res = FALSE;

  *duration = GST_CLOCK_TIME_NONE;

  if (mmtdemux->duration != 0 &&
      mmtdemux->duration != G_MAXINT64 && mmtdemux->timescale != 0) {
    *duration = QTTIME_TO_GSTTIME (mmtdemux, mmtdemux->duration);
    res = TRUE;
  } else {
    *duration = GST_CLOCK_TIME_NONE;
  }
...
static gboolean
gst_mmtdemux_handle_src_query (GstPad * pad, GstObject * parent,
    GstQuery * query)
{
  gboolean res = FALSE;
  GstMMTDemux *mmtdemux = GST_QTDEMUX (parent);

  GST_DEBUG_OBJECT (pad, "%s query", GST_QUERY_TYPE_NAME (query));
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_POSITION:{
      ...
      if (fmt == GST_FORMAT_TIME
          && GST_CLOCK_TIME_IS_VALID (mmtdemux->segment.position)) {
        gst_query_set_position (query, GST_FORMAT_TIME,
            mmtdemux->segment.position);
        res = TRUE;
      }
    }
      break;
    case GST_QUERY_DURATION:{
      ...
      if (fmt == GST_FORMAT_TIME) {
        ...
        if (!res) {
          GstClockTime duration;
          if (gst_mmtdemux_get_duration (mmtdemux, &duration) && duration > 0) {
    ...
    case GST_QUERY_CONVERT:{
      ...
      res = gst_mmtdemux_src_convert (mmtdemux, pad,
    ...
    case GST_QUERY_SEEKING:{
      ...
      if (!res) {
        ...
        if (fmt == GST_FORMAT_TIME) {
          ...
          gst_mmtdemux_get_duration (mmtdemux, &duration);
          seekable = TRUE;
          if (!mmtdemux->pullbased) {
            GstQuery *q;

            /* we might be able with help from upstream */
            seekable = FALSE;
            q = gst_query_new_seeking (GST_FORMAT_BYTES);
            if (gst_pad_peer_query (mmtdemux->sinkpad, q)) {
              gst_query_parse_seeking (q, &fmt, &seekable, NULL, NULL);
              GST_DEBUG_OBJECT (mmtdemux, "upstream BYTE seekable %d", seekable);
            }
            gst_query_unref (q);
          }
    ...
    case GST_QUERY_SEGMENT:
    {
      ...
      format = mmtdemux->segment.format;

      start =
          gst_segment_to_stream_time (&mmtdemux->segment, format,
          mmtdemux->segment.start);
      if ((stop = mmtdemux->segment.stop) == -1)
        stop = mmtdemux->segment.duration;
      else
        stop = gst_segment_to_stream_time (&mmtdemux->segment, format, stop);

      gst_query_set_segment (query, mmtdemux->segment.rate, format, start, stop);
      res = TRUE;
      break;
    }
    default:
...
static void
gst_mmtdemux_push_tags (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  if (G_LIKELY (stream->pad)) {
    GST_DEBUG_OBJECT (mmtdemux, "Checking pad %s:%s for tags",
        GST_DEBUG_PAD_NAME (stream->pad));

    if (!gst_tag_list_is_empty (stream->stream_tags)) {
      GST_DEBUG_OBJECT (mmtdemux, "Sending tags %" GST_PTR_FORMAT,
          stream->stream_tags);
    ...
    if (G_UNLIKELY (stream->send_global_tags)) {
      GST_DEBUG_OBJECT (mmtdemux, "Sending global tags %" GST_PTR_FORMAT,
          mmtdemux->tag_list);
      gst_pad_push_event (stream->pad,
          gst_event_new_tag (gst_tag_list_ref (mmtdemux->tag_list)));
...
static void
gst_mmtdemux_push_event (GstMMTDemux * mmtdemux, GstEvent * event)
{
  guint n;
  gboolean has_valid_stream = FALSE;
  GstEventType etype = GST_EVENT_TYPE (event);

  GST_DEBUG_OBJECT (mmtdemux, "pushing %s event on all source pads",
      GST_EVENT_TYPE_NAME (event));

  for (n = 0; n < mmtdemux->n_streams; n++) {
    GstPad *pad;
    QtDemuxStream *stream = mmtdemux->streams[n];
    GST_DEBUG_OBJECT (mmtdemux, "pushing on pad %i", n);
  ...
  if (!has_valid_stream && etype == GST_EVENT_EOS) {
    gst_mmtdemux_post_no_playable_stream_error (mmtdemux);
  }
}

/* push a pending newsegment event, if any from the streaming thread */
static void
gst_mmtdemux_push_pending_newsegment (GstMMTDemux * mmtdemux)
{
  if (mmtdemux->pending_newsegment) {
    gst_mmtdemux_push_event (mmtdemux, mmtdemux->pending_newsegment);
    mmtdemux->pending_newsegment = NULL;
  }
}
 ...
 */
static guint32
gst_mmtdemux_find_index (GstMMTDemux * mmtdemux, QtDemuxStream * str,
    guint64 media_time)
{
    ...
    QtDemuxStream * str, gint64 media_offset)
{
  ...
  while (index < str->n_samples - 1) {
    if (!mmtdemux_parse_samples (mmtdemux, str, index + 1))
      goto parse_failed;
...
parse_failed:
  {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing of index %u failed!", index + 1);
    return -1;
  }
}
    ...
    GstClockTime media_time)
{
  ...
  if (str->stbl_index >= 0 &&
    ...
    return gst_mmtdemux_find_index (mmtdemux, str, media_time);
 ...
 */
static guint32
gst_mmtdemux_find_keyframe (GstMMTDemux * mmtdemux, QtDemuxStream * str,
    guint32 index, gboolean next)
{
  ...
  while (new_index < str->n_samples) {
    if (next && !mmtdemux_parse_samples (mmtdemux, str, new_index))
      goto parse_failed;
  ...
  if (new_index == str->n_samples) {
    GST_DEBUG_OBJECT (mmtdemux, "no next keyframe");
    new_index = -1;
  }

beach:
  GST_DEBUG_OBJECT (mmtdemux, "searching for keyframe index %s index %u "
...
parse_failed:
  {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing of index %u failed!", new_index);
    return -1;
  }
}
 ...
 */
static guint32
gst_mmtdemux_find_segment (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GstClockTime time_position)
{
  gint i;
  guint32 seg_idx;

  GST_DEBUG_OBJECT (stream->pad, "finding segment for %" GST_TIME_FORMAT,
  ...
  for (i = 0; i < stream->n_segments; i++) {
    ...
    if (i < stream->n_segments - 1) {
      if (segment->time <= time_position && time_position < segment->stop_time) {
        GST_DEBUG_OBJECT (stream->pad, "segment %d matches", i);
        seg_idx = i;
        break;
      }
    } else {
 ...
 */
static void
gst_mmtdemux_move_stream (GstMMTDemux * mmtdemux, QtDemuxStream * str,
    guint32 index)
{
  /* no change needed */
  ...
  GST_DEBUG_OBJECT (mmtdemux, "moving to sample %u of %u", index,
      str->n_samples);
...
static void
gst_mmtdemux_adjust_seek (GstMMTDemux * mmtdemux, gint64 desired_time,
    gboolean use_sparse, gboolean next, gint64 * key_time, gint64 * key_offset)
{
  ...
  for (n = 0; n < mmtdemux->n_streams; n++) {
    ...
    str = mmtdemux->streams[n];

    if (CUR_STREAM (str)->sparse && !use_sparse)
      continue;

    seg_idx = gst_mmtdemux_find_segment (mmtdemux, str, desired_time);
    GST_DEBUG_OBJECT (mmtdemux, "align segment %d", seg_idx);
    ...
    index = gst_mmtdemux_find_index_linear (mmtdemux, str, media_start);
    GST_DEBUG_OBJECT (mmtdemux, "sample for %" GST_TIME_FORMAT " at %u"
    ...
    if (!empty_segment) {
      ...
      kindex = gst_mmtdemux_find_keyframe (mmtdemux, str, index, next);

      /* we will settle for one before if none found after */
      if (next && kindex == -1)
        kindex = gst_mmtdemux_find_keyframe (mmtdemux, str, index, FALSE);
...
static gboolean
gst_mmtdemux_convert_seek (GstPad * pad, GstFormat * format,
...
*/
static gboolean
gst_mmtdemux_do_push_seek (GstMMTDemux * mmtdemux, GstPad * pad, GstEvent * event)
{
  gdouble rate;
  GstFormat format;
  GstSeekFlags flags;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "doing push-based seek");
  ...
  if (!gst_mmtdemux_convert_seek (pad, &format, cur_type, &cur,
          stop_type, &stop))
    goto no_format;

  /* Upstream seek in bytes will have undefined stop, but mmtdemux stores
  ...
  gst_mmtdemux_adjust_seek (mmtdemux, cur, FALSE, FALSE, &key_cur, &byte_cur);

  if (byte_cur == -1)
    goto abort_seek;

  GST_DEBUG_OBJECT (mmtdemux, "Pushing BYTE seek rate %g, "
      "start %" G_GINT64_FORMAT ", stop %" G_GINT64_FORMAT, rate, byte_cur,
      stop);

  GST_OBJECT_LOCK (mmtdemux);
  mmtdemux->seek_offset = byte_cur;
  if (!(flags & GST_SEEK_FLAG_KEY_UNIT)) {
    mmtdemux->push_seek_start = cur;
  } else {
    mmtdemux->push_seek_start = key_cur;
  }

  if (stop_type == GST_SEEK_TYPE_NONE) {
    mmtdemux->push_seek_stop = mmtdemux->segment.stop;
  } else {
    mmtdemux->push_seek_stop = original_stop;
  }
  GST_OBJECT_UNLOCK (mmtdemux);

  /* BYTE seek event */
  event = gst_event_new_seek (rate, GST_FORMAT_BYTES, flags, cur_type, byte_cur,
      stop_type, stop);
  gst_event_set_seqnum (event, seqnum);
  res = gst_pad_push_event (mmtdemux->sinkpad, event);
...
abort_seek:
  {
    GST_DEBUG_OBJECT (mmtdemux, "could not determine byte position to seek to, "
        "seek aborted.");
    return FALSE;
  }
unsupported_seek:
  {
    GST_DEBUG_OBJECT (mmtdemux, "unsupported seek, seek aborted.");
    return FALSE;
  }
no_format:
  {
    GST_DEBUG_OBJECT (mmtdemux, "unsupported format given, seek aborted.");
    return FALSE;
  }
}
 ...
 */
static gboolean
gst_mmtdemux_perform_seek (GstMMTDemux * mmtdemux, GstSegment * segment,
    guint32 seqnum, GstSeekFlags flags)
{
  gint64 desired_offset;
  gint n;

  mmtdemux->seek_position = desired_offset = segment->position;

  GST_DEBUG_OBJECT (mmtdemux, "seeking to %" GST_TIME_FORMAT,
  ...
  if ((flags & GST_SEEK_FLAG_KEY_UNIT) && !mmtdemux->fragmented) {
    gint64 min_offset;
    ...
    gst_mmtdemux_adjust_seek (mmtdemux, desired_offset, TRUE, next, &min_offset,
        NULL);
    GST_DEBUG_OBJECT (mmtdemux, "keyframe seek, align to %"
        GST_TIME_FORMAT, GST_TIME_ARGS (min_offset));
    desired_offset = min_offset;
  }

  /* and set all streams to the final position */
  gst_flow_combiner_reset (mmtdemux->flowcombiner);
  mmtdemux->segment_seqnum = seqnum;
  for (n = 0; n < mmtdemux->n_streams; n++) {
    QtDemuxStream *stream = mmtdemux->streams[n];
  ...
  if (mmtdemux->fragmented)
    mmtdemux->fragmented_seek_pending = TRUE;
...
static gboolean
gst_mmtdemux_do_seek (GstMMTDemux * mmtdemux, GstPad * pad, GstEvent * event)
{
  gdouble rate;
  GstFormat format;
  GstSeekFlags flags;
  ...
  if (event) {
    GST_DEBUG_OBJECT (mmtdemux, "doing seek with event");
    ...
    GST_DEBUG_OBJECT (mmtdemux, "seek format %s", gst_format_get_name (format));
  } else {
    GST_DEBUG_OBJECT (mmtdemux, "doing seek without event");
    flags = 0;
  }
  ...
  if (flush) {
    ...
    gst_pad_push_event (mmtdemux->sinkpad, gst_event_ref (flush_event));
    /* make sure out loop function exits */
    gst_mmtdemux_push_event (mmtdemux, flush_event);
  } else {
    /* non flushing seek, pause the task */
    gst_pad_pause_task (mmtdemux->sinkpad);
  }

  /* wait for streaming to finish */
  GST_PAD_STREAM_LOCK (mmtdemux->sinkpad);

  /* copy segment, we need this because we still need the old
   * segment when we close the current segment. */
  memcpy (&seeksegment, &mmtdemux->segment, sizeof (GstSegment));

  if (event) {
    /* configure the segment with the seek variables */
    GST_DEBUG_OBJECT (mmtdemux, "configuring seek");
    if (!gst_segment_do_seek (&seeksegment, rate, format, flags,
            cur_type, cur, stop_type, stop, &update)) {
      ret = FALSE;
      GST_ERROR_OBJECT (mmtdemux, "inconsistent seek values, doing nothing");
    } else {
      /* now do the seek */
      ret = gst_mmtdemux_perform_seek (mmtdemux, &seeksegment, seqnum, flags);
    }
  } else {
    /* now do the seek */
    ret = gst_mmtdemux_perform_seek (mmtdemux, &seeksegment, seqnum, flags);
  ...
  memcpy (&mmtdemux->segment, &seeksegment, sizeof (GstSegment));

  if (mmtdemux->segment.flags & GST_SEEK_FLAG_SEGMENT) {
    GstMessage *msg = gst_message_new_segment_start (GST_OBJECT_CAST (mmtdemux),
        mmtdemux->segment.format, mmtdemux->segment.position);
    if (seqnum != GST_SEQNUM_INVALID)
      gst_message_set_seqnum (msg, seqnum);
    gst_element_post_message (GST_ELEMENT_CAST (mmtdemux), msg);
  }

  /* restart streaming, NEWSEGMENT will be sent from the streaming thread. */
  gst_pad_start_task (mmtdemux->sinkpad, (GstTaskFunction) gst_mmtdemux_loop,
      mmtdemux->sinkpad, NULL);

  GST_PAD_STREAM_UNLOCK (mmtdemux->sinkpad);
...
static gboolean
mmtdemux_ensure_index (GstMMTDemux * mmtdemux)
{
  guint i;

  GST_DEBUG_OBJECT (mmtdemux, "collecting all metadata for all streams");

  /* Build complete index */
  for (i = 0; i < mmtdemux->n_streams; i++) {
    QtDemuxStream *stream = mmtdemux->streams[i];

    if (!mmtdemux_parse_samples (mmtdemux, stream, stream->n_samples - 1))
      goto parse_error;
  }
  return TRUE;
...
static gboolean
gst_mmtdemux_handle_src_event (GstPad * pad, GstObject * parent,
    GstEvent * event)
{
  gboolean res = TRUE;
  GstMMTDemux *mmtdemux = GST_QTDEMUX (parent);
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
    {
      ...
      if (seqnum == mmtdemux->segment_seqnum) {
      ...
      if (mmtdemux->upstream_format_is_time && mmtdemux->fragmented) {
      ...
      if (!mmtdemux->fragmented)
        if (!mmtdemux_ensure_index (mmtdemux))
          goto index_failed;
#ifndef GST_DISABLE_GST_DEBUG
      ts = gst_util_get_timestamp () - ts;
      GST_INFO_OBJECT (mmtdemux,
          "Time taken to parse index %" GST_TIME_FORMAT, GST_TIME_ARGS (ts));
#endif
    }
      if (mmtdemux->pullbased) {
        res = gst_mmtdemux_do_seek (mmtdemux, pad, event);
      } else if (gst_pad_push_event (mmtdemux->sinkpad, gst_event_ref (event))) {
        GST_DEBUG_OBJECT (mmtdemux, "Upstream successfully seeked");
        res = TRUE;
      } else if (mmtdemux->state == QTDEMUX_STATE_MOVIE && mmtdemux->n_streams
          && !mmtdemux->fragmented) {
        res = gst_mmtdemux_do_push_seek (mmtdemux, pad, event);
      } else {
        GST_DEBUG_OBJECT (mmtdemux,
...
index_failed:
  {
    GST_ERROR_OBJECT (mmtdemux, "Index failed");
    gst_event_unref (event);
    res = FALSE;
    goto done;
  }
 ...
 * @_stream and @_index. */
static void
gst_mmtdemux_find_sample (GstMMTDemux * mmtdemux, gint64 byte_pos, gboolean fw,
    gboolean set, QtDemuxStream ** _stream, gint * _index, gint64 * _time)
{
  ...
  for (n = 0; n < mmtdemux->n_streams; ++n) {
    ...
    for (; (i >= 0) && (i < str->n_samples); i += inc) {
      ...
      if (set) {
        gst_mmtdemux_move_stream (mmtdemux, str, i);
        set_sample = TRUE;
      }
    ...
    if (!set_sample)
      gst_mmtdemux_move_stream (mmtdemux, str, str->n_samples);
  }
...
static gboolean
gst_mmtdemux_setcaps (GstMMTDemux * demux, GstCaps * caps)
{
...
static void
gst_mmtdemux_reset (GstMMTDemux * mmtdemux, gboolean hard)
{
  gint n;

  GST_DEBUG_OBJECT (mmtdemux, "Resetting demux");
  gst_pad_stop_task (mmtdemux->sinkpad);

  if (hard || mmtdemux->upstream_format_is_time) {
    mmtdemux->state = QTDEMUX_STATE_INITIAL;
    mmtdemux->neededbytes = 16;
    mmtdemux->todrop = 0;
    mmtdemux->pullbased = FALSE;
    mmtdemux->posted_redirect = FALSE;
    mmtdemux->first_mdat = -1;
    mmtdemux->header_size = 0;
    mmtdemux->mdatoffset = -1;
    mmtdemux->restoredata_offset = -1;
    if (mmtdemux->mdatbuffer)
      gst_buffer_unref (mmtdemux->mdatbuffer);
    if (mmtdemux->restoredata_buffer)
      gst_buffer_unref (mmtdemux->restoredata_buffer);
    mmtdemux->mdatbuffer = NULL;
    mmtdemux->restoredata_buffer = NULL;
    mmtdemux->mdatleft = 0;
    mmtdemux->mdatsize = 0;
    if (mmtdemux->comp_brands)
      gst_buffer_unref (mmtdemux->comp_brands);
    mmtdemux->comp_brands = NULL;
    mmtdemux->last_moov_offset = -1;
    if (mmtdemux->moov_node_compressed) {
      g_node_destroy (mmtdemux->moov_node_compressed);
      if (mmtdemux->moov_node)
        g_free (mmtdemux->moov_node->data);
    }
    mmtdemux->moov_node_compressed = NULL;
    if (mmtdemux->moov_node)
      g_node_destroy (mmtdemux->moov_node);
    mmtdemux->moov_node = NULL;
    if (mmtdemux->tag_list)
      gst_mini_object_unref (GST_MINI_OBJECT_CAST (mmtdemux->tag_list));
    mmtdemux->tag_list = gst_tag_list_new_empty ();
    gst_tag_list_set_scope (mmtdemux->tag_list, GST_TAG_SCOPE_GLOBAL);
#if 0
    if (mmtdemux->element_index)
      gst_object_unref (mmtdemux->element_index);
    mmtdemux->element_index = NULL;
#endif
    mmtdemux->major_brand = 0;
    if (mmtdemux->pending_newsegment)
      gst_event_unref (mmtdemux->pending_newsegment);
    mmtdemux->pending_newsegment = NULL;
    mmtdemux->upstream_format_is_time = FALSE;
    mmtdemux->upstream_seekable = FALSE;
    mmtdemux->upstream_size = 0;

    mmtdemux->fragment_start = -1;
    mmtdemux->fragment_start_offset = -1;
    mmtdemux->duration = 0;
    mmtdemux->is_mmpu = 0;
    mmtdemux->moof_offset = 0;
    mmtdemux->chapters_track_id = 0;
    mmtdemux->have_group_id = FALSE;
    mmtdemux->group_id = G_MAXUINT;
  ...
  mmtdemux->offset = 0;
  gst_adapter_clear (mmtdemux->adapter);
  gst_segment_init (&mmtdemux->segment, GST_FORMAT_TIME);
  mmtdemux->segment_seqnum = GST_SEQNUM_INVALID;

  if (hard) {
    for (n = 0; n < mmtdemux->n_streams; n++) {
      gst_mmtdemux_stream_free (mmtdemux, mmtdemux->streams[n]);
      mmtdemux->streams[n] = NULL;
    }
    mmtdemux->n_streams = 0;
    mmtdemux->n_video_streams = 0;
    mmtdemux->n_audio_streams = 0;
    mmtdemux->n_sub_streams = 0;
    mmtdemux->exposed = FALSE;
    mmtdemux->fragmented = FALSE;
    mmtdemux->mss_mode = FALSE;
    gst_caps_replace (&mmtdemux->media_caps, NULL);
    mmtdemux->timescale = 0;
    mmtdemux->got_moov = FALSE;
    if (mmtdemux->protection_system_ids) {
      g_ptr_array_free (mmtdemux->protection_system_ids, TRUE);
      mmtdemux->protection_system_ids = NULL;
    }
  } else if (mmtdemux->mss_mode) {
    gst_flow_combiner_reset (mmtdemux->flowcombiner);
    for (n = 0; n < mmtdemux->n_streams; n++)
      gst_mmtdemux_stream_clear (mmtdemux, mmtdemux->streams[n]);
  } else {
    gst_flow_combiner_reset (mmtdemux->flowcombiner);
    for (n = 0; n < mmtdemux->n_streams; n++) {
      mmtdemux->streams[n]->sent_eos = FALSE;
      mmtdemux->streams[n]->time_position = 0;
      mmtdemux->streams[n]->accumulated_base = 0;
    }
    if (!mmtdemux->pending_newsegment) {
      mmtdemux->pending_newsegment = gst_event_new_segment (&mmtdemux->segment);
      if (mmtdemux->segment_seqnum != GST_SEQNUM_INVALID)
        gst_event_set_seqnum (mmtdemux->pending_newsegment,
            mmtdemux->segment_seqnum);
    }
  }
}
...
static void
gst_mmtdemux_map_and_push_segments (GstMMTDemux * mmtdemux, GstSegment * segment)
{
  gint n, i;
  ...
  for (n = 0; n < mmtdemux->n_streams; n++) {
    ...
    for (i = 0; i < stream->n_segments; i++) {
      if (stream->segments[i].stop_time > segment->start) {
        gst_mmtdemux_activate_segment (mmtdemux, stream, i,
            stream->time_position);
        if (QTSEGMENT_IS_EMPTY (&stream->segments[i])) {
          /* push the empty segment and move to the next one */
          gst_mmtdemux_send_gap_for_segment (mmtdemux, stream, i,
              stream->time_position);
          continue;
        }
...
static gboolean
gst_mmtdemux_handle_sink_event (GstPad * sinkpad, GstObject * parent,
    GstEvent * event)
{
  GstMMTDemux *demux = GST_QTDEMUX (parent);
  gboolean res = TRUE;

  GST_DEBUG_OBJECT (demux, "handling %s event", GST_EVENT_TYPE_NAME (event));
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEGMENT:
    {
      ...
      if (segment.format == GST_FORMAT_BYTES) {
        if (GST_CLOCK_TIME_IS_VALID (segment.start)) {
          ...
          gst_mmtdemux_find_sample (demux, segment.start, TRUE, FALSE, NULL,
              NULL, (gint64 *) & segment.start);
          if ((gint64) segment.start < 0)
            segment.start = 0;
        }
        if (GST_CLOCK_TIME_IS_VALID (segment.stop)) {
          gst_mmtdemux_find_sample (demux, segment.stop, FALSE, FALSE, NULL,
         ...
         * new segment */
        gst_mmtdemux_process_adapter (demux, TRUE);
		  if(0)
				bkpgst_mmtdemux_process_adapter (demux, TRUE);
      } else {
      ...
      if (demux->n_streams) {
        if (demux->fragmented) {
          ...
          gst_mmtdemux_push_event (demux, segment_event);
        } else {
          gst_event_replace (&demux->pending_newsegment, NULL);
          gst_mmtdemux_map_and_push_segments (demux, &segment);
        }
      }
      ...
      if (demux->upstream_format_is_time) {
        ...
        gst_mmtdemux_find_sample (demux, offset, TRUE, TRUE, &stream, &idx,
            NULL);
        if (stream) {
    ...
    case GST_EVENT_FLUSH_STOP:
    {
      ...
      gst_mmtdemux_reset (demux, FALSE);
    ...
    case GST_EVENT_EOS:
      ...
      if (!demux->pullbased) {
        ...
        if (!has_valid_stream)
          gst_mmtdemux_post_no_playable_stream_error (demux);
        else {
          GST_DEBUG_OBJECT (demux, "Data still available after EOS: %u",
              (guint) gst_adapter_available (demux->adapter));
          if (gst_mmtdemux_process_adapter (demux, TRUE) != GST_FLOW_OK) {
            res = FALSE;
          }
        }
      }
    ...
    case GST_EVENT_CAPS:{
      ...
      gst_mmtdemux_setcaps (demux, caps);
      res = TRUE;
      gst_event_unref (event);
      goto drop;
    }
    case GST_EVENT_PROTECTION:
    {
      ...
      GST_DEBUG_OBJECT (demux, "Received protection event for system ID %s",
          system_id);
      gst_mmtdemux_append_protection_system_id (demux, system_id);
      ...
      gst_mmtdemux_push_event (demux, event);
      res = TRUE;
      goto drop;
    }
    default:
...
#if 0
static void
gst_mmtdemux_set_index (GstElement * element, GstIndex * index)
{
  GstMMTDemux *demux = GST_QTDEMUX (element);
...
static GstIndex *
gst_mmtdemux_get_index (GstElement * element)
{
...
static void
gst_mmtdemux_stbl_free (QtDemuxStream * stream)
{
...
static void
gst_mmtdemux_stream_flush_segments_data (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream)
{
...
static void
gst_mmtdemux_stream_flush_samples_data (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream)
{
  g_free (stream->samples);
  stream->samples = NULL;
  gst_mmtdemux_stbl_free (stream);
...
static void
gst_mmtdemux_stream_clear (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  gint i;
  if (stream->allocator)
  ...
  gst_mmtdemux_stream_flush_segments_data (mmtdemux, stream);
  gst_mmtdemux_stream_flush_samples_data (mmtdemux, stream);
}

static void
gst_mmtdemux_stream_reset (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  gint i;
  gst_mmtdemux_stream_clear (mmtdemux, stream);
  for (i = 0; i < stream->stsd_entries_length; i++) {
    QtDemuxStreamStsdEntry *entry = &stream->stsd_entries[i];
    if (entry->caps) {
...
static void
gst_mmtdemux_stream_free (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  gst_mmtdemux_stream_reset (mmtdemux, stream);
  gst_tag_list_unref (stream->stream_tags);
  if (stream->pad) {
    gst_element_remove_pad (GST_ELEMENT_CAST (mmtdemux), stream->pad);
    gst_flow_combiner_remove_pad (mmtdemux->flowcombiner, stream->pad);
  }
  g_free (stream);
}
void
...
static void
gst_mmtdemux_remove_stream (GstMMTDemux * mmtdemux, int i)
{
  g_assert (i >= 0 && i < mmtdemux->n_streams && mmtdemux->streams[i] != NULL);

  gst_mmtdemux_stream_free (mmtdemux, mmtdemux->streams[i]);
  mmtdemux->streams[i] = mmtdemux->streams[mmtdemux->n_streams - 1];
  mmtdemux->streams[mmtdemux->n_streams - 1] = NULL;
  mmtdemux->n_streams--;
}

static GstStateChangeReturn
gst_mmtdemux_change_state (GstElement * element, GstStateChange transition)
{
  GstMMTDemux *mmtdemux = GST_QTDEMUX (element);
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PAUSED_TO_READY:{
      gst_mmtdemux_reset (mmtdemux, TRUE);
      break;
    }
    default:
      break;
...
mmtdemux_parse_ftyp (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  ...
  mmtdemux->header_size += length;

  /* only consider at least a sufficiently complete ftyp atom */
  if (length >= 20) {
    GstBuffer *buf;

    mmtdemux->major_brand = QT_FOURCC (buffer + 8);
    GST_DEBUG_OBJECT (mmtdemux, "major brand: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->major_brand));
    if (mmtdemux->comp_brands)
      gst_buffer_unref (mmtdemux->comp_brands);
    buf = mmtdemux->comp_brands = gst_buffer_new_and_alloc (length - 16);
...
mmtdemux_parse_mmpu (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  ...
  /* only consider at least a sufficiently complete ftyp atom */
	 buffer +=8; //skipping offser(4)+fourcc(4)
	 bitfield =  QT_UINT8((guint8*) buffer);
	 mmtdemux->is_complete = bitfield & 0x01;
	 mmtdemux->is_adc_present = bitfield & 0x02;
	 mmtdemux->reserved = bitfield & 0xFC;
	 buffer +=1;
	 mmtdemux->mpu_sequence_number = QT_UINT32((guint32*)buffer);
	 buffer +=4;
	 mmtdemux->asset_id_scheme = QT_UINT32((guint32*)buffer);
	 buffer +=4;
	 mmtdemux->asset_id_length = QT_UINT32((guint32*)buffer);
    GST_DEBUG_OBJECT (mmtdemux,
          "is_complete: %" G_GUINT32_FORMAT ", is_adc_present: %" G_GUINT32_FORMAT 
           ", mpu_sequence_number: %" G_GUINT32_FORMAT", asset_id_scheme: %" G_GUINT32_FORMAT ", asset_id_length: %" G_GUINT32_FORMAT,
          mmtdemux->is_complete, mmtdemux->is_adc_present, 
		  mmtdemux->mpu_sequence_number, mmtdemux->asset_id_scheme, mmtdemux->asset_id_length);
#if 0
  version = QT_UINT8 ((guint8 *) mvhd->data + 8);
  if (version == 1) {
    creation_time = QT_UINT64 ((guint8 *) mvhd->data + 12);
    mmtdemux->timescale = QT_UINT32 ((guint8 *) mvhd->data + 28);
    mmtdemux->duration = QT_UINT64 ((guint8 *) mvhd->data + 32);
  } else if (version == 0) {
    creation_time = QT_UINT32 ((guint8 *) mvhd->data + 12);
    mmtdemux->timescale = QT_UINT32 ((guint8 *) mvhd->data + 20);
    mmtdemux->duration = QT_UINT32 ((guint8 *) mvhd->data + 24);
  } else {
    GST_WARNING_OBJECT (mmtdemux, "Unhandled mvhd version %d", version);
    return FALSE;
  }
...
static void
mmtdemux_parse_mmth (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  mmtdemux->header_size += length;

  /* only consider at least a sufficiently complete mmth atom */
}


#if 0
static void
mmtdemux_parse_mmthsample (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  mmtdemux->header_size += length;
  ...
  if (length >= 23) {
    ...
    mmtdemux->Sequence_Number = QT_UINT32((guint32*) buffer);;
    GST_DEBUG_OBJECT (mmtdemux, "Sequence_Number: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->Sequence_Number));
    buffer +=4;   

   mmtdemux->trackrefindex = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "TrackRefIndex: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->trackrefindex));
    buffer +=1;

   mmtdemux->movie_fragemnt_sequence_number = QT_UINT8((guint32*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "movie_fragemnt_sequence_number: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->movie_fragemnt_sequence_number));
    buffer +=4;

   mmtdemux->SampleNumber = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "SampleNumber: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->SampleNumber));
    buffer +=1;

   mmtdemux->priority = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "priority: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->priority));
    buffer +=1;

   mmtdemux->dependency_counter = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "dependency_counter: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->dependency_counter));
    buffer +=1;

   mmtdemux->MFU_offset = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "offset: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->MFU_offset));
    buffer +=4;

  mmtdemux->length = QT_UINT32((guint32*) buffer);
  GST_DEBUG_OBJECT (mmtdemux, "length: %" GST_FOURCC_FORMAT,
  GST_FOURCC_ARGS (mmtdemux->length));
   
  }
}
#endif
static void
mmtdemux_parse_hmhd (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  mmtdemux->header_size += length;
  ...
  if (length >= 23) {
    ...
    mmtdemux->maxPDUsize = QT_UINT16((guint16*) buffer);;
    GST_DEBUG_OBJECT (mmtdemux, "maxPDUsize: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->maxPDUsize));
    buffer +=2;   

   mmtdemux->avgPDUsize = QT_UINT16((guint16*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "avgPDUsize: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->avgPDUsize));
    buffer +=2;

   mmtdemux->maxbitrate = QT_UINT8((guint32*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "maxbitrate: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->maxbitrate));
    buffer +=4;

   mmtdemux->avgbitrate = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "avgbitrate: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->avgbitrate));
    buffer +=4;

   mmtdemux->hmhdreserved = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (mmtdemux, "reserved: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (mmtdemux->hmhdreserved));
...
static void
mmtdemux_handle_xmp_taglist (GstMMTDemux * mmtdemux, GstTagList * taglist,
    GstTagList * xmptaglist)
{
  ...
  if (xmptaglist) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "Found XMP tags %" GST_PTR_FORMAT, xmptaglist);
...
static void
mmtdemux_parse_piff (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length,
    guint offset)
{
  GstByteReader br;
  guint8 version;
  ...
  if (mmtdemux->n_streams == 0)
    return;

  stream = mmtdemux->streams[0];
  ...
  gst_mmtdemux_append_protection_system_id (mmtdemux, system_id);
  ...
  if (ss_info->crypto_info) {
    GST_DEBUG_OBJECT (mmtdemux, "unreffing existing crypto_info");
  ...
  if (!gst_byte_reader_get_uint8 (&br, &version)) {
    GST_ERROR_OBJECT (mmtdemux, "Error getting box's version field");
    return;
  }

  if (!gst_byte_reader_get_uint24_be (&br, &flags)) {
    GST_ERROR_OBJECT (mmtdemux, "Error getting box's flags field");
    return;
  }
  ...
  if ((flags & 0x000001)) {
    ...
    if (!gst_byte_reader_get_uint24_le (&br, &algorithm_id)) {
      GST_ERROR_OBJECT (mmtdemux, "Error getting box's algorithm ID field");
      return;
    }
    ...
    if (algorithm_id == 0) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "AES 128-bits CTR encrypted stream");
    } else if (algorithm_id == 2) {
      /* FIXME: maybe store this in properties? */
      GST_DEBUG_OBJECT (mmtdemux, "AES 128-bits CBC encrypted stream");
    }
    ...
    GST_DEBUG_OBJECT (mmtdemux, "default sample properties: "
  ...
  if (!gst_byte_reader_get_uint32_be (&br, &mmtdemux->cenc_aux_sample_count)) {
    GST_ERROR_OBJECT (mmtdemux, "Error getting box's sample count field");
    return;
  }

  ss_info->crypto_info =
      g_ptr_array_new_full (mmtdemux->cenc_aux_sample_count,
      (GDestroyNotify) mmtdemux_gst_structure_free);

  for (i = 0; i < mmtdemux->cenc_aux_sample_count; ++i) {
    GstStructure *properties;
    guint8 *data;
    GstBuffer *buf;

    properties = mmtdemux_get_cenc_sample_properties (mmtdemux, stream, i);
    if (properties == NULL) {
      GST_ERROR_OBJECT (mmtdemux, "failed to get properties for sample %u", i);
      return;
    }

    if (!gst_byte_reader_dup_data (&br, iv_size, &data)) {
      GST_ERROR_OBJECT (mmtdemux, "IV data not present for sample %u", i);
    ...
    if (uses_sub_sample_encryption) {
      ...
      if (!gst_byte_reader_get_uint16_be (&br, &n_subsamples)
          || n_subsamples == 0) {
        GST_ERROR_OBJECT (mmtdemux,
            "failed to get subsample count for sample %u", i);
        gst_structure_free (properties);
        return;
      }
      GST_DEBUG_OBJECT (mmtdemux, "subsample count: %u", n_subsamples);
      if (!gst_byte_reader_dup_data (&br, n_subsamples * 6, &data)) {
        GST_ERROR_OBJECT (mmtdemux, "failed to get subsample data for sample %u",
            i);
        gst_structure_free (properties);
        return;
      }
...
static void
mmtdemux_parse_uuid (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  ...
  if (length <= offset + 16) {
    GST_DEBUG_OBJECT (mmtdemux, "uuid atom is too short, skipping");
    return;
  }
  ...
  if (memcmp (buffer + offset, xmp_uuid, 16) == 0) {
    ...
    mmtdemux->tag_list = gst_tag_list_make_writable (mmtdemux->tag_list);

    mmtdemux_handle_xmp_taglist (mmtdemux, mmtdemux->tag_list, taglist);
    ...
    GST_ERROR_OBJECT (mmtdemux, "contents: %s", contents);

    g_free (contents);

    GST_ELEMENT_ERROR (mmtdemux, STREAM, DECRYPT,
    ...
    mmtdemux_parse_piff (mmtdemux, buffer, length, offset);
  } else {
    GST_DEBUG_OBJECT (mmtdemux, "Ignoring unknown uuid: %08x-%08x-%08x-%08x",
...
static void
mmtdemux_parse_sidx (GstMMTDemux * mmtdemux, const guint8 * buffer, gint length)
{
  ...
  res =
      ...
      &consumed);
  GST_DEBUG_OBJECT (mmtdemux, "sidx parse result: %d", res);
  if (res == GST_ISOFF_QT_PARSER_DONE) {
    check_update_duration (mmtdemux, sidx_parser.cumulative_pts);
  }
...
static gboolean
mmtdemux_parse_mehd (GstMMTDemux * mmtdemux, GstByteReader * br)
{
  guint32 version = 0;
  ...
  GST_INFO_OBJECT (mmtdemux, "mehd duration: %" G_GUINT64_FORMAT, duration);
  mmtdemux->duration = duration;

  return TRUE;

failed:
  {
    GST_DEBUG_OBJECT (mmtdemux, "parsing mehd failed");
    return FALSE;
  }
}

static gboolean
mmtdemux_parse_trex (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    guint32 * ds_duration, guint32 * ds_size, guint32 * ds_flags)
{
  if (!stream->parsed_trex && mmtdemux->moov_node) {
    GNode *mvex, *trex;
    GstByteReader trex_data;
	 GST_DEBUG_OBJECT(mmtdemux, "AAAAAAAAAAA not parsed and moov_node");

    mvex = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_mvex);
    if (mvex) {
      trex = mmtdemux_tree_get_child_by_type_full (mvex, FOURCC_trex,
          &trex_data);
      while (trex) {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "fragment defaults for stream %d; "
      ...
      next:
        ...
        trex = mmtdemux_tree_get_sibling_by_type_full (trex, FOURCC_trex,
            &trex_data);
      }
    }
  }
 ...
 */
static void
check_update_duration (GstMMTDemux * mmtdemux, GstClockTime duration)
{
  guint i;
  guint64 movdur;
  GstClockTime prevdur;

  movdur = GSTTIME_TO_QTTIME (mmtdemux, duration);

  if (movdur > mmtdemux->duration) {
    prevdur = QTTIME_TO_GSTTIME (mmtdemux, mmtdemux->duration);
    GST_DEBUG_OBJECT (mmtdemux,
        "Updating total duration to %" GST_TIME_FORMAT " was %" GST_TIME_FORMAT,
        GST_TIME_ARGS (duration), GST_TIME_ARGS (prevdur));
    mmtdemux->duration = movdur;
    GST_DEBUG_OBJECT (mmtdemux,
        "mmtdemux->segment.duration: %" GST_TIME_FORMAT " .stop: %"
        GST_TIME_FORMAT, GST_TIME_ARGS (mmtdemux->segment.duration),
        GST_TIME_ARGS (mmtdemux->segment.stop));
    if (mmtdemux->segment.duration == prevdur) {
      ...
      GstClockTime fixeddur = QTTIME_TO_GSTTIME (mmtdemux, movdur);
      GST_DEBUG_OBJECT (mmtdemux, "Updated segment.duration and segment.stop");
      mmtdemux->segment.duration = fixeddur;
      mmtdemux->segment.stop = fixeddur;
    }
  }
  for (i = mmtdemux->mediaType==1?0:1; i < mmtdemux->n_streams; i++) {
    QtDemuxStream *stream = mmtdemux->streams[i];
    if (stream) {
      ...
      if (movdur > stream->duration) {
        ...
        if (stream->dummy_segment &&
          ...
          if (stream->segment_index != -1) {
            ...
            if (mmtdemux->segment.rate >= 0) {
              pos = stream->segment.start;
            } else {
              pos = stream->segment.stop;
            }

            gst_mmtdemux_stream_update_segment (mmtdemux, stream,
...
static gboolean
mmtdemux_parse_trun (GstMMTDemux * mmtdemux, GstByteReader * trun,
    ...
    gboolean has_tfdt)
{
  ...
  GST_DEBUG_OBJECT (mmtdemux, "parsing trun stream %d; "
      "default dur %d, size %d, flags 0x%x, base offset %" G_GINT64_FORMAT ", "
      "decode_ts %" G_GINT64_FORMAT, stream->track_id, d_sample_duration,
  ...
  if (flags & TR_DATA_OFFSET) {
    ...
    if (!gst_byte_reader_get_int32_be (trun, &data_offset))
      goto fail;
    GST_DEBUG_OBJECT (mmtdemux, "trun data offset %d", data_offset);
    /* default base offset = first byte of moof */
    if (*base_offset == -1) {
      GST_DEBUG_OBJECT (mmtdemux, "base_offset at moof");
    ...
    *running_offset = *base_offset + data_offset;
    ...
    if (*base_offset == -1) {
      *base_offset = moof_offset + moof_length + 8;
      GST_DEBUG_OBJECT (mmtdemux, "base_offset assumed in mdat after moof");
      ismv = TRUE;
    }
    if (*running_offset == -1)
  ...
  GST_DEBUG_OBJECT (mmtdemux, "running offset now %" G_GINT64_FORMAT,
      *running_offset);
  GST_DEBUG_OBJECT (mmtdemux, "trun offset %d, flags 0x%x, entries %d",
  ...
  if (flags & TR_FIRST_SAMPLE_FLAGS) {
    if (G_UNLIKELY (flags & TR_SAMPLE_FLAGS)) {
      ...
      if (!gst_byte_reader_get_uint32_be (trun, &first_flags))
        goto fail;
      GST_DEBUG_OBJECT (mmtdemux, "first flags: 0x%x", first_flags);
    }
  }
  ...
  if (flags & TR_SAMPLE_DURATION) {
    GST_DEBUG_OBJECT (mmtdemux, "entry duration present");
    dur_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_SAMPLE_SIZE) {
    GST_DEBUG_OBJECT (mmtdemux, "entry size present");
    size_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_SAMPLE_FLAGS) {
    GST_DEBUG_OBJECT (mmtdemux, "entry flags present");
    flags_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_COMPOSITION_TIME_OFFSETS) {
    ct_offset = entry_size;
    GST_DEBUG_OBJECT (mmtdemux, "entry ct offset present: %u", ct_offset);
    entry_size += 4;
  }
  ...
  GST_DEBUG_OBJECT (mmtdemux, "%u allocating n_samples %u * %u (%.2f MB)",
      stream->n_samples, stream->n_samples + samples_count, (guint) sizeof (QtDemuxSample),
  ...
  if (mmtdemux->fragment_start != -1) {
    timestamp = GSTTIME_TO_QTSTREAMTIME (stream, mmtdemux->fragment_start);
	 GST_DEBUG_OBJECT (mmtdemux,"TTTIME");
    mmtdemux->fragment_start = -1;
  } else {
    if (stream->n_samples == 0) {
      if (decode_ts > 0) {
	 ...
	 GST_DEBUG_OBJECT (mmtdemux,"TTTIME last dur moof %"G_GINT64_FORMAT" stream timestamp: %"G_GUINT64_FORMAT" && stream duration: %"G_GUINT32_FORMAT" && timestamp: %"G_GINT64_FORMAT, stream->duration_last_moof, stream->samples[stream->n_samples - 1].timestamp, stream->samples[stream->n_samples - 1].duration, timestamp);
       ...
       * former */
      if (has_tfdt && !mmtdemux->upstream_format_is_time
	 ...
	 //GST_DEBUG_OBJECT (mmtdemux,"TTTIME");
      ...
      gst_ts = QTSTREAMTIME_TO_GSTTIME (stream, timestamp);
      GST_INFO_OBJECT (mmtdemux, "first sample ts %" GST_TIME_FORMAT
	...
	for (i = 0; i < samples_count; i++) {
	   ...
	   if(!mmtdemux->mediaType)
		  mmtdemux->sample_ptsoffset[i+1] = ct;
		sample->size = size;
		sample->timestamp = timestamp;
		sample->duration = dur;
		sample->sample_number = i+1;
		if(mmtdemux->mediaType == 0)
	   {
			//GST_DEBUG_OBJECT(mmtdemux, "OXXXXX sample_number %"G_GUINT32_FORMAT" sample timestamp %"G_GINT64_FORMAT " PTS offset: %"G_GUINT32_FORMAT" sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT, sample->sample_number, sample->timestamp, sample->pts_offset, sample->offset, sample->size, sample->duration);
			GST_DEBUG_OBJECT(mmtdemux, "OXXXXX sample_number %"G_GUINT32_FORMAT" sample timestamp %"G_GINT64_FORMAT " PTS offset: %"G_GUINT32_FORMAT" sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT"SEEK POSITION %"G_GUINT64_FORMAT, sample->sample_number, sample->timestamp, sample->pts_offset, sample->offset, sample->size, sample->duration, mmtdemux->seek_position);
		  sample_pts_offset[sample->sample_number] = sample->pts_offset;
	   }
		else
			GST_DEBUG_OBJECT(mmtdemux, "IXXXXX sample_number %"G_GUINT32_FORMAT" sample timestamp %"G_GINT64_FORMAT " PTS offset: %"G_GUINT32_FORMAT" sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT"SEEK POSITION %"G_GUINT64_FORMAT, sample->sample_number, sample->timestamp, sample->pts_offset, sample->offset, sample->size, sample->duration, mmtdemux->seek_position);
		//GST_DEBUG_OBJECT(mmtdemux, "IXXXXX sample timestamp %"G_GINT64_FORMAT "sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT, sample->timestamp, sample->offset, sample->size, sample->duration);
  ...
  check_update_duration (mmtdemux, QTSTREAMTIME_TO_GSTTIME (stream, timestamp));
   ...
   * input */
  mmtdemux->mdatleft = *running_offset - initial_offset;
  mmtdemux->mdatoffset = initial_offset;
  mmtdemux->mdatsize = mmtdemux->mdatleft;

  stream->n_samples += samples_count;
  stream->n_samples_moof += samples_count;
  //TODO: TIMEPOS for now keep it commented
  //stream->time_position = stream->total_timeposition;
  //stream->time_position = 0;
  //stream->time_position = QTSAMPLE_DTS(stream, sample);
  //GST_DEBUG_OBJECT(mmtdemux, "LLLLLLLLLLL media type: %d stream samples: %d, mdatoffset:%"G_GUINT64_FORMAT", mdatoffset:%"G_GUINT64_FORMAT", mdatsize:%"G_GUINT64_FORMAT, mmtdemux->mediaType, stream->n_samples, mmtdemux->mdatleft, mmtdemux->mdatoffset, mmtdemux->mdatsize);
...
fail:
  {
    GST_WARNING_OBJECT (mmtdemux, "failed to parse trun");
    return FALSE;
  }
out_of_memory:
  {
    GST_WARNING_OBJECT (mmtdemux, "failed to allocate %d samples",
        stream->n_samples);
    return FALSE;
  }
index_too_big:
  {
    GST_WARNING_OBJECT (mmtdemux, "not allocating index of %d samples, would "
...
mmtdemux_find_stream (GstMMTDemux * mmtdemux, guint32 id)
{
  QtDemuxStream *stream;
  gint i;
  ...
  /* check */
  if (G_UNLIKELY (!id)) {
    GST_DEBUG_OBJECT (mmtdemux, "invalid track id 0");
    return NULL;
  }

  /* try to get it fast and simple */
  if (G_LIKELY (id <= mmtdemux->n_streams)) {
    stream = mmtdemux->streams[id - 1];
  ...
  for (i = 0; i < mmtdemux->n_streams; i++) {
    stream = mmtdemux->streams[i];
    if (stream->track_id == id)
      return stream;
  }
  if (mmtdemux->mss_mode) {
    /* mss should have only 1 stream anyway */
    return mmtdemux->streams[0];
  }
...
static gboolean
mmtdemux_parse_mfhd (GstMMTDemux * mmtdemux, GstByteReader * mfhd,
...
fail:
  {
    GST_WARNING_OBJECT (mmtdemux, "Failed to parse mfhd atom");
    return FALSE;
  }
}

static gboolean
mmtdemux_parse_tfhd (GstMMTDemux * mmtdemux, GstByteReader * tfhd,
    ...
    gint64 * base_offset)
{
  ...
  if (!gst_byte_reader_get_uint32_be (tfhd, &track_id))
    goto invalid_track;
	 ...
	 GST_INFO_OBJECT (mmtdemux, "MMMMMMMMOOF stream with track id %i  mediaType: %d",
        track_id, mmtdemux->mediaType);
#if 1
  //if(track_id == 2 && mmtdemux->ismmth)
//	 goto invalid_track;
  if(mmtdemux->mediaType == 1)
	  track_id = 2;
#endif
  *stream = mmtdemux_find_stream (mmtdemux, track_id);
  if (G_UNLIKELY (!*stream))
    goto unknown_stream;

  if (flags & TF_DEFAULT_BASE_IS_MOOF)
    *base_offset = mmtdemux->moof_offset;
  ...
  mmtdemux_parse_trex (mmtdemux, *stream,
  ...
  if (flags & TF_DEFAULT_SAMPLE_FLAGS)
    if (!gst_byte_reader_get_uint32_be (tfhd, default_sample_flags))
      goto invalid_track;
  GST_DEBUG_OBJECT (mmtdemux, " trak id:%u, base_offset: %"G_GUINT64_FORMAT", default_sample_duration: %u, default_sample_size: %u, default_sample_flags: %u", track_id, *base_offset, *default_sample_duration, *default_sample_size, *default_sample_flags );

  return TRUE;

invalid_track:
  {
    GST_WARNING_OBJECT (mmtdemux, "invalid track fragment header");
    return FALSE;
  }
unknown_stream:
  {
    GST_DEBUG_OBJECT (mmtdemux, "unknown stream in tfhd");
    return TRUE;
  }
}

static gboolean
mmtdemux_parse_tfdt (GstMMTDemux * mmtdemux, GstByteReader * br,
    guint64 * decode_time)
{
  guint32 version = 0;
  ...
  GST_INFO_OBJECT (mmtdemux, "Track fragment decode time: %" G_GUINT64_FORMAT,
      *decode_time);
...
failed:
  {
    GST_DEBUG_OBJECT (mmtdemux, "parsing tfdt failed");
    return FALSE;
  }
}
...
static GstStructure *
mmtdemux_get_cenc_sample_properties (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, guint sample_index)
{
 ...
 * or NULL on failure */
static guint8 *
mmtdemux_parse_saiz (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GstByteReader * br, guint32 * sample_count)
{
  ...
  g_return_val_if_fail (mmtdemux != NULL, NULL);
  g_return_val_if_fail (stream != NULL, NULL);
  ...
  if (!gst_byte_reader_get_uint8 (br, &default_info_size))
    return NULL;
  GST_DEBUG_OBJECT (mmtdemux, "default_info_size: %u", default_info_size);

  if (!gst_byte_reader_get_uint32_be (br, sample_count))
    return NULL;
  GST_DEBUG_OBJECT (mmtdemux, "sample_count: %u", *sample_count);
...
static gboolean
mmtdemux_parse_saio (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    ...
    guint64 * offset)
{
  ...
  g_return_val_if_fail (mmtdemux != NULL, FALSE);
  ...
  GST_DEBUG_OBJECT (mmtdemux, "aux_info_type: '%" GST_FOURCC_FORMAT "', "
  ...
  if (entry_count != 1) {
    GST_ERROR_OBJECT (mmtdemux, "multiple offsets are not supported");
    return FALSE;
  }
  ...
  GST_DEBUG_OBJECT (mmtdemux, "offset: %" G_GUINT64_FORMAT, *offset);
  return TRUE;
}

static void
mmtdemux_gst_structure_free (GstStructure * gststructure)
{
  if (gststructure) {
...
static gboolean
mmtdemux_parse_cenc_aux_info (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GstByteReader * br, guint8 * info_sizes, guint32 sample_count)
{
  ...
  if (old_entries) {
    GST_DEBUG_OBJECT (mmtdemux, "Preserving %d old crypto info entries",
        old_entries);
  ...
  for (i = 0; i < sample_count; ++i) {
    ...
    if (!gst_structure_get_uint (properties, "iv_size", &iv_size)) {
      GST_ERROR_OBJECT (mmtdemux, "failed to get iv_size for sample %u", i);
      gst_structure_free (properties);
      return FALSE;
    }
    if (!gst_byte_reader_dup_data (br, iv_size, &data)) {
      GST_ERROR_OBJECT (mmtdemux, "failed to get IV for sample %u", i);
      gst_structure_free (properties);
      return FALSE;
    }
    buf = gst_buffer_new_wrapped (data, iv_size);
...
static gchar *
mmtdemux_uuid_bytes_to_string (gconstpointer uuid_bytes)
{
 ...
 * otherwise. */
static gboolean
mmtdemux_parse_pssh (GstMMTDemux * mmtdemux, GNode * node)
{
  gchar *sysid_string;
  ...
  if (G_UNLIKELY (pssh_size < 32U)) {
    GST_ERROR_OBJECT (mmtdemux, "invalid box size");
    return FALSE;
  }

  sysid_string =
      mmtdemux_uuid_bytes_to_string ((const guint8 *) node->data + 12);

  gst_mmtdemux_append_protection_system_id (mmtdemux, sysid_string);

  pssh = gst_buffer_new_wrapped (g_memdup (node->data, pssh_size), pssh_size);
  GST_DEBUG_OBJECT (mmtdemux, "cenc pssh size: %" G_GSIZE_FORMAT,
  ...
  for (i = 0; i < mmtdemux->n_streams; ++i) {
    g_queue_push_tail (&mmtdemux->streams[i]->protection_scheme_event_queue,
        gst_event_ref (event));
  }
  g_free (sysid_string);
  gst_event_unref (event);
    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  mmtdemux_parse_node (mmtdemux, moof_node, buffer, length);
  mmtdemux_node_dump (mmtdemux, moof_node);
  //mmtdemux->upstream_format_is_time = FALSE;

  /* Get fragment number from mfhd and check it's valid */
  mfhd_node =
      mmtdemux_tree_get_child_by_type_full (moof_node, FOURCC_mfhd, &mfhd_data);
  if (mfhd_node == NULL)
    goto missing_mfhd;
  if (!mmtdemux_parse_mfhd (mmtdemux, &mfhd_data, &frag_num))
    goto fail;
  GST_DEBUG_OBJECT (mmtdemux, "Fragment #%d", frag_num);

  /* unknown base_offset to start with */
  base_offset = running_offset = -1;
  traf_node = mmtdemux_tree_get_child_by_type (moof_node, FOURCC_traf);
  while (traf_node) {
    guint64 decode_time = 0;

    /* Fragment Header node */
    tfhd_node =
        mmtdemux_tree_get_child_by_type_full (traf_node, FOURCC_tfhd,
        &tfhd_data);
    if (!tfhd_node)
      goto missing_tfhd;
    if (!mmtdemux_parse_tfhd (mmtdemux, &tfhd_data, &stream, &ds_duration,
    ...
    saiz_node =
        mmtdemux_tree_get_child_by_type_full (traf_node, FOURCC_saiz,
        &saiz_data);
    if (saiz_node) {
      guint32 info_type = 0;
      guint64 offset = 0;
      ...
      mmtdemux->cenc_aux_info_sizes =
          mmtdemux_parse_saiz (mmtdemux, stream, &saiz_data,
          &mmtdemux->cenc_aux_sample_count);
      if (mmtdemux->cenc_aux_info_sizes == NULL) {
        GST_ERROR_OBJECT (mmtdemux, "failed to parse saiz box");
        goto fail;
      }
      saio_node =
          mmtdemux_tree_get_child_by_type_full (traf_node, FOURCC_saio,
          &saio_data);
      if (!saio_node) {
        GST_ERROR_OBJECT (mmtdemux, "saiz box without a corresponding saio box");
        g_free (mmtdemux->cenc_aux_info_sizes);
        mmtdemux->cenc_aux_info_sizes = NULL;
        goto fail;
      }

      if (G_UNLIKELY (!mmtdemux_parse_saio (mmtdemux, stream, &saio_data,
                  &info_type, &info_type_parameter, &offset))) {
        GST_ERROR_OBJECT (mmtdemux, "failed to parse saio box");
        g_free (mmtdemux->cenc_aux_info_sizes);
        mmtdemux->cenc_aux_info_sizes = NULL;
        goto fail;
      }
      if (base_offset > -1 && base_offset > mmtdemux->moof_offset)
        offset += (guint64) (base_offset - mmtdemux->moof_offset);
      if (info_type == FOURCC_cenc && info_type_parameter == 0U) {
        GstByteReader br;
        if (offset > length) {
          GST_DEBUG_OBJECT (mmtdemux, "cenc auxiliary info stored out of moof");
          mmtdemux->cenc_aux_info_offset = offset;
        } else {
          gst_byte_reader_init (&br, buffer + offset, length - offset);
          if (!mmtdemux_parse_cenc_aux_info (mmtdemux, stream, &br,
                  mmtdemux->cenc_aux_info_sizes,
                  mmtdemux->cenc_aux_sample_count)) {
            GST_ERROR_OBJECT (mmtdemux, "failed to parse cenc auxiliary info");
            g_free (mmtdemux->cenc_aux_info_sizes);
            mmtdemux->cenc_aux_info_sizes = NULL;
            goto fail;
          }
    ...
    tfdt_node =
        mmtdemux_tree_get_child_by_type_full (traf_node, FOURCC_tfdt,
        &tfdt_data);
    if (tfdt_node) {
      /* We'll use decode_time to interpolate timestamps
       * in case the input timestamps are missing */
      mmtdemux_parse_tfdt (mmtdemux, &tfdt_data, &decode_time);

      GST_DEBUG_OBJECT (mmtdemux, "decode time %" G_GINT64_FORMAT
    ...
    if (G_UNLIKELY (base_offset < -1))
      goto lost_offset;
//TODO: BY SAFWAN, DO NOT FLUSH
#if 0
    if (mmtdemux->upstream_format_is_time)
      gst_mmtdemux_stream_flush_samples_data (mmtdemux, stream);
#endif
    ...
    /* Track Run node */
    trun_node =
        mmtdemux_tree_get_child_by_type_full (traf_node, FOURCC_trun,
        &trun_data);
    while (trun_node) {
      mmtdemux_parse_trun (mmtdemux, &trun_data, stream,
          ds_duration, ds_size, ds_flags, moof_offset, length, &base_offset,
          &running_offset, decode_time, (tfdt_node != NULL));
      /* iterate all siblings */
	   break;
      trun_node = mmtdemux_tree_get_sibling_by_type_full (trun_node, FOURCC_trun,
          &trun_data);
    }

    uuid_node = mmtdemux_tree_get_child_by_type (traf_node, FOURCC_uuid);
    if (uuid_node) {
      guint8 *uuid_buffer = (guint8 *) uuid_node->data;
      guint32 box_length = QT_UINT32 (uuid_buffer);

      mmtdemux_parse_uuid (mmtdemux, uuid_buffer, box_length);
    }
  ...
  next:
    /* iterate all siblings */
	 break;
    traf_node = mmtdemux_tree_get_sibling_by_type (traf_node, FOURCC_traf);
  }

  /* parse any protection system info */
  pssh_node = mmtdemux_tree_get_child_by_type (moof_node, FOURCC_pssh);
  while (pssh_node) {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing pssh box.");
    mmtdemux_parse_pssh (mmtdemux, pssh_node);
    pssh_node = mmtdemux_tree_get_sibling_by_type (pssh_node, FOURCC_pssh);
  }
...
missing_tfhd:
  {
    GST_DEBUG_OBJECT (mmtdemux, "missing tfhd box");
    goto fail;
  }
missing_mfhd:
  {
    GST_DEBUG_OBJECT (mmtdemux, "Missing mfhd box");
    goto fail;
  }
lost_offset:
  {
    GST_DEBUG_OBJECT (mmtdemux, "lost offset");
    goto fail;
  }
fail:
  {
...
static gboolean
mmtdemux_parse_tfra (GstMMTDemux * mmtdemux, GNode * tfra_node)
{
  QtDemuxStream *stream;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "parsing tfra box for track id %u", track_id);
  ...
  GST_DEBUG_OBJECT (mmtdemux, "%u entries, sizes: value %u, traf %u, trun %u, "
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "fragment time: %" GST_TIME_FORMAT ", "
...
#if 0
    ret = gst_mmtdemux_pull_atom (mmtdemux, moof_offset, 0, &buf);
    if (ret != GST_FLOW_OK)
      goto corrupt_file;
    mmtdemux_parse_moof (mmtdemux, GST_BUFFER_DATA (buf), GST_BUFFER_SIZE (buf),
        moof_offset, stream);
    gst_buffer_unref (buf);
#endif
  }

  check_update_duration (mmtdemux, time);
...
unknown_trackid:
  {
    GST_WARNING_OBJECT (mmtdemux, "Couldn't find stream for track %u", track_id);
    return FALSE;
  }
corrupt_file:
  {
    GST_WARNING_OBJECT (mmtdemux, "broken traf box, ignoring");
    return FALSE;
  }
no_samples:
  {
    GST_WARNING_OBJECT (mmtdemux, "stream has no samples");
    return FALSE;
  }
}

static gboolean
mmtdemux_pull_mfro_mfra (GstMMTDemux * mmtdemux)
{
  ...
  if (!gst_pad_peer_query_duration (mmtdemux->sinkpad, GST_FORMAT_BYTES, &len))
    goto size_query_failed;

  /* mfro box should be at the very end of the file */
  flow = gst_mmtdemux_pull_atom (mmtdemux, len - 16, 16, &mfro);
  if (flow != GST_FLOW_OK)
    goto exit;
  ...
  GST_INFO_OBJECT (mmtdemux, "Found mfro box");
  if (mfro_map.size < 16)
  ...
  GST_INFO_OBJECT (mmtdemux, "mfra offset: %" G_GUINT64_FORMAT ", size %u",
      mfra_offset, mfra_size);

  /* now get and parse mfra box */
  flow = gst_mmtdemux_pull_atom (mmtdemux, mfra_offset, mfra_size, &mfra);
  if (flow != GST_FLOW_OK)
    goto broken_file;
  ...
  mmtdemux_parse_node (mmtdemux, mfra_node, mfra_map.data, mfra_map.size);

  tfra_node = mmtdemux_tree_get_child_by_type (mfra_node, FOURCC_tfra);

  while (tfra_node) {
    mmtdemux_parse_tfra (mmtdemux, tfra_node);
    /* iterate all siblings */
    tfra_node = mmtdemux_tree_get_sibling_by_type (tfra_node, FOURCC_tfra);
  }
  g_node_destroy (mfra_node);

  GST_INFO_OBJECT (mmtdemux, "parsed movie fragment random access box (mfra)");
  ret = TRUE;
...
size_query_failed:
  {
    GST_WARNING_OBJECT (mmtdemux, "could not query upstream size");
    goto exit;
  }
invalid_mfro_size:
  {
    GST_WARNING_OBJECT (mmtdemux, "mfro size is too small");
    goto exit;
  }
invalid_mfra_size:
  {
    GST_WARNING_OBJECT (mmtdemux, "mfra_size in mfro box is invalid");
    goto exit;
  }
broken_file:
  {
    GST_WARNING_OBJECT (mmtdemux, "bogus mfra offset or size, broken file");
    goto exit;
  }
}
...
guint64 skip_bytes(unsigned char *buffer, guint64 Rsize)
{
	...
	while(1)
	{
		if (Rsize != offset)
		{
			if((buffer[offset] == 0x40 && buffer[offset+1]==0x00 && buffer[offset+2]==0x00  && buffer[offset+3]==0x14) || (buffer[offset] == 0x40 && buffer[offset+1]==0x00 && buffer[offset+2]==0x00  && buffer[offset+3]==0x15) || (buffer[offset] == 0x40 && buffer[offset+1]==0x02 && buffer[offset+2]==0x00) || (buffer[offset] == 0x40 && buffer[offset+1]==0x00 && buffer[offset+2]==0x00  && buffer[offset+3]==0x0B)|| (buffer[offset] == 0x40 && buffer[offset+1]==0x00 && buffer[offset+2]==0x00  && buffer[offset+3]==0x0C))
			{
				GST_DEBUG_OBJECT(NULL, "ROOOOOOOOOOFFSETSIZE: %d", (int)offset);
				return offset;
			}
			offset ++;
		}
		else if(Rsize == offset) {
			GST_DEBUG_OBJECT(NULL, "OOOOOOOOOOFFSETSIZE: %d", (int)offset);
			break;
		}
	}
	 return Rsize;
...
gst_mmtdemux_loop_state_mmt_header(GstMMTDemux * mmtdemux)
{
	guint64 PayldrLength;
	GstBuffer *buf = NULL;
	GstFlowReturn ret = GST_FLOW_OK;
	guint64 cur_offset;
	GstMapInfo map;
	static int wait_count;
	int SigOffset, offset/*, packet_offset*/;
   guint header_count;
	cur_offset = mmtdemux->offset;
	GST_DEBUG_OBJECT (mmtdemux, "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW loop at position %" G_GUINT64_FORMAT " %x, state %s",
			cur_offset, (int)cur_offset, qt_demux_state_string (mmtdemux->state));
	PayldrLength = offset = 0;

	ret = gst_pad_pull_range (mmtdemux->sinkpad, cur_offset, 24, &buf);
	...
	if (G_LIKELY (map.size >= 8)){
		PayldrLength=ParseMMTPHDR(map.data, mmtdemux, &header_count);
		gst_buffer_unmap (buf, &map);
		gst_buffer_unref (buf);
		buf = NULL;
		//GST_DEBUG_OBJECT (mmtdemux, "SKIP BYTES offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
		if((PayldrLength == 3) || (PayldrLength <= 1))
		{
			GST_DEBUG_OBJECT (mmtdemux, "SKIP BYTES offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
			ret = gst_pad_pull_range (mmtdemux->sinkpad, mmtdemux->offset, 4000, &buf);
			if (G_UNLIKELY (ret != GST_FLOW_OK))
			{
				GST_DEBUG_OBJECT (mmtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
				return GST_FLOW_EOS;
			}
			gst_buffer_map (buf, &map, GST_MAP_READ);
			mmtdemux->offset +=skip_bytes(map.data, 4000);
			gst_buffer_unmap (buf, &map);
			gst_buffer_unref (buf);
			return GST_FLOW_OK;
		}
		if(PayldrLength != 0x2)
		{
			GST_DEBUG_OBJECT (mmtdemux, "AV_PACKET offset:%d, payload length: %"G_GUINT64_FORMAT "header count %u", offset, PayldrLength, header_count);
		   mmtdemux->offset += header_count;
			mmtdemux->ref_offset = mmtdemux->offset;

			ret = gst_pad_pull_range (mmtdemux->sinkpad, mmtdemux->offset, PayldrLength, &buf);
			if (G_UNLIKELY (ret != GST_FLOW_OK))
			{
				GST_DEBUG_OBJECT (mmtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
				return GST_FLOW_EOS;
			...
			gst_buffer_map (buf, &map, GST_MAP_READ);
			//if(PayldrLength !=4)
			if((mmtdemux->video_packet_id != GST_SIGNALS_NOT_DETECTED) || (mmtdemux->audio_packet_id != GST_SIGNALS_NOT_DETECTED))
			{
				GST_DEBUG_OBJECT (mmtdemux, "MLLL packet id audio :%hd video: %hd ",mmtdemux->video_packet_id, mmtdemux->audio_packet_id );
				ret = ParseMMTPV1Pack(mmtdemux, map.data, NULL);
			}
			else
				GST_DEBUG_OBJECT (mmtdemux, "EEEEEEEEEEEEEEEErrorM packet id audio :%hd video: %hd ",mmtdemux->video_packet_id, mmtdemux->audio_packet_id );

			gst_buffer_unmap (buf, &map);
			gst_buffer_unref (buf);
			if(ret == -1)
				GST_DEBUG_OBJECT (mmtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
			mmtdemux->offset +=PayldrLength+2; 
		}
		else
		{
			GST_DEBUG_OBJECT(NULL, "header_count: %u", header_count);
			ret = gst_pad_pull_range (mmtdemux->sinkpad, mmtdemux->offset, 8192, &buf);
			if (G_UNLIKELY (ret != GST_FLOW_OK))
			{
				GST_DEBUG_OBJECT (mmtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
				return GST_FLOW_EOS;
			...
			gst_buffer_map (buf, &map, GST_MAP_READ);
			SigOffset =ParseSignal(map.data, mmtdemux);
			GST_DEBUG_OBJECT (mmtdemux, "Bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  mmtdemux->offset, SigOffset);
			mmtdemux->offset +=SigOffset;

			GST_DEBUG_OBJECT (mmtdemux, " extract_initial_length_mmt length %"G_GUINT64_FORMAT, PayldrLength);
			gst_buffer_unmap (buf, &map);
			gst_buffer_unref (buf);
		}
	}
...
static GstFlowReturn
gst_mmtdemux_loop_state_header (GstMMTDemux * mmtdemux)
{
  guint64 length = 0;
  guint32 fourcc = 0;
  GstBuffer *buf = NULL;
  GstFlowReturn ret = GST_FLOW_OK;
  guint64 cur_offset = mmtdemux->offset;
  GstMapInfo map;

  ret = gst_pad_pull_range (mmtdemux->sinkpad, cur_offset, 16, &buf);
  if (G_UNLIKELY (ret != GST_FLOW_OK))
    goto beach;
  gst_buffer_map (buf, &map, GST_MAP_READ);
  GST_DEBUG_OBJECT (mmtdemux, "SAFF extract_initial_length_and_fourcc");
  if (G_LIKELY (map.size >= 8))
  ...
  if (G_UNLIKELY (length == 0)) {
    GST_ELEMENT_WARNING (mmtdemux, STREAM, DEMUX,
        (_("Invalid atom size.")),
  ...
  switch (fourcc) {
    case FOURCC_moof:
      ...
      if (!mmtdemux->moof_offset) {
        mmtdemux->moof_offset = mmtdemux->offset;
      }
      if (mmtdemux_pull_mfro_mfra (mmtdemux)) {
        /* FIXME */
      } else {
        mmtdemux->offset += length;      /* skip moof and keep going */
      }
      if (mmtdemux->got_moov) {
        GST_INFO_OBJECT (mmtdemux, "moof header, got moov, done with headers");
        ret = GST_FLOW_EOS;
        goto beach;
      }
      break;
    ...
    {
      GST_DEBUG_OBJECT (mmtdemux,
          "SSADA skipping atom '%" GST_FOURCC_FORMAT "' at %" G_GUINT64_FORMAT,
          GST_FOURCC_ARGS (fourcc), cur_offset);
      mmtdemux->offset = add_offset (mmtdemux->offset, length);
      break;
    }
    case FOURCC_moov:
    {
      ...
      if (mmtdemux->got_moov) {
        GST_DEBUG_OBJECT (mmtdemux, "Skipping moov atom as we have one already");
        mmtdemux->offset = add_offset (mmtdemux->offset, length);
        goto beach;
      }

      ret = gst_pad_pull_range (mmtdemux->sinkpad, cur_offset, length, &moov);
      if (ret != GST_FLOW_OK)
        goto beach;
      ...
      mmtdemux->offset += length;

      mmtdemux_parse_moov (mmtdemux, map.data, length);
      mmtdemux_node_dump (mmtdemux, mmtdemux->moov_node);
      ...
      gst_buffer_unmap (moov, &map);
      gst_buffer_unref (moov);
      mmtdemux->got_moov = TRUE;
      if(mmtdemux->is_mmpu == 1)
		  mmtdemux->offset += 22;
    ...
    case FOURCC_ftyp:
    {
      ...
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &ftyp);
      if (ret != GST_FLOW_OK)
        goto beach;
      mmtdemux->offset += length;
      gst_buffer_map (ftyp, &map, GST_MAP_READ);
      mmtdemux_parse_ftyp (mmtdemux, map.data, map.size);
    ...
    case FOURCC_mmpu:
    {
      ...
      mmtdemux->is_mmpu = 1;
       ...
       GST_DEBUG_OBJECT (mmtdemux, "SAFISDA: Skipping mmpu atom for now..");
      /* extract major brand; might come in handy for ISO vs QT issues */
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &mmpu);
      if (ret != GST_FLOW_OK)
        goto beach;
      mmtdemux->offset += length;
      gst_buffer_map (mmpu, &map, GST_MAP_READ);
      mmtdemux_parse_mmpu (mmtdemux, map.data, map.size);
      gst_buffer_unmap (mmpu, &map);
      gst_buffer_unref (mmpu);
      break;
    }
   case FOURCC_mmth:
    {
      ...
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &mmth);
      if (ret != GST_FLOW_OK)
        goto beach;
      mmtdemux->offset += length;
      gst_buffer_map (mmth, &map, GST_MAP_READ);
      mmtdemux_parse_mmth (mmtdemux, map.data, map.size);
      gst_buffer_unmap (mmth, &map);
      gst_buffer_unref (mmth);
      break;
    }
    case FOURCC_uuid:
    {
      ...
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &uuid);
      if (ret != GST_FLOW_OK)
        goto beach;
      mmtdemux->offset += length;
      gst_buffer_map (uuid, &map, GST_MAP_READ);
      mmtdemux_parse_uuid (mmtdemux, map.data, map.size);
    ...
    {
      GstBuffer *sidx = NULL;
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &sidx);
      if (ret != GST_FLOW_OK)
        goto beach;
      mmtdemux->offset += length;
      gst_buffer_map (sidx, &map, GST_MAP_READ);
      mmtdemux_parse_sidx (mmtdemux, map.data, map.size);
    ...
    default:
    {
      ...
      GST_DEBUG_OBJECT (mmtdemux,
          ...
          cur_offset);
      ret = gst_mmtdemux_pull_atom (mmtdemux, cur_offset, length, &unknown);
      if (ret != GST_FLOW_OK)
        goto beach;
...
beach:
  if (ret == GST_FLOW_EOS && (mmtdemux->got_moov || mmtdemux->media_caps)) {
    /* digested all data, show what we have */
    mmtdemux_prepare_streams (mmtdemux);
    ret = mmtdemux_expose_streams (mmtdemux);

    mmtdemux->state = QTDEMUX_STATE_MOVIE;
    GST_DEBUG_OBJECT (mmtdemux, "switching state to STATE_MOVIE (%d)",
        mmtdemux->state);
    return ret;
  }
  return ret;
...
gst_mmtdemux_seek_to_previous_keyframe (GstMMTDemux * mmtdemux)
{
  ...
  for (n = 0; n < mmtdemux->n_streams; n++) {
    QtDemuxStream *str = mmtdemux->streams[n];
  ...
  if (G_UNLIKELY (!ref_str)) {
    GST_DEBUG_OBJECT (mmtdemux, "couldn't find any stream");
    goto eos;
  }

  if (G_UNLIKELY (!ref_str->from_sample)) {
    GST_DEBUG_OBJECT (mmtdemux, "reached the beginning of the file");
    goto eos;
  }
  ...
  if (ref_str->subtype == FOURCC_vide) {
    k_index = gst_mmtdemux_find_keyframe (mmtdemux, ref_str,
  ...
  GST_DEBUG_OBJECT (mmtdemux, "keyframe index %u ts %" G_GUINT64_FORMAT
  ...
  GST_DEBUG_OBJECT (mmtdemux, "preferred stream played from sample %u, "
  ...
  mmtdemux->segment.position = last_stop;
  GST_DEBUG_OBJECT (mmtdemux, "last_stop now is %" GST_TIME_FORMAT,
      GST_TIME_ARGS (last_stop));

  if (G_UNLIKELY (last_stop < mmtdemux->segment.start)) {
    GST_DEBUG_OBJECT (mmtdemux, "reached the beginning of segment");
    goto eos;
  }
  ...
  for (n = 0; n < mmtdemux->n_streams; n++) {
    ...
    if (str == ref_str) {
      ...
      seg = &str->segments[seg_idx];
      k_index = ref_k_index;
      GST_DEBUG_OBJECT (mmtdemux, "reference stream %d segment %d, "
          "sample at index %d", n, ref_str->segment_index, k_index);
    } else {
      seg_idx = gst_mmtdemux_find_segment (mmtdemux, str, k_pos);
      GST_DEBUG_OBJECT (mmtdemux,
      ...
      /* get the index of the sample with media time */
      index = gst_mmtdemux_find_index_linear (mmtdemux, str, seg_time);
      GST_DEBUG_OBJECT (mmtdemux,
          "stream %d sample for %" GST_TIME_FORMAT " at %u", n,
          GST_TIME_ARGS (seg_time), index);

      /* find previous keyframe */
      k_index = gst_mmtdemux_find_keyframe (mmtdemux, str, index, FALSE);
    }
    ...
    gst_mmtdemux_move_stream (mmtdemux, str, k_index);
    GST_DEBUG_OBJECT (mmtdemux, "stream %d keyframe at %u, time position %"
 ...
 */
static void
gst_mmtdemux_stream_segment_get_boundaries (GstMMTDemux * mmtdemux,
    ...
    GstClockTime * _start, GstClockTime * _stop, GstClockTime * _time)
{
  ...
  GST_DEBUG_OBJECT (stream->pad, "seg_time %" GST_TIME_FORMAT,
  ...
  /* mmtdemux->segment.stop is in outside-time-realm, whereas
  ...
  } else if (mmtdemux->segment.rate >= 0) {
    start = MIN (segment->media_start + seg_time, stop);
    time = offset;
  } else {
    if (segment->media_start >= mmtdemux->segment.start) {
      time = segment->time;
    } else {
      time = segment->time + (mmtdemux->segment.start - segment->media_start);
    }

    start = MAX (segment->media_start, mmtdemux->segment.start);
 ...
 */
static gboolean
gst_mmtdemux_stream_update_segment (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    ...
    GstClockTime * _stop)
{
  ...
  if (G_UNLIKELY (mmtdemux->segment.duration != GST_CLOCK_TIME_NONE &&
          segment->time > mmtdemux->segment.duration)) {
    GST_WARNING_OBJECT (stream->pad, "file duration %" GST_TIME_FORMAT
        " < segment->time %" GST_TIME_FORMAT,
        GST_TIME_ARGS (mmtdemux->segment.duration),
        GST_TIME_ARGS (segment->time));
    return FALSE;
  }

  gst_mmtdemux_stream_segment_get_boundaries (mmtdemux, stream, offset,
      &start, &stop, &time);
  ...
  rate = segment->rate * mmtdemux->segment.rate;

  /* Copy flags from main segment */
  stream->segment.flags = mmtdemux->segment.flags;

  /* update the segment values used for clipping */
  stream->segment.offset = mmtdemux->segment.offset;
  stream->segment.base = mmtdemux->segment.base + stream->accumulated_base;
  stream->segment.applied_rate = mmtdemux->segment.applied_rate;
 ...
 */
static gboolean
gst_mmtdemux_activate_segment (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    guint32 seg_idx, GstClockTime offset)
{
  ...
  GST_DEBUG_OBJECT (stream->pad, "activate segment %d, offset %" GST_TIME_FORMAT,
      seg_idx, GST_TIME_ARGS (offset));

  if (!gst_mmtdemux_stream_update_segment (mmtdemux, stream, seg_idx, offset,
          &start, &stop))
    return FALSE;
  ...
  if (mmtdemux->fragmented && !mmtdemux->fragmented_seek_pending) {
    stream->to_sample = G_MAXUINT32;
    return TRUE;
  } else {
    /* well, it will be taken care of below */
    mmtdemux->fragmented_seek_pending = FALSE;
  ...
  if (G_LIKELY (!QTSEGMENT_IS_EMPTY (segment))) {
    if (mmtdemux->segment.rate >= 0) {
      index = gst_mmtdemux_find_index_linear (mmtdemux, stream, start);
      stream->to_sample = G_MAXUINT32;
      GST_DEBUG_OBJECT (stream->pad,
      ...
      index = gst_mmtdemux_find_index_linear (mmtdemux, stream, stop);
  ...
  /* gst_mmtdemux_parse_sample () called from gst_mmtdemux_find_index_linear ()
  ...
  kf_index = gst_mmtdemux_find_keyframe (mmtdemux, stream, index, FALSE);
...
/* indent does stupid stuff with stream->samples[].timestamp */
  ...
  if (index > stream->sample_index) {
    ...
    if (kf_index > stream->sample_index) {
      ...
      gst_mmtdemux_move_stream (mmtdemux, stream, kf_index);
    } else {
      GST_DEBUG_OBJECT (stream->pad,
 ...
 */
static gboolean
gst_mmtdemux_prepare_current_sample (GstMMTDemux * mmtdemux,
    ...
    gboolean * keyframe)
{
  ...
  *empty = FALSE;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "segment active, index = %u of %u",
  ...
  if (G_UNLIKELY (stream->sample_index >= stream->n_samples)) {
    ...
    GST_INFO_OBJECT (mmtdemux, "out of samples, trying to add more");
    do {
      GstFlowReturn flow;

      GST_OBJECT_LOCK (mmtdemux);
      flow = mmtdemux_add_fragmented_samples (mmtdemux);
      GST_OBJECT_UNLOCK (mmtdemux);
  ...
  if (!mmtdemux_parse_samples (mmtdemux, stream, stream->sample_index)) {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing of index %u failed!",
        stream->sample_index);
    return FALSE;
  }

  /* now get the info for the sample we're at */
  GST_DEBUG_OBJECT(mmtdemux, "SAMPLE_INDEX: %"G_GUINT32_FORMAT, stream->sample_index);
  sample = &stream->samples[stream->sample_index];

  *dts = QTSAMPLE_DTS (stream, sample);
  *pts = QTSAMPLE_PTS (stream, sample);
  //*dts = sample->timestamp;
  //*pts = sample->timestamp + stream->cslg_shift +sample->pts_offset;
  *offset = sample->sample_number;
  *size = sample->size;
  *duration = QTSAMPLE_DUR_DTS (stream, sample, *dts);
  *keyframe = QTSAMPLE_KEYFRAME (stream, sample);
   GST_DEBUG_OBJECT(mmtdemux, "VVVVVVVVV time_stamp %"G_GUINT64_FORMAT " pts offset %"G_GUINT32_FORMAT " PTS %"G_GUINT64_FORMAT " DTS %"G_GUINT64_FORMAT " cslg_shift: %"G_GUINT32_FORMAT, sample->timestamp, sample->pts_offset, QTSAMPLE_PTS (stream, sample), QTSAMPLE_DTS (stream, sample), (stream)->cslg_shift);
 ...
 */
static void
gst_mmtdemux_advance_sample (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  QtDemuxSample *sample;
  QtDemuxSegment *segment;
  ...
  if (G_UNLIKELY (QTSEGMENT_IS_EMPTY (segment))) {
    GST_DEBUG_OBJECT (mmtdemux, "Empty segment, no samples to advance");
    goto next_segment;
  }
  ...
  if (QTSAMPLE_DTS (stream, sample) >= segment->media_start) {
    ...
    stream->time_position =
	 ...
	 //GST_DEBUG_OBJECT (mmtdemux, "%s_time_position:%"G_GUINT64_FORMAT" sample_timestamp:%" G_GUINT64_FORMAT" segment time: %" G_GUINT64_FORMAT, GST_PAD_NAME (stream->pad), stream->time_position, sample->timestamp, segment->time);
  } else {
...
next_segment:
  {
    GST_DEBUG_OBJECT (mmtdemux, "segment %d Eended %d", stream->segment_index, stream->n_segments);

    if (stream->segment_index == stream->n_segments - 1) {
      /* are we at the end of the last segment, we're EOS */
	 GST_DEBUG_OBJECT(mmtdemux,"RRRR set time postition to GST_CLOCK_TIME_NONE");
	   stream->total_timeposition = stream->time_position;
      //stream->time_position = GST_CLOCK_TIME_NONE;
    } else {
	 GST_DEBUG_OBJECT(mmtdemux,"RRRR set time postition to segment->stop.time");
...
static void
gst_mmtdemux_sync_streams (GstMMTDemux * demux)
{
  gint i;
  ...
  for (i = 0; i < demux->n_streams; i++) {
    ...
    GST_DEBUG_OBJECT (demux, "current position: %" GST_TIME_FORMAT
 ...
 */
static GstFlowReturn
gst_mmtdemux_combine_flows (GstMMTDemux * demux, QtDemuxStream * stream,
    GstFlowReturn ret)
{
  GST_DEBUG_OBJECT (demux, "flow return: %s", gst_flow_get_name (ret));
  ...
  GST_DEBUG_OBJECT (demux, "combined flow return: %s", gst_flow_get_name (ret));
  return ret;
}
...
static GstBuffer *
gst_mmtdemux_clip_buffer (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GstBuffer * buf)
{
...
wrong_type:
  {
    GST_DEBUG_OBJECT (mmtdemux, "unknown stream type");
    return buf;
  }
bad_frame_size:
  {
    GST_DEBUG_OBJECT (mmtdemux, "bad frame size");
    return buf;
  }
no_pts:
  {
    GST_DEBUG_OBJECT (mmtdemux, "no pts on buffer");
    return buf;
  }
clipped:
  {
    GST_DEBUG_OBJECT (mmtdemux, "clipped buffer");
    gst_buffer_unref (buf);
    return NULL;
  }
}
...
static GstBuffer *
gst_mmtdemux_align_buffer (GstMMTDemux * demux,
...
static GstBuffer *
gst_mmtdemux_process_buffer (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GstBuffer * buf)
{
  GstMapInfo map;
  guint nsize = 0;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "3GPP timed text subtitle: %d/%" G_GSIZE_FORMAT "",
      nsize, map.size);
 ...
 */
static GstFlowReturn
gst_mmtdemux_decorate_and_push_buffer (GstMMTDemux * mmtdemux,
    ...
    gboolean keyframe, GstClockTime position, guint64 byte_position)
{
  ...
  if (G_UNLIKELY (CUR_STREAM (stream)->fourcc == FOURCC_rtsp)) {
    ...
    if (url != NULL && strlen (url) != 0) {
      ...
      gst_element_post_message (GST_ELEMENT_CAST (mmtdemux),
          gst_message_new_element (GST_OBJECT_CAST (mmtdemux),
              gst_structure_new ("redirect",
                  "new-location", G_TYPE_STRING, url, NULL)));
      mmtdemux->posted_redirect = TRUE;
    } else {
      GST_WARNING_OBJECT (mmtdemux, "Redirect URI of stream is empty, not "
          "posting");
    }
    g_free (url);
  }
//if(mmtdemux->mainbuf == NULL)
if(1)
{
  /* position reporting */
  if (mmtdemux->segment.rate >= 0) {
    GST_DEBUG_OBJECT (mmtdemux, "SYNC STREAM");
    mmtdemux->segment.position = position;
    gst_mmtdemux_sync_streams (mmtdemux);
  }

  if (G_UNLIKELY (!stream->pad)) {
    //GST_DEBUG_LOG (mmtdemux, "No output pad for stream, ignoring");
    gst_buffer_unref (buf);
    goto exit;
  }
  ...
  while (stream->buffers) {
    ...
    if (G_UNLIKELY (stream->discont)) {
      GST_DEBUG_OBJECT (mmtdemux, "marking discont buffer");
    ...
    if (stream->alignment > 1)
      buffer = gst_mmtdemux_align_buffer (mmtdemux, buffer, stream->alignment);
  ...
  if (G_UNLIKELY (stream->need_process))
      GST_DEBUG_OBJECT (mmtdemux, "process buffer");
    buf = gst_mmtdemux_process_buffer (mmtdemux, stream, buf);
...
#if 0
  if (G_UNLIKELY (mmtdemux->element_index)) {
    ...
    if (GST_CLOCK_TIME_IS_VALID (stream_time)) {
      ...
      gst_index_add_association (mmtdemux->element_index,
          mmtdemux->index_id,
  ...
  if (stream->need_clip)
    buf = gst_mmtdemux_clip_buffer (mmtdemux, stream, buf);
  ...
  if (!keyframe) {
  GST_DEBUG_OBJECT (mmtdemux,"PPPPPPPPPPad:%s_FALSE", GST_PAD_NAME (stream->pad));
    GST_BUFFER_FLAG_SET (buf, GST_BUFFER_FLAG_DELTA_UNIT);
    stream->on_keyframe = FALSE;
  } else {
  GST_DEBUG_OBJECT (mmtdemux,"PPPPPPPPPPad:%s_TRUE", GST_PAD_NAME (stream->pad));
    stream->on_keyframe = TRUE;
  }
  GST_DEBUG_OBJECT (mmtdemux,"PPPPPPPPPPad:%s", GST_PAD_NAME (stream->pad));


  GST_DEBUG_OBJECT (mmtdemux,
      "%s_Pushing buffer with dts %" GST_TIME_FORMAT ", pts %" GST_TIME_FORMAT
      ", duration %" GST_TIME_FORMAT " on pad %s", GST_PAD_NAME (stream->pad), GST_TIME_ARGS (dts),
  ...
  if (stream->protected && stream->protection_scheme_type == FOURCC_cenc) {
    ...
    if (info->crypto_info == NULL) {
      GST_DEBUG_OBJECT (mmtdemux, "cenc metadata hasn't been parsed yet");
      gst_buffer_unref (buf);
      goto exit;
    }
    ...
    if (G_LIKELY (index >= 0 && index < info->crypto_info->len)) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "attaching cenc metadata [%u/%u]", index,
          info->crypto_info->len);
      if (!crypto_info || !gst_buffer_add_protection_meta (buf, crypto_info))
        GST_ERROR_OBJECT (mmtdemux, "failed to attach cenc metadata to buffer");
    } else {
      GST_INFO_OBJECT (mmtdemux, "No crypto info with index %d and sample %d",
          index, stream->sample_index);
    }
  }

  if (stream->alignment > 1)
    buf = gst_mmtdemux_align_buffer (mmtdemux, buf, stream->alignment);
...
gst_mmtdemux_stream_seek_fragment (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
...
static gboolean
gst_mmtdemux_do_fragmented_seek (GstMMTDemux * mmtdemux)
{
  ...
  for (i = 0; i < mmtdemux->n_streams; i++) {
    ...
    entry =
        gst_mmtdemux_stream_seek_fragment (mmtdemux, stream,
  ...
  GST_INFO_OBJECT (mmtdemux, "seek to %" GST_TIME_FORMAT ", best fragment "
      "moof offset: %" G_GUINT64_FORMAT ", ts %" GST_TIME_FORMAT,
      GST_TIME_ARGS (mmtdemux->streams[0]->time_position),
      best_entry->moof_offset, GST_TIME_ARGS (best_entry->ts));

  mmtdemux->moof_offset = best_entry->moof_offset;

  mmtdemux_add_fragmented_samples (mmtdemux);
...
GstFlowReturn
gst_mmtdemux_state_movie (GstMMTDemux * mmtdemux, GstBuffer *mdatbuf, guint64 mfuoffset, int mediaType, gint32 sample_number)
{
	GstFlowReturn ret = GST_FLOW_OK;
	QtDemuxStream *stream;
	...
	guint size;
	gint index;
	guint32 extracted_sample_number = 0;
	//QtDemuxSample *sample;
	GstClockTime position;
	gst_mmtdemux_push_pending_newsegment (mmtdemux);

	if (mmtdemux->fragmented_seek_pending) {
		GST_INFO_OBJECT (mmtdemux, "pending fragmented seek");
		if (gst_mmtdemux_do_fragmented_seek (mmtdemux)) {
			GST_INFO_OBJECT (mmtdemux, "fragmented seek done!");
			mmtdemux->fragmented_seek_pending = FALSE;
		} else {
			GST_INFO_OBJECT (mmtdemux, "fragmented seek still pending");
		}
	}
 //  if(!mediaType)
	//	  fprintf(stderr, "HHHHHHHH===================f pts: %"G_GUINT64_FORMAT" \n", mdatbuf->pts);

	/* Figure out the next stream sample to output, min_time is expressed in
	 * global time and runs over the edit list segments. */
	min_time =G_MAXUINT64;
	index = -1;
	if((mdatbuf == NULL) || (mmtdemux->n_streams != 2))
		return ret;
	stream = mmtdemux->streams[mediaType];
	position = stream->time_position;
	GST_DEBUG_OBJECT (mmtdemux,"%dAPPPPPPPPPPad: sample_number: %"G_GUINT32_FORMAT" sample_index: %u  position: %"G_GUINT64_FORMAT" prev_position: %"G_GUINT64_FORMAT" Total samples: %d", mediaType, sample_number, stream->sample_index, position, stream->time_position, stream->n_samples);
	//GST_DEBUG_OBJECT (mmtdemux,"%dAPPPPPPPPPPad:%s, sample_number: %"G_GUINT32_FORMAT" sample_index: %u  position: %"G_GUINT64_FORMAT" prev_position: %"G_GUINT64_FORMAT" Total samples: %d", mediaType, GST_PAD_NAME (stream->pad), sample_number, stream->sample_index, position, stream->time_position, stream->n_samples);

	/* position of -1 is EOS */
	if (position != GST_CLOCK_TIME_NONE && position < min_time) {
		GST_DEBUG_OBJECT (mmtdemux, "ABCD position is less then min time");
		min_time = position;
		index = mediaType;
	}
	/* all are EOS */
	if (G_UNLIKELY (index == -1)) {
		GST_DEBUG_OBJECT (mmtdemux, "all streams are EOS");
		goto eos;
	}

	/* check for segment end */
	if (G_UNLIKELY (mmtdemux->segment.stop != -1
				&& ((mmtdemux->segment.rate >= 0 && mmtdemux->segment.stop <= min_time)
					|| (mmtdemux->segment.rate < 0
						&& mmtdemux->segment.start > min_time))
				&& mmtdemux->streams[index]->on_keyframe)) {
		GST_DEBUG_OBJECT (mmtdemux, "we reached the end of our segment.");
		mmtdemux->streams[index]->time_position = GST_CLOCK_TIME_NONE;
		goto eos_stream;
	}
	...
	stream = mmtdemux->streams[index];
	/* fetch info for the current sample of this stream */
	if (G_UNLIKELY (!gst_mmtdemux_prepare_current_sample (mmtdemux, stream, &empty,
					&offset, &sample_size, &dts, &pts, &duration, &keyframe)))
	{
		GST_DEBUG_OBJECT (mmtdemux, "HKMS..... OUT OF SAMPLES");
		fprintf (stderr, "HKMS..... OUT OF SAMPLES");
	 #if 0
	   stream->n_samples += 1;
		stream->samples[stream->n_samples].timestamp +=
			stream->samples[stream->n_samples - 2].duration;
		if(mmtdemux->mainbuf)
		{
			//mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples - 1].timestamp+ADVANCE_TIME;
			if(!mediaType)
				mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples].timestamp+ADVANCE_TIME+ QT_GSTTIME(stream, mmtdemux->sample_ptsoffset[sample_number]);
		  else
			mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples].timestamp+ADVANCE_TIME;
		}
		else
			mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples].timestamp;
	 #endif
		ret = gst_pad_push (stream->pad, mdatbuf);
		return GST_FLOW_OK;
	}
	extracted_sample_number = (guint32 )offset;
	if((mmtdemux->mainbuf == NULL) && (sample_number > extracted_sample_number))
	{
		stream->sample_index = stream->sample_index+(sample_number-extracted_sample_number);
		GST_DEBUG_OBJECT (mmtdemux, "different sample number..... %"G_GUINT32_FORMAT " :: %"G_GUINT32_FORMAT, sample_number, extracted_sample_number);
//		gst_mmtdemux_state_movie (mmtdemux, mdatbuf, mfuoffset, mediaType, sample_number);
//		return GST_FLOW_OK;
	}
//	if(stream->pad != NULL)
//		GST_DEBUG_OBJECT (mmtdemux, "%s_BISMIL sample_size: %d, offset: %"G_GUINT64_FORMAT, GST_PAD_NAME (stream->pad), sample_size, offset);
	gst_mmtdemux_stream_check_and_change_stsd_index (mmtdemux, stream);
#if 0
	if (stream->new_caps) {
		gst_mmtdemux_configure_stream (mmtdemux, stream);
		mmtdemux_do_allocation (mmtdemux, stream);
	}
#endif
	/* If we're doing a keyframe-only trickmode, only push keyframes on video streams */
	if (G_UNLIKELY (mmtdemux->
				segment.flags & GST_SEGMENT_FLAG_TRICKMODE_KEY_UNITS)) {
		if (stream->subtype == FOURCC_vide && !keyframe) {
			GST_DEBUG_OBJECT (mmtdemux, "Skipping non-keyframe on stream %d", index);
			goto next;
		}
	}
...
//	fprintf (stderr,
			...
			", size %d, dts=%" GST_TIME_FORMAT "=== %"G_GUINT64_FORMAT", :: %"GST_TIME_FORMAT" pts=%" GST_TIME_FORMAT
			", :: %"GST_TIME_FORMAT" duration %" GST_TIME_FORMAT" :: %"GST_TIME_FORMAT"\n", index, empty, offset, sample_size,
			GST_TIME_ARGS (dts), dts, GST_TIME_ARGS(mdatbuf->dts), GST_TIME_ARGS (pts), GST_TIME_ARGS(mdatbuf->pts), GST_TIME_ARGS (duration), GST_TIME_ARGS(mdatbuf->duration));
	//offset = mfuoffset;
	if (G_UNLIKELY (empty)) {
...
//		goto next;
	...
	if (mmtdemux->cenc_aux_info_offset > 0) {
		GstMapInfo map;
		GstByteReader br;
		...
		ret =
			gst_mmtdemux_pull_atom (mmtdemux, mmtdemux->offset,
					offset + stream->offset_in_sample - mmtdemux->offset, &aux_info);
		if (G_UNLIKELY (ret != GST_FLOW_OK))
			goto beach;
		gst_buffer_map (aux_info, &map, GST_MAP_READ);
		GST_DEBUG_OBJECT (mmtdemux, "parsing cenc auxiliary info");
		gst_byte_reader_init (&br, map.data + 8, map.size);
		if (!mmtdemux_parse_cenc_aux_info (mmtdemux, stream, &br,
					mmtdemux->cenc_aux_info_sizes, mmtdemux->cenc_aux_sample_count)) {
			GST_ERROR_OBJECT (mmtdemux, "failed to parse cenc auxiliary info");
	...
	GST_DEBUG_OBJECT (mmtdemux, "reading %d bytes @ %" G_GUINT64_FORMAT, size,
			offset);
   ...
   if(!mediaType)
		  fprintf(stderr, "SSSSSS===================f pts: %"G_GUINT64_FORMAT" === pts %"G_GUINT64_FORMAT" dts %"G_GUINT64_FORMAT" sample_number %d pts offset %"G_GUINT64_FORMAT"\n", pts, mdatbuf->pts, dts, sample_number, QT_GSTTIME(stream, mmtdemux->sample_ptsoffset[sample_number]));
	if(mmtdemux->mainbuf)
	{
#if 0
	   if(!set_pts[mediaType])
	 {
		  //pts += mdatbuf->pts;
		  //dts += mdatbuf->dts;
		  set_pts[mediaType] = 1;
	...
	//	dts += ADVANCE_TIME;
	 //  if(!mediaType)
		//  pts = mdatbuf->pts + QT_GSTTIME(stream, mmtdemux->sample_ptsoffset[sample_number]);// + ADVANCE_TIME;
	   //else
		pts = mdatbuf->pts;// + ADVANCE_TIME;
		dts = mdatbuf->dts;// + ADVANCE_TIME;
	}
	...
	{
		ret = gst_pad_push (stream->pad, mdatbuf);
		//ret = gst_mmtdemux_decorate_and_push_buffer (mmtdemux, stream, mdatbuf,
		//   dts, pts, duration, keyframe, min_time, 0);
		return GST_FLOW_OK;
	}
	if(mmtdemux->seek_position)
	{
		if (mmtdemux->seek_position > stream->time_position)
		{
			fprintf(stderr, "seeeking...... %"G_GUINT64_FORMAT" %"G_GUINT64_FORMAT"\n", mmtdemux->seek_position, stream->time_position);
		}
	   else
		{
			fprintf(stderr, "KKKK seeeking...... %"G_GUINT64_FORMAT" %"G_GUINT64_FORMAT"\n", mmtdemux->seek_position, stream->time_position);
			mmtdemux->seek_position = 0;
		}
	}
	ret = gst_mmtdemux_decorate_and_push_buffer (mmtdemux, stream, mdatbuf,
			dts, pts, duration, keyframe, min_time, 0);
	//dts, pts, duration, keyframe, 0, 0);
	if (size != sample_size) {
		...
		GstClockTime time_position = QTSTREAMTIME_TO_GSTTIME (stream,
				sample->timestamp +
				stream->offset_in_sample / CUR_STREAM (stream)->bytes_per_frame);
		GST_DEBUG_OBJECT(mmtdemux, "size is not equal to sample size");
		fprintf(stderr, "size is not equal to sample size");
	...
	/* combine flows */
	ret = gst_mmtdemux_combine_flows (mmtdemux, stream, ret);
	...
	if (stream->offset_in_sample >= sample_size) {
		GST_DEBUG_OBJECT (mmtdemux, "SYXYXYX offset_in_sample is greater than sample size");
		gst_mmtdemux_advance_sample (mmtdemux, stream);
	}
	goto beach;
...
eos:
	{
		GST_DEBUG_OBJECT (mmtdemux, "No samples left for any streams - EOS");
		ret = GST_FLOW_EOS;
		goto beach;
	}
eos_stream:
	{
		GST_DEBUG_OBJECT (mmtdemux, "No samples left for stream");
...
gst_mmtdemux_loop_state_movie (GstMMTDemux * mmtdemux)
{
  ...
  if (mmtdemux->fragmented_seek_pending) {
    GST_DEBUG_OBJECT (mmtdemux, "pending fragmented seek");
    if (gst_mmtdemux_do_fragmented_seek (mmtdemux)) {
      GST_DEBUG_OBJECT (mmtdemux, "fragmented seek done!");
      mmtdemux->fragmented_seek_pending = FALSE;
    } else {
      GST_DEBUG_OBJECT (mmtdemux, "fragmented seek still pending");
    }
  }
  ...
  for (i = 0; i < mmtdemux->n_streams; i++) {
    ...
    stream = mmtdemux->streams[i];
    position = stream->time_position;
	GST_DEBUG_OBJECT (mmtdemux,"%dAPPPPPPPPPPad:%s position: %"G_GUINT64_FORMAT, i, GST_PAD_NAME (stream->pad), position);
  ...
  ret = gst_mmtdemux_pull_atom (mmtdemux, offset + stream->offset_in_sample,
      size, &buf);
  GST_DEBUG_OBJECT (mmtdemux, "index: %d ABBBB offset_in_sample @ %" G_GUINT64_FORMAT" :: offset:%"G_GUINT64_FORMAT, index, buf->offset, buf->offset_end);
#if 0
	 if(mmtdemux->mmdatbuffer[index])
		  mmtdemux->mmdatbuffer[index] = gst_buffer_append (mmtdemux->mmdatbuffer[index], buf);
	 else
		  mmtdemux->mmdatbuffer[index] = buf;
#endif
  GST_DEBUG_OBJECT (mmtdemux, "offset_in_sample @ %" G_GUINT32_FORMAT" :: offset:%"G_GUINT64_FORMAT, stream->offset_in_sample, offset);
  if (G_UNLIKELY (ret != GST_FLOW_OK))
    goto beach;
  GST_DEBUG_OBJECT (mmtdemux, "UUUU size %d && sample_size @ %" G_GUINT32_FORMAT, size,
      sample_size);
  ...
  ret = gst_mmtdemux_decorate_and_push_buffer (mmtdemux, stream, buf,
  ...
  if (size != sample_size) {
    ...
    GstClockTime time_position = QTSTREAMTIME_TO_GSTTIME (stream,
        sample->timestamp +
        stream->offset_in_sample / CUR_STREAM (stream)->bytes_per_frame);
	 GST_DEBUG_OBJECT(mmtdemux, "JJJJJJJ SIZE IS NOT EQUAL TO SAMPLE SIZE");
    if (time_position >= segment->media_start) {
      /* inside the segment, update time_position, looks very familiar to
       * GStreamer segments, doesn't it? */
      stream->time_position = (time_position - segment->media_start) +
...
static void
gst_mmtdemux_loop (GstPad * pad)
{
  GstMMTDemux *mmtdemux;
  guint64 cur_offset;
  GstFlowReturn ret;

  mmtdemux = GST_QTDEMUX (gst_pad_get_parent (pad));
  ...
  GST_DEBUG_OBJECT (mmtdemux, "loop at position %" G_GUINT64_FORMAT " %x, state %s",
      cur_offset, (int)cur_offset, qt_demux_state_string (mmtdemux->state));

  switch (mmtdemux->state) {
    case QTDEMUX_STATE_INITIAL:
    case QTDEMUX_STATE_HEADER:
    case QTDEMUX_STATE_META:
	 //TODO MMTPARSING;
      ret = gst_mmtdemux_loop_state_mmt_header (mmtdemux);
      //ret = gst_mmtdemux_loop_state_header (mmtdemux);
      break;
#if 1
    case QTDEMUX_STATE_BUFFER_MM:
      ret = gst_mmtdemux_loop_state_mmt_header (mmtdemux);
      ret = gst_mmtdemux_loop_state_header (mmtdemux);
      break;
#endif
    case QTDEMUX_STATE_MOVIE:
      ret = gst_mmtdemux_loop_state_movie (mmtdemux);
      if (mmtdemux->segment.rate < 0 && ret == GST_FLOW_EOS) {
        ret = gst_mmtdemux_seek_to_previous_keyframe (mmtdemux);
      }
      break;
    default:
      /* ouch */
  ...
  gst_object_unref (mmtdemux);
  return;

  /* ERRORS */
invalid_state:
  {
    GST_ELEMENT_ERROR (mmtdemux, STREAM, FAILED,
        (NULL), ("streaming stopped, invalid state"));
    gst_pad_pause_task (pad);
    gst_mmtdemux_push_event (mmtdemux, gst_event_new_eos ());
    goto done;
  }
pause:
  {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "pausing task, reason %s", reason);
    ...
    if (ret == GST_FLOW_EOS) {
      ...
      if (mmtdemux->segment.flags & GST_SEEK_FLAG_SEGMENT) {
        ...
        if (mmtdemux->segment.rate >= 0) {
          ...
          GST_DEBUG_OBJECT (mmtdemux, "Sending segment done, at end of segment");
          message = gst_message_new_segment_done (GST_OBJECT_CAST (mmtdemux),
              GST_FORMAT_TIME, stop);
          event = gst_event_new_segment_done (GST_FORMAT_TIME, stop);
          if (mmtdemux->segment_seqnum != GST_SEQNUM_INVALID) {
            gst_message_set_seqnum (message, mmtdemux->segment_seqnum);
            gst_event_set_seqnum (event, mmtdemux->segment_seqnum);
          }
          gst_element_post_message (GST_ELEMENT_CAST (mmtdemux), message);
          gst_mmtdemux_push_event (mmtdemux, event);
        } else {
          GstMessage *message;
          GstEvent *event;

          /*  For Reverse Playback */
          GST_DEBUG_OBJECT (mmtdemux, "Sending segment done, at start of segment");
          message = gst_message_new_segment_done (GST_OBJECT_CAST (mmtdemux),
              GST_FORMAT_TIME, mmtdemux->segment.start);
        ...
        GST_DEBUG_OBJECT (mmtdemux, "Sending EOS at end of segment");
        event = gst_event_new_eos ();
        if (mmtdemux->segment_seqnum != GST_SEQNUM_INVALID)
          gst_event_set_seqnum (event, mmtdemux->segment_seqnum);
        gst_mmtdemux_push_event (mmtdemux, event);
      ...
      GST_ELEMENT_FLOW_ERROR (mmtdemux, ret);
      gst_mmtdemux_push_event (mmtdemux, gst_event_new_eos ());
    }
    goto done;
  }
 ...
 */
static gboolean
has_next_entry (GstMMTDemux * demux)
{
  QtDemuxStream *stream;
  int i;
  ...
  for (i = 0; i < demux->n_streams; i++) {
    ...
    if (stream->sample_index >= stream->n_samples) {
      GST_DEBUG_OBJECT (demux, "stream %d samples exhausted", i);
      continue;
    }
 ...
 */
static guint64
next_entry_size (GstMMTDemux * demux)
{
  QtDemuxStream *stream;
  int i;
  int smallidx = -1;
  ...
  GST_DEBUG_OBJECT (demux, "Finding entry at offset %" G_GUINT64_FORMAT,
      demux->offset);
  ...
  for (i = 0; i < demux->n_streams; i++) {
    ...
    if (!mmtdemux_parse_samples (demux, stream, stream->sample_index)) {
      GST_DEBUG_OBJECT (demux, "Parsing of index %u from stbl atom failed!",
          stream->sample_index);
      return -1;
    }
...
static void
gst_mmtdemux_post_progress (GstMMTDemux * demux, gint num, gint denom)
{
...
static gboolean
mmtdemux_seek_offset (GstMMTDemux * demux, guint64 offset)
{
  GstEvent *event;
  gboolean res = 0;
...
static void
gst_mmtdemux_check_seekability (GstMMTDemux * demux)
{
  GstQuery *query;
...
static void
gst_mmtdemux_drop_data (GstMMTDemux * demux, gint bytes)
{
  g_return_if_fail (bytes <= demux->todrop);

  GST_DEBUG_OBJECT (demux, "Dropping %d bytes", bytes);
...
static void
gst_mmtdemux_check_send_pending_segment (GstMMTDemux * demux)
{
  if (G_UNLIKELY (demux->pending_newsegment)) {
    gint i;

    gst_mmtdemux_push_pending_newsegment (demux);
    /* clear to send tags on all streams */
    for (i = 0; i < demux->n_streams; i++) {
      QtDemuxStream *stream;
      stream = demux->streams[i];
      gst_mmtdemux_push_tags (demux, stream);
...
static void
gst_mmtdemux_send_gap_for_segment (GstMMTDemux * demux,
    QtDemuxStream * stream, gint segment_index, GstClockTime pos)
{
  GstClockTime ts, dur;
  GstEvent *gap;
...
static void
gst_mmtdemux_stream_send_initial_gap_segments (GstMMTDemux * demux,
    QtDemuxStream * stream)
{
  gint i;
   ...
   * 'real' data */
  for (i = 0; i < stream->n_segments; i++) {
    gst_mmtdemux_activate_segment (demux, stream, i, stream->time_position);

    if (QTSEGMENT_IS_EMPTY (&stream->segments[i])) {
      gst_mmtdemux_send_gap_for_segment (demux, stream, i,
          stream->time_position);
    } else {
...
static GstFlowReturn adapter_send_pkt(GstMMTDemux *mmtdemux, GstBuffer *buf, guint64 size) 
{
	guint64 PayldrLength;
	GstFlowReturn ret = GST_FLOW_OK;
	guint8 *data;
	...
	data = (guint8 *)gst_adapter_map (mmtdemux->adapter, 16);
	PayldrLength=ParseMMTPHDR(data, mmtdemux, &header_count);
	gst_adapter_unmap (mmtdemux->adapter);

	GST_DEBUG_OBJECT (mmtdemux, "INT_SIZEEEEEEEEE offset:%d, payload length: %"G_GUINT64_FORMAT "packet length: %"G_GUINT64_FORMAT " header count: %u", offset, PayldrLength, size, header_count);
	if(PayldrLength != 0x2)
	{
		data = (unsigned char *)gst_adapter_map (mmtdemux->adapter, PayldrLength);
		mmtdemux->packet_offset = mmtdemux->offset+ header_count;
		GST_DEBUG_OBJECT (mmtdemux, "AV_PACKET offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
		mmtdemux->ref_offset = mmtdemux->packet_offset;
		gst_adapter_unmap (mmtdemux->adapter);
		if((PayldrLength != 4) && (PayldrLength != 3))
			ParseMMTPV1Pack(mmtdemux, data+header_count, buf);
	}
	else
	{
		data = (guint8 *)gst_adapter_map (mmtdemux->adapter, size);
		SigOffset =ParseSignal(data, mmtdemux);
		GST_DEBUG_OBJECT (mmtdemux, "Bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  mmtdemux->offset, SigOffset);
		mmtdemux->offset +=SigOffset;

		GST_DEBUG_OBJECT (mmtdemux, " extract_initial_length_mmt length %"G_GUINT64_FORMAT, PayldrLength);
		gst_adapter_unmap (mmtdemux->adapter);
		gst_adapter_flush (mmtdemux->adapter, size);
	}

	return ret;
}

static GstFlowReturn send_pkt(GstMMTDemux *mmtdemux, GstBuffer *buf, guint64 size) 
{
	guint64 PayldrLength;
	GstFlowReturn ret = GST_FLOW_OK;
	GstMapInfo map;
	int SigOffset, offset/*, packet_offset*/;
	guint header_count;
   unsigned char bitstream[4096] = {'\0'};
	PayldrLength = offset = 0;

	gst_buffer_map (buf, &map, GST_MAP_READ);
	if (G_LIKELY (map.size >= 8)){
	   memcpy(bitstream, map.data, size);
		gst_buffer_unmap (buf, &map);
		//PayldrLength=ParseMMTPHDR(map.data, mmtdemux, &header_count);
		PayldrLength=ParseMMTPHDR(bitstream, mmtdemux, &header_count);

		GST_DEBUG_OBJECT (mmtdemux, "INT_SIZEEEEEEEEE offset:%d, payload length: %"G_GUINT64_FORMAT "packet length: %"G_GUINT64_FORMAT, offset, PayldrLength, size);
		if(PayldrLength != 0x2)
		{
			mmtdemux->packet_offset = mmtdemux->offset + header_count;
			GST_DEBUG_OBJECT (mmtdemux, "AV_PACKET offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
			mmtdemux->ref_offset = mmtdemux->packet_offset;
			if((mmtdemux->video_packet_id != GST_SIGNALS_NOT_DETECTED) || (mmtdemux->audio_packet_id != GST_SIGNALS_NOT_DETECTED))
				//ParseMMTPV1Pack(mmtdemux, map.data+header_count, buf);
				ParseMMTPV1Pack(mmtdemux, bitstream+header_count, buf);
		}
		else
		{
			//SigOffset =ParseSignal(map.data,mmtdemux);
			SigOffset =ParseSignal(bitstream, mmtdemux);
			GST_DEBUG_OBJECT (mmtdemux, "bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  mmtdemux->offset, SigOffset);
			mmtdemux->offset +=SigOffset;
...
static GstFlowReturn
gst_mmtdemux_chain (GstPad * sinkpad, GstObject * parent, GstBuffer * inbuf)
{
  GstMMTDemux *demux;
  guint64 int_size;
#if 0
     ...
     GstMapInfo map;
	  ...
	  if(fp == NULL)
		  fp = fopen("video.mmt", "w");
	  if(fp != NULL)
	  {
		  gst_buffer_map (inbuf, &map, GST_MAP_READ);
		  fwrite(map.data, 1, gst_buffer_get_size (inbuf), fp);
  ...
  int_size = (guint64)gst_buffer_get_size (inbuf);
		  
  if(demux->mainbuf == NULL)
	 demux->mainbuf = inbuf;
  //return GST_FLOW_OK;
  //gst_adapter_push (demux->adapter, inbuf);
  return send_pkt(demux, inbuf, int_size);
  return adapter_send_pkt(demux, inbuf, int_size);

  if (GST_BUFFER_FLAG_IS_SET (inbuf, GST_BUFFER_FLAG_DISCONT)) {
    gboolean is_gap_input = FALSE;
    gint i;

    GST_DEBUG_OBJECT (demux, "Got DISCONT, marking all streams as DISCONT %d", demux->n_streams);
    ...
    if (demux->upstream_format_is_time
      ...
      for (i = 0; i < demux->n_streams; i++) {
        ...
        res =
            gst_mmtdemux_find_index_for_given_media_offset_linear (demux,
    ...
    if (!is_gap_input && demux->fragmented && demux->segment.rate < 0) {
      gst_mmtdemux_process_adapter (demux, TRUE);
      for (i = 0; i < demux->n_streams; i++)
        gst_mmtdemux_stream_flush_samples_data (demux, demux->streams[i]);
    }
  }
  ...
  return gst_mmtdemux_process_adapter (demux, FALSE);
}
static GstFlowReturn
gst_mmtdemux_process_adapter (GstMMTDemux * demux, gboolean force)
{
	GstFlowReturn ret = GST_FLOW_OK;
		const guint8 *data;
		guint8 *data1;
   ...
   //int SigOffset/*, packet_offset*/;
	...
	while (((gst_adapter_available (demux->adapter)) >= demux->neededbytes) &&
		...
		gst_mmtdemux_check_seekability (demux);

		data = gst_adapter_map (demux->adapter, demux->neededbytes);
	   data1 = (unsigned char*)data;
		  PayldrLength=ParseMMTPHDR(data1, demux, &header_count);
    if(PayldrLength != 0x2)
    {
            GST_DEBUG_OBJECT (demux, "AV_PACKET payload length: %"G_GUINT64_FORMAT, PayldrLength);
				//ParseMMTPV1Pack(demux, data1);
    }
    else
	 {
		 GST_DEBUG_OBJECT (demux, "SIGNALS payload length: %"G_GUINT64_FORMAT, PayldrLength);
		 //SigOffset =ParseSignal(data1, gst_buffer_get_size (data));
		// GST_DEBUG_OBJECT (demux, "Bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  demux->offset, SigOffset);
		 //mmtdemux->offset +=SigOffset;

	 }

		/* get fourcc/length, set neededbytes */
		GST_DEBUG_OBJECT (demux, "MMOSS extract_initial_length_and_fourcc");
...
static GstFlowReturn
bkpgst_mmtdemux_process_adapter (GstMMTDemux * demux, gboolean force)
{
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  while (((gst_adapter_available (demux->adapter)) >= demux->neededbytes) &&
    ...
    switch (demux->state) {
      case QTDEMUX_STATE_INITIAL:{
        ...
        if (fourcc == FOURCC_mdat) {
          gint next_entry = next_entry_size (demux);
				GST_DEBUG_OBJECT (demux, "MDAT chaing QTDEMUX mode to STATE_MOVIE");
          ...
          buffer_data:
            ...
            if (demux->upstream_seekable &&
              ...
              res = mmtdemux_seek_offset (demux, target);
            } else {
      ...
      case QTDEMUX_STATE_HEADER:{
        ...
        /* parse the header */
  GST_DEBUG_OBJECT (demux, "MMMOSS extract_initial_length_and_fourcc");
        ...
        if (fourcc == FOURCC_moov) {
          ...
          if (demux->got_moov && demux->n_streams > 0 &&
            ...
            mmtdemux_parse_moov (demux, data, demux->neededbytes);
            mmtdemux_node_dump (demux, demux->moov_node);
            mmtdemux_parse_tree (demux);
            mmtdemux_prepare_streams (demux);
            if (!demux->got_moov)
              mmtdemux_expose_streams (demux);
            else {

              for (n = 0; n < demux->n_streams; n++) {
                QtDemuxStream *stream = demux->streams[n];

                gst_mmtdemux_configure_stream (demux, stream);
              }
            }

            demux->got_moov = TRUE;
            gst_mmtdemux_check_send_pending_segment (demux);

            /* fragmented streams headers shouldn't contain edts atoms */
            if (!demux->fragmented) {
              for (n = 0; n < demux->n_streams; n++) {
                gst_mmtdemux_stream_send_initial_gap_segments (demux,
                    demux->streams[n]);
              }
            }
          ...
          if ((demux->got_moov || demux->media_caps) && demux->fragmented) {
            ...
            if (!mmtdemux_parse_moof (demux, data, demux->neededbytes,
          ...
          mmtdemux_parse_ftyp (demux, data, demux->neededbytes);
        } else if (fourcc == FOURCC_uuid) {
          GST_DEBUG_OBJECT (demux, "Parsing [uuid]");
          mmtdemux_parse_uuid (demux, data, demux->neededbytes);
        } else if (fourcc == FOURCC_sidx) {
          GST_DEBUG_OBJECT (demux, "Parsing [sidx]");
          mmtdemux_parse_sidx (demux, data, demux->neededbytes);
        } else {
          switch (fourcc) {
            case FOURCC_styp:
        ...
        if (demux->mdatbuffer && demux->n_streams) {
          ...
          if (demux->got_moov && demux->first_mdat != -1
            ...
            res = mmtdemux_seek_offset (demux, demux->first_mdat);
            if (res) {
      ...
      case QTDEMUX_STATE_BUFFER_MDAT:{
        ...
        demux->neededbytes = 16;
        demux->state = QTDEMUX_STATE_INITIAL;
        gst_mmtdemux_post_progress (demux, 1, 1);
      ...
      case QTDEMUX_STATE_MOVIE:{
        ...
        if (demux->fragmented) {
          ...
          if (G_LIKELY (demux->todrop < demux->mdatleft)) {
            ...
            gst_mmtdemux_drop_data (demux, demux->mdatleft);
            demux->mdatleft = 0;
        ...
        if (demux->todrop) {
          if (demux->cenc_aux_info_offset > 0) {
            ...
            if (!mmtdemux_parse_cenc_aux_info (demux, demux->streams[0], &br,
          ...
          gst_mmtdemux_drop_data (demux, demux->todrop);
        }
        ...
        gst_mmtdemux_stream_check_and_change_stsd_index (demux, stream);
        ...
        if (G_LIKELY (!(STREAM_IS_EOS (stream)))) {
          ...
          if (G_UNLIKELY (demux->segment.stop != -1
            ...
            ret = gst_mmtdemux_decorate_and_push_buffer (demux, stream, outbuf,
                dts, pts, duration, keyframe, dts, demux->offset);
          }

          /* combine flows */
          ret = gst_mmtdemux_combine_flows (demux, stream, ret);
        } else {
          /* skip this data, stream is EOS */
          gst_adapter_flush (demux->adapter, demux->neededbytes);
        }
        ...
        GST_DEBUG_OBJECT (demux, "increasing offset %" G_GUINT64_FORMAT " by %u",
            demux->offset, demux->neededbytes);
        demux->offset += demux->neededbytes;
        GST_DEBUG_OBJECT (demux, "offset is now %" G_GUINT64_FORMAT,
            demux->offset);
  ...
  if (ret == GST_FLOW_OK && demux->state == QTDEMUX_STATE_BUFFER_MDAT &&
    ...
    gst_mmtdemux_post_progress (demux, gst_adapter_available (demux->adapter),
        demux->neededbytes);
  }
done:
...
invalid_state:
  {
    GST_ELEMENT_ERROR (demux, STREAM, FAILED,
        (NULL), ("mmtdemuxer invalid state %d", demux->state));
    ret = GST_FLOW_ERROR;
    goto done;
  }
no_moov:
...
static gboolean
mmtdemux_sink_activate (GstPad * sinkpad, GstObject * parent)
{
  GstQuery *query;
  gboolean pull_mode;
...
static gboolean
mmtdemux_sink_activate_mode (GstPad * sinkpad, GstObject * parent,
  ...
  switch (mode) {
    ...
    case GST_PAD_MODE_PULL:
      if (active) {
        ...
        res = gst_pad_start_task (sinkpad, (GstTaskFunction) gst_mmtdemux_loop,
            sinkpad, NULL);
      } else {
...
#ifdef HAVE_ZLIB
static void *
mmtdemux_inflate (void *z_buffer, guint z_length, guint * length)
{
  guint8 *buffer;
  z_stream z;
  int ret;
...
mmtdemux_parse_moov (GstMMTDemux * mmtdemux, const guint8 * buffer, guint length)
{
  ...
  mmtdemux->moov_node = g_node_new ((guint8 *) buffer);

  /* counts as header data */
  mmtdemux->header_size += length;

  GST_DEBUG_OBJECT (mmtdemux, "parsing 'moov' atom");
  mmtdemux_parse_node (mmtdemux, mmtdemux->moov_node, buffer, length);

  cmov = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_cmov);
  if (cmov) {
    guint32 method;
    GNode *dcom;
    GNode *cmvd;
    ...
    dcom = mmtdemux_tree_get_child_by_type (cmov, FOURCC_dcom);
    cmvd = mmtdemux_tree_get_child_by_type (cmov, FOURCC_cmvd);
    ...
    switch (method) {
      ...
      case FOURCC_zlib:{
        ...
        buf =
            (guint8 *) mmtdemux_inflate ((guint8 *) cmvd->data + 12,
            compressed_length, &uncompressed_length);

        if (buf) {
          mmtdemux->moov_node_compressed = mmtdemux->moov_node;
          mmtdemux->moov_node = g_node_new (buf);

          mmtdemux_parse_node (mmtdemux, mmtdemux->moov_node, buf,
              uncompressed_length);
        }
        break;
      }
#endif /* HAVE_ZLIB */
      default:
        GST_WARNING_OBJECT (mmtdemux, "unknown or unhandled header compression "
...
invalid_compression:
  {
    GST_ERROR_OBJECT (mmtdemux, "invalid compressed header");
    return FALSE;
  }
}

static gboolean
mmtdemux_parse_container (GstMMTDemux * mmtdemux, GNode * node, const guint8 * buf,
    const guint8 * end)
{
  while (G_UNLIKELY (buf < end)) {
    ...
    if (G_UNLIKELY (buf + 4 > end)) {
      GST_DEBUG_OBJECT (mmtdemux, "buffer overrun");
      break;
    }
    len = QT_UINT32 (buf);
    if (G_UNLIKELY (len == 0)) {
      GST_DEBUG_OBJECT (mmtdemux, "empty container");
      break;
    }
    if (G_UNLIKELY (len < 8)) {
      GST_WARNING_OBJECT (mmtdemux, "length too short (%d < 8)", len);
      break;
    }
    if (G_UNLIKELY (len > (end - buf))) {
      GST_WARNING_OBJECT (mmtdemux, "length too long (%d > %d)", len,
          (gint) (end - buf));
      break;
    }
    ...
    GST_DEBUG_OBJECT (mmtdemux, "adding new node of len %d", len);
    mmtdemux_parse_node (mmtdemux, child, buf, len);
...
static gboolean
mmtdemux_parse_theora_extension (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GNode * xdxt)
{
  ...
  while (buf < end) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "%p %p", buf, end);
    ...
    GST_WARNING_OBJECT (mmtdemux, "have cookie %" GST_FOURCC_FORMAT,
    ...
    switch (type) {
      case FOURCC_tCtH:
        ...
        GST_DEBUG_OBJECT (mmtdemux, "parsing theora header");
        break;
      case FOURCC_tCt_:
        buffer = gst_buffer_new_and_alloc (size);
        gst_buffer_fill (buffer, 0, buf, size);
        stream->buffers = g_slist_append (stream->buffers, buffer);
        GST_DEBUG_OBJECT (mmtdemux, "parsing theora comment");
        break;
      case FOURCC_tCtC:
        buffer = gst_buffer_new_and_alloc (size);
        gst_buffer_fill (buffer, 0, buf, size);
        stream->buffers = g_slist_append (stream->buffers, buffer);
        GST_DEBUG_OBJECT (mmtdemux, "parsing theora codebook");
        break;
      default:
        GST_WARNING_OBJECT (mmtdemux,
...
static gboolean
mmtdemux_parse_node (GstMMTDemux * mmtdemux, GNode * node, const guint8 * buffer,
    guint length)
{
  guint32 fourcc = 0;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "mmtdemux_parse buffer %p length %u", buffer, length);
  ...
  type = mmtdemux_type_get (fourcc);

  end = buffer + length;

  GST_DEBUG_OBJECT (mmtdemux,
      "MOSS:: parsing '%" GST_FOURCC_FORMAT "', length=%u, name '%s'",
  ...
  if (type->flags & QT_FLAG_CONTAINER) {
    mmtdemux_parse_container (mmtdemux, node, buffer + 8, end);
  } else {
    switch (fourcc) {
      case FOURCC_stsd:
      {
        if (node_length < 20) {
          GST_DEBUG_OBJECT (mmtdemux, "skipping small stsd box");
          break;
        }
        GST_DEBUG_OBJECT (mmtdemux,
        ...
        mmtdemux_parse_container (mmtdemux, node, buffer + 16, end);
        break;
      }
      case FOURCC_hmhd:
      {
        if (node_length < 20) {
          GST_DEBUG_OBJECT (mmtdemux, "skipping hmhd box");
          break;
        }
        GST_DEBUG_OBJECT (mmtdemux,
            "parsing hmhd (hint handler table, hint description) atom");
			mmtdemux_parse_hmhd (mmtdemux, buffer, end - buffer);
        mmtdemux_parse_container (mmtdemux, node, buffer + 16, end);
        break;
      }
      case FOURCC_mp4a:
      ...
      case FOURCC_fLaC:
      {
        ...
        if (length < min_size) {
          GST_DEBUG_OBJECT (mmtdemux, "skipping small %" GST_FOURCC_FORMAT " box",
              GST_FOURCC_ARGS (fourcc));
          break;
        }
        ...
        GST_DEBUG_OBJECT (mmtdemux, "%" GST_FOURCC_FORMAT " version 0x%08x",
        ...
        if (offset)
          mmtdemux_parse_container (mmtdemux, node, buffer + offset, end);
        break;
      }
      case FOURCC_mp4v:
      case FOURCC_MP4V:
      ...
      case FOURCC_encv:
      {
        ...
        if (node_length < 86) {
          GST_WARNING_OBJECT (mmtdemux, "%" GST_FOURCC_FORMAT
              " sample description length too short (%u < 86)",
              GST_FOURCC_ARGS (fourcc), node_length);
          break;
        }

        GST_DEBUG_OBJECT (mmtdemux, "parsing in %" GST_FOURCC_FORMAT,
            GST_FOURCC_ARGS (fourcc));
        ...
        GST_DEBUG_OBJECT (mmtdemux, "version %08x", version);
        ...
        if (str_len < 32)
          GST_DEBUG_OBJECT (mmtdemux, "compressorname = %.*s", str_len,
              (char *) buffer + 51);
        else
          GST_WARNING_OBJECT (mmtdemux,
              "compressorname length too big (%u > 31)", str_len);

        GST_MEMDUMP_OBJECT (mmtdemux, "video sample description", buffer,
            end - buffer);
        mmtdemux_parse_container (mmtdemux, node, buffer + 86, end);
        break;
      }
      case FOURCC_meta:
      {
        GST_DEBUG_OBJECT (mmtdemux, "parsing meta atom");
        ...
        if (QT_FOURCC (buffer + 12) == FOURCC_hdlr) {
          ...
          mmtdemux_parse_container (mmtdemux, node, buffer + 12, end);
        } else
          GST_WARNING_OBJECT (mmtdemux, "Unknown 'meta' atom format");
        break;
      }
      case FOURCC_mp4s:
      {
        GST_MEMDUMP_OBJECT (mmtdemux, "mp4s", buffer, end - buffer);
      ...
      case FOURCC_XiTh:
      {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "parsing XiTh atom version 0x%08x", version);
        ...
        switch (version) {
          case 0x00000001:
            ...
            break;
          default:
            GST_DEBUG_OBJECT (mmtdemux, "unknown version 0x%08x", version);
            offset = 0;
            break;
        }
        if (offset) {
      ...
      case FOURCC_in24:
      {
        mmtdemux_parse_container (mmtdemux, node, buffer + 0x34, end);
        break;
      }
      case FOURCC_uuid:
      {
        mmtdemux_parse_uuid (mmtdemux, buffer, end - buffer);
        break;
      }
      case FOURCC_enca:
      {
        mmtdemux_parse_container (mmtdemux, node, buffer + 36, end);
        break;
      }
      case FOURCC_mmth:
		{
			mmtdemux->ismmth=1;
			mmtdemux_parse_mmth (mmtdemux, buffer+8, node_length);
			mmtdemux_parse_container (mmtdemux, node, buffer + 22, end);
			break;
		}
      default:
  ...
  GST_DEBUG_OBJECT (mmtdemux, "parsed '%" GST_FOURCC_FORMAT "'",
      GST_FOURCC_ARGS (fourcc));
  return TRUE;
...
static GNode *
mmtdemux_tree_get_child_by_type (GNode * node, guint32 fourcc)
{
  GNode *child;
  guint8 *buffer;
...
static GNode *
mmtdemux_tree_get_child_by_type_full (GNode * node, guint32 fourcc,
...
static GNode *
mmtdemux_tree_get_child_by_index (GNode * node, guint index)
{
  return g_node_nth_child (node, index);
}

static GNode *
mmtdemux_tree_get_sibling_by_type_full (GNode * node, guint32 fourcc,
    GstByteReader * parser)
{
  GNode *child;
  guint8 *buffer;
...
static GNode *
mmtdemux_tree_get_sibling_by_type (GNode * node, guint32 fourcc)
{
  return mmtdemux_tree_get_sibling_by_type_full (node, fourcc, NULL);
}

static void
mmtdemux_do_allocation (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
 ...
 */
  ...
  if (!gst_pad_peer_query (stream->pad, query)) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "peer ALLOCATION query failed");
  }
...
static gboolean
gst_mmtdemux_configure_protected_caps (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream)
{
  GstStructure *s;
  ...
  if (stream->protection_scheme_type != FOURCC_cenc) {
    GST_ERROR_OBJECT (mmtdemux, "unsupported protection scheme");
    return FALSE;
  }
  if (mmtdemux->protection_system_ids == NULL) {
    GST_ERROR_OBJECT (mmtdemux, "stream is protected using cenc, but no "
        "cenc protection system information has been found");
    return FALSE;
  }
  g_ptr_array_add (mmtdemux->protection_system_ids, NULL);
  selected_system = gst_protection_select_system ((const gchar **)
      mmtdemux->protection_system_ids->pdata);
  g_ptr_array_remove_index (mmtdemux->protection_system_ids,
      mmtdemux->protection_system_ids->len - 1);
  if (!selected_system) {
    GST_ERROR_OBJECT (mmtdemux, "stream is protected, but no "
...
static gboolean
gst_mmtdemux_configure_stream (GstMMTDemux * mmtdemux, QtDemuxStream * stream)
{
  if (stream->subtype == FOURCC_vide) {
    ...
    if ((stream->n_samples == 1 && stream->first_duration == 0)
        || (mmtdemux->fragmented && stream->n_samples_moof == 1)) {
      /* still frame */
      ...
      if (stream->duration == 0 || stream->n_samples < 2) {
        ...
        if (mmtdemux->fragmented && stream->n_samples_moof > 0
            && stream->duration_moof > 0) {
    ...
    if (CUR_STREAM (stream)->caps) {
      ...
      if (CUR_STREAM (stream)->par_w > 0 && CUR_STREAM (stream)->par_h > 0) {
        GST_DEBUG_OBJECT (mmtdemux, "par %d:%d", CUR_STREAM (stream)->par_w,
        ...
        if (n == d)
          n = d = 1;
        GST_DEBUG_OBJECT (mmtdemux, "setting PAR to %d/%d", n, d);
  ...
  if (stream->pad) {
    GstCaps *prev_caps = NULL;
        //GST_ERROR_OBJECT (mmtdemux,
          //  "RRRRRRRRRRRRRR check for stream pad.");

    GST_PAD_ELEMENT_PRIVATE (stream->pad) = stream;
    gst_pad_set_event_function (stream->pad, gst_mmtdemux_handle_src_event);
    gst_pad_set_query_function (stream->pad, gst_mmtdemux_handle_src_query);
    gst_pad_set_active (stream->pad, TRUE);

    gst_pad_use_fixed_caps (stream->pad);

    if (stream->protected) {
      if (!gst_mmtdemux_configure_protected_caps (mmtdemux, stream)) {
        GST_ERROR_OBJECT (mmtdemux,
    ...
    GST_DEBUG_OBJECT (mmtdemux, "setting caps %" GST_PTR_FORMAT,
        CUR_STREAM (stream)->caps);
    if (stream->new_stream) {
      ...
      event =
          gst_pad_get_sticky_event (mmtdemux->sinkpad, GST_EVENT_STREAM_START,
          0);
      if (event) {
        gst_event_parse_stream_flags (event, &stream_flags);
        if (gst_event_parse_group_id (event, &mmtdemux->group_id))
          mmtdemux->have_group_id = TRUE;
        else
          mmtdemux->have_group_id = FALSE;
        gst_event_unref (event);
      } else if (!mmtdemux->have_group_id) {
        mmtdemux->have_group_id = TRUE;
        mmtdemux->group_id = gst_util_group_id_next ();
      }

      stream->new_stream = FALSE;
      stream_id =
          gst_pad_create_stream_id_printf (stream->pad,
          GST_ELEMENT_CAST (mmtdemux), "%03u", stream->track_id);
      event = gst_event_new_stream_start (stream_id);
      if (mmtdemux->have_group_id)
        gst_event_set_group_id (event, mmtdemux->group_id);
      if (stream->disabled)
    ...
    if (CUR_STREAM (stream)->caps) {
      if (!prev_caps
        ...
        GST_DEBUG_OBJECT (mmtdemux, "ignore duplicated caps");
      }
    } else {
      GST_WARNING_OBJECT (mmtdemux, "stream without caps");
    }
...
static void
gst_mmtdemux_stream_check_and_change_stsd_index (GstMMTDemux * demux,
    QtDemuxStream * stream)
{
...
static gboolean
gst_mmtdemux_add_stream (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, GstTagList * list)
{
  ...
  if (stream->subtype == FOURCC_vide) {
    gchar *name = g_strdup_printf ("video_%u", mmtdemux->n_video_streams);
	 GST_DEBUG_OBJECT (mmtdemux, "QQQQQQQ stream subtype: video");

    stream->pad =
        gst_pad_new_from_static_template (&gst_mmtdemux_videosrc_template, name);
    g_free (name);

    if (!gst_mmtdemux_configure_stream (mmtdemux, stream)) {
    ...
    mmtdemux->n_video_streams++;
  } else if (stream->subtype == FOURCC_soun) {
    gchar *name = g_strdup_printf ("audio_%u", mmtdemux->n_audio_streams);
	 GST_DEBUG_OBJECT (mmtdemux, "QQQQQQQ stream subtype: audio");

    stream->pad =
        gst_pad_new_from_static_template (&gst_mmtdemux_audiosrc_template, name);
    g_free (name);
    if (!gst_mmtdemux_configure_stream (mmtdemux, stream)) {
      gst_object_unref (stream->pad);
      stream->pad = NULL;
    ...
    mmtdemux->n_audio_streams++;
  } else if (stream->subtype == FOURCC_strm) {
    GST_DEBUG_OBJECT (mmtdemux, "stream type, not creating pad");
  } else if (stream->subtype == FOURCC_subp || stream->subtype == FOURCC_text
      || stream->subtype == FOURCC_sbtl || stream->subtype == FOURCC_subt) {
    gchar *name = g_strdup_printf ("subtitle_%u", mmtdemux->n_sub_streams);

    stream->pad =
        gst_pad_new_from_static_template (&gst_mmtdemux_subsrc_template, name);
    g_free (name);
    if (!gst_mmtdemux_configure_stream (mmtdemux, stream)) {
      gst_object_unref (stream->pad);
      stream->pad = NULL;
  ...
  if (stream->pad) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "adding pad %s %p to mmtdemux %p",
        GST_OBJECT_NAME (stream->pad), stream->pad, mmtdemux);
    gst_element_add_pad (GST_ELEMENT_CAST (mmtdemux), stream->pad);
    gst_flow_combiner_add_pad (mmtdemux->flowcombiner, stream->pad);
    ...
    for (l = mmtdemux->protection_event_queue.head; l != NULL; l = l->next)
...
static GstFlowReturn
mmtdemux_find_atom (GstMMTDemux * mmtdemux, guint64 * offset,
    guint64 * length, guint32 fourcc)
{
  ...
  GST_DEBUG_OBJECT (mmtdemux, "finding fourcc %" GST_FOURCC_FORMAT " at offset %"
      G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), *offset);
  GST_DEBUG_OBJECT (mmtdemux, "MMMM finding fourcc %" GST_FOURCC_FORMAT " at offset %"
      G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), *offset);
  ...
  while (TRUE) {
    ...
    buf = NULL;
    ret = gst_pad_pull_range (mmtdemux->sinkpad, *offset, 16, &buf);
    if (G_UNLIKELY (ret != GST_FLOW_OK))
      goto locate_failed;
    ...
    if (G_UNLIKELY (*length == 0)) {
      GST_DEBUG_OBJECT (mmtdemux, "invalid length 0");
      ret = GST_FLOW_ERROR;
      goto locate_failed;
    }

    if (lfourcc == fourcc) {
      GST_DEBUG_OBJECT (mmtdemux, "found fourcc at offset %" G_GUINT64_FORMAT,
          *offset);
      break;
    } else {
      GST_DEBUG_OBJECT (mmtdemux,
...
locate_failed:
  {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "fourcc not found");
    return ret;
  }
}
...
static GstFlowReturn
mmtdemux_add_fragmented_samples (GstMMTDemux * mmtdemux)
{
  ...
  offset = mmtdemux->moof_offset;
  GST_DEBUG_OBJECT (mmtdemux, "next moof at offset %" G_GUINT64_FORMAT, offset);

  if (!offset) {
    GST_DEBUG_OBJECT (mmtdemux, "no next moof");
    return GST_FLOW_EOS;
  }
  ...
  GST_DEBUG_OBJECT (mmtdemux, "SSADA");
  ret = mmtdemux_find_atom (mmtdemux, &offset, &length, FOURCC_moof);
  if (ret != GST_FLOW_OK)
    goto flow_failed;

  ret = gst_mmtdemux_pull_atom (mmtdemux, offset, length, &buf);
  if (G_UNLIKELY (ret != GST_FLOW_OK))
    goto flow_failed;
  gst_buffer_map (buf, &map, GST_MAP_READ);
  if (!mmtdemux_parse_moof (mmtdemux, map.data, map.size, offset, NULL)) {
    gst_buffer_unmap (buf, &map);
    gst_buffer_unref (buf);
    buf = NULL;
    goto parse_failed;
...
exit:
  ...
  mmtdemux->moof_offset = offset;

  return res;

parse_failed:
  {
    GST_DEBUG_OBJECT (mmtdemux, "failed to parse moof");
    offset = 0;
    res = GST_FLOW_ERROR;
    goto exit;
  }
flow_failed:
  {
    ...
    if (ret != GST_FLOW_FLUSHING) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "upstream WRONG_STATE");
...
static gboolean
mmtdemux_stbl_init (GstMMTDemux * mmtdemux, QtDemuxStream * stream, GNode * stbl)
{
  stream->stbl_index = -1;      /* no samples have yet been parsed */
  stream->sample_index = -1;

  /* time-to-sample atom */
  if (!mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stts, &stream->stts))
    goto corrupt_file;
  ...
  if (!gst_byte_reader_skip (&stream->stts, 1 + 3) ||
    ...
    goto corrupt_file;
  GST_DEBUG_OBJECT (mmtdemux, "%u timestamp blocks", stream->n_sample_times);
  ...
  if (!qt_atom_parser_has_chunks (&stream->stts, stream->n_sample_times, 8)) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "overriding to %u timestamp blocks",
  ...
  if ((stream->stss_present =
          ! !mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stss,
    ...
    if ((stream->stps_present =
            ! !mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stps,
  ...
  /* sample size */
  if (!mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stsz, &stream->stsz))
    goto no_samples;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "SSSSSSSSAMPLES n_samples %u",
      stream->n_samples_per_chunk);

  /* sample-to-chunk atom */
  if (!mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stsc, &stream->stsc))
    goto corrupt_file;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "n_samples_per_chunk %u",
      stream->n_samples_per_chunk);
  ...
  /* chunk offset */
  if (mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_stco, &stream->stco))
    stream->co_size = sizeof (guint32);
  else if (mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_co64,
          &stream->stco))
  ...
  GST_DEBUG_OBJECT (mmtdemux, "allocating n_samples %u * %u (%.2f MB)",
  ...
  if ((stream->ctts_present =
          ! !mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_ctts,
    ...
    if (mmtdemux_tree_get_child_by_type_full (stbl, FOURCC_cslg, &cslg)) {
 ...
 */
static gboolean
mmtdemux_parse_samples (GstMMTDemux * mmtdemux, QtDemuxStream * stream, guint32 n)
{
  gint i, j, k;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "parsing samples for stream fourcc %"
  ...
  GST_DEBUG_OBJECT (mmtdemux, "parsing up to sample %u", n);

  if (!stream->stsz.data) {
    /* so we already parsed and passed all the moov samples;
     * onto fragmented ones */
    g_assert (mmtdemux->fragmented);
    goto done;
  }
  ...
  if (!stream->chunks_are_samples) {
    ...
    if (stream->sample_size == 0) {
      ...
      for (cur = first; cur <= last; cur++) {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "sample %d has size %u",
            (guint) (cur - samples), cur->size);
      }
    } else {
      /* samples have the same size */
      GST_DEBUG_OBJECT (mmtdemux, "all samples have size %u", stream->sample_size);
      for (cur = first; cur <= last; cur++)
  ...
  for (i = stream->stsc_index; i < n_samples_per_chunk; i++) {
    ...
    if (stream->stsc_chunk_index >= stream->last_chunk
      ...
      if (G_UNLIKELY (stream->first_chunk == 0))
		{
		  GST_DEBUG_OBJECT(mmtdemux, "CORRUPTED_FILE");
			goto corrupt_file;
		}
    ...
    if (stream->chunks_are_samples) {
      ...
      for (j = stream->stsc_chunk_index; j < last_chunk; j++) {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "Created entry %d with offset "
      ...
      for (j = stream->stsc_chunk_index; j < last_chunk; j++) {
        ...
        for (k = stream->stsc_sample_index; k < samples_per_chunk; k++) {
          GST_DEBUG_OBJECT (mmtdemux, "creating entry %d with offset %"
...
done2:
  {
    ...
    for (i = stream->stts_index; i < n_sample_times; i++) {
      ...
      if (stream->stts_sample_index >= stream->stts_samples
        ...
        GST_DEBUG_OBJECT (mmtdemux, "block %d, %u timestamps, duration %u",
...
done3:
  {
    ...
    if (stream->stss_present == TRUE) {
      ...
      if (!n_sample_syncs) {
        GST_DEBUG_OBJECT (mmtdemux, "all samples are keyframes");
        stream->all_keyframe = TRUE;
      } else {
        for (i = stream->stss_index; i < n_sample_syncs; i++) {
          ...
          if (G_LIKELY (index > 0 && index <= n_samples)) {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "samples at %u is keyframe", index);
      ...
      stream->all_keyframe = TRUE;
      GST_DEBUG_OBJECT (mmtdemux, "setting all keyframes");
    }
  }
...
ctts:
  ...
  if (n + 1 == stream->n_samples) {
    gst_mmtdemux_stbl_free (stream);
    GST_DEBUG_OBJECT (mmtdemux, "parsed all available samples;");
    if (mmtdemux->pullbased) {
      GST_DEBUG_OBJECT (mmtdemux, "checking for more samples");
      while (n + 1 == stream->n_samples)
        if (mmtdemux_add_fragmented_samples (mmtdemux) != GST_FLOW_OK)
          break;
    }
  }
#endif
...
already_parsed:
  {
    ...
    if (mmtdemux->fragmented && n == stream->stbl_index)
      goto done;
    GST_OBJECT_UNLOCK (mmtdemux);
    return TRUE;
  }
 ...
 */
static gboolean
mmtdemux_parse_segments (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    GNode * trak)
{
  GNode *edts;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "looking for edit list container");
  stream->n_segments = 0;
  stream->segments = NULL;
  if ((edts = mmtdemux_tree_get_child_by_type (trak, FOURCC_edts))) {
    GNode *elst;
    gint n_segments;
    ...
    GST_DEBUG_OBJECT (mmtdemux, "looking for edit list");
    if (!(elst = mmtdemux_tree_get_child_by_type (edts, FOURCC_elst)))
      goto done;
    ...
    if (size < 16) {
      GST_WARNING_OBJECT (mmtdemux, "Invalid edit list");
      goto done;
    }
    ...
    for (i = 0; i < n_segments; i++) {
      ...
      if (duration || media_start == GST_CLOCK_TIME_NONE) {
        time += duration;
        stime = QTTIME_TO_GSTTIME (mmtdemux, time);
        segment->duration = stime - segment->time;
      } else {
        /* zero duration does not imply media_start == media_stop
         * but, only specify media_start.*/
        stime = QTTIME_TO_GSTTIME (mmtdemux, mmtdemux->duration);
      ...
      if (rate_int <= 1) {
         ...
         * value */
        GST_WARNING_OBJECT (mmtdemux, "found suspicious rate %" G_GUINT32_FORMAT,
            rate_int);
        segment->rate = 1;
      } else {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "created segment %d time %" GST_TIME_FORMAT
          ...
          stream->timescale);
      if (segment->stop_time > mmtdemux->segment.stop) {
        GST_WARNING_OBJECT (mmtdemux, "Segment %d "
            ...
            GST_TIME_ARGS (mmtdemux->segment.stop));
        mmtdemux->segment.stop = segment->stop_time;
      }

      buffer += entry_size;
    }
    GST_DEBUG_OBJECT (mmtdemux, "found %d segments", count);
  ...
  if (!mmtdemux->pullbased && !allow_pushbased_edts) {
    GST_WARNING_OBJECT (mmtdemux, "streaming; discarding edit list segments");
  ...
  if (stream->n_segments == 0) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "created dummy segment %" GST_TIME_FORMAT,
        GST_TIME_ARGS (stream_duration));
    stream->n_segments = 1;
    stream->dummy_segment = TRUE;
  }
  GST_DEBUG_OBJECT (mmtdemux, "using %d segments", stream->n_segments);
 ...
 */
static void
mmtdemux_parse_svq3_stsd_data (GstMMTDemux * mmtdemux,
    const guint8 * stsd_entry_data, const guint8 ** gamma, GstBuffer ** seqh)
{
  ...
  if (length < 32) {
    GST_WARNING_OBJECT (mmtdemux, "stsd too short");
    goto end;
  }
  ...
  if (version == 3) {
    if (length >= 70) {
      ...
      while (length > 8) {
        ...
        if (size == 0) {
          GST_WARNING_OBJECT (mmtdemux, "Atom of size 0 found, aborting "
              "svq3 atom parsing");
          goto end;
        }
        ...
        switch (fourcc) {
          case FOURCC_gama:{
            if (size == 12) {
              ...
              GST_WARNING_OBJECT (mmtdemux, "Unexpected size %" G_GUINT32_FORMAT
          ...
          case FOURCC_SMI_:{
            if (size > 16 && QT_FOURCC (data) == FOURCC_SEQH) {
              guint32 seqh_size;
              if (_seqh != NULL) {
                GST_WARNING_OBJECT (mmtdemux, "Unexpected second SEQH SMI atom "
                    " found, ignoring");
              } else {
          ...
          default:{
            GST_WARNING_OBJECT (mmtdemux, "Unhandled atom %" GST_FOURCC_FORMAT
      ...
      GST_WARNING_OBJECT (mmtdemux, "SVQ3 entry too short in stsd atom");
    }
  } else {
    GST_WARNING_OBJECT (mmtdemux, "Unexpected version for SVQ3 entry %"
...
static gchar *
mmtdemux_get_rtsp_uri_from_hndl (GstMMTDemux * mmtdemux, GNode * minf)
{
  GNode *dinf;
  GstByteReader dref;
  gchar *uri = NULL;
   ...
   */
  dinf = mmtdemux_tree_get_child_by_type (minf, FOURCC_dinf);
  GST_DEBUG_OBJECT (mmtdemux, "Trying to obtain rtsp URI for stream trak");

  if (dinf) {
    guint32 dref_num_entries = 0;
    if (mmtdemux_tree_get_child_by_type_full (dinf, FOURCC_dref, &dref) &&
      ...
      for (i = 0; i < dref_num_entries; i++) {
        ...
        if (gst_byte_reader_get_uint32_be (&dref, &size) &&
          ...
          if (type == FOURCC_hndl) {
            GST_DEBUG_OBJECT (mmtdemux, "Found hndl atom");
            ...
            if (!gst_byte_reader_skip (&dref, 4) ||
              ...
              GST_WARNING_OBJECT (mmtdemux, "Failed to parse hndl atom");
              break;
            }
            ...
            while (gst_byte_reader_get_remaining (&dref) >= 8) {
              ...
              if (gst_byte_reader_get_uint32_be (&dref, &atom_size) &&
                ...
                if (atom_type == FOURCC_data) {
                  ...
                  GST_DEBUG_OBJECT (mmtdemux, "Found data atom inside "
                  ...
                  if (gst_byte_reader_peek_data (&dref, atom_size - 8,
                    ...
                    if (g_strstr_len ((gchar *) uri_aux, 7, "rtsp://") != NULL)
                      ...
                      GST_WARNING_OBJECT (mmtdemux, "Data atom in hndl atom "
                          "didn't contain a rtsp address");
                  } else {
                    GST_WARNING_OBJECT (mmtdemux, "Failed to get the data "
                        "atom contents");
                  }
                  break;
                }
                ...
                GST_WARNING_OBJECT (mmtdemux, "Failed to parse hndl child "
                    "atom header");
                break;
              }
            }
          ...
          GST_WARNING_OBJECT (mmtdemux, "Error parsing dref atom");
        }
      }
      GST_DEBUG_OBJECT (mmtdemux, "Finished parsing dref atom");
    }
  }
  return uri;
}
...
static guint
mmtdemux_parse_amr_bitrate (GstBuffer * buf, gboolean wb)
{
...
static gboolean
mmtdemux_parse_transformation_matrix (GstMMTDemux * mmtdemux,
    GstByteReader * reader, guint32 * matrix, const gchar * atom)
{
  ...
  GST_DEBUG_OBJECT (mmtdemux, "Transformation matrix from atom %s", atom);
  GST_DEBUG_OBJECT (mmtdemux, "%u.%u %u.%u %u.%u", matrix[0] >> 16,
      matrix[0] & 0xFFFF, matrix[1] >> 16, matrix[1] & 0xFF, matrix[2] >> 16,
      matrix[2] & 0xFF);
  GST_DEBUG_OBJECT (mmtdemux, "%u.%u %u.%u %u.%u", matrix[3] >> 16,
      matrix[3] & 0xFFFF, matrix[4] >> 16, matrix[4] & 0xFF, matrix[5] >> 16,
      matrix[5] & 0xFF);
  GST_DEBUG_OBJECT (mmtdemux, "%u.%u %u.%u %u.%u", matrix[6] >> 16,
...
static void
mmtdemux_inspect_transformation_matrix (GstMMTDemux * mmtdemux,
...
#define QTCHECK_MATRIX(m,a,b,d,e) ((m)[0] == (a << 16) && (m)[1] == (b << 16) && \
  ...
  if (matrix[2] == 0 && matrix[5] == 0 && matrix[8] == 1 << 30) {
    ...
    if (QTCHECK_MATRIX (matrix, 1, 0, 0, 1)) {
      ...
      GST_FIXME_OBJECT (mmtdemux, "Unhandled transformation matrix values");
    }

    GST_DEBUG_OBJECT (mmtdemux, "Transformation matrix rotation %s",
        rotation_tag);
...
static gboolean
mmtdemux_parse_protection_scheme_info (GstMMTDemux * mmtdemux,
    QtDemuxStream * stream, GNode * container, guint32 * original_fmt)
{
  ...
  sinf = mmtdemux_tree_get_child_by_type (container, FOURCC_sinf);
  if (G_UNLIKELY (!sinf)) {
    if (stream->protection_scheme_type == FOURCC_cenc) {
      GST_ERROR_OBJECT (mmtdemux, "sinf box does not contain schi box, which is "
  ...
  frma = mmtdemux_tree_get_child_by_type (sinf, FOURCC_frma);
  if (G_UNLIKELY (!frma)) {
    GST_ERROR_OBJECT (mmtdemux, "sinf box does not contain mandatory frma box");
    return FALSE;
  }

  *original_fmt = QT_FOURCC ((const guint8 *) frma->data + 8);
  GST_DEBUG_OBJECT (mmtdemux, "original stream format: '%" GST_FOURCC_FORMAT "'",
      GST_FOURCC_ARGS (*original_fmt));

  schm = mmtdemux_tree_get_child_by_type (sinf, FOURCC_schm);
  if (!schm) {
    GST_DEBUG_OBJECT (mmtdemux, "sinf box does not contain schm box");
    return FALSE;
  }
  ...
  schi = mmtdemux_tree_get_child_by_type (sinf, FOURCC_schi);
  if (!schi) {
    GST_DEBUG_OBJECT (mmtdemux, "sinf box does not contain schi box");
    return FALSE;
  }
  if (stream->protection_scheme_type == FOURCC_cenc) {
    QtDemuxCencSampleSetInfo *info;
    ...
    tenc = mmtdemux_tree_get_child_by_type (schi, FOURCC_tenc);
    if (!tenc) {
      GST_ERROR_OBJECT (mmtdemux, "schi box does not contain tenc box, "
 ...
 */
static gboolean
mmtdemux_parse_trak (GstMMTDemux * mmtdemux, GNode * trak)
{
  GstByteReader tkhd;
  int offset;
  GNode *mdia;
  ...
  guint32 track_id;
  guint32 dummy;
  static int trak_count, mediaType;
  if(!trak_count || (mediaType != mmtdemux->mediaType))
	  trak_count = 1;
  else
	 trak_count = 2;
	 mediaType = mmtdemux->mediaType;

  GST_DEBUG_OBJECT (mmtdemux, "parse_trak");

  if (!mmtdemux_tree_get_child_by_type_full (trak, FOURCC_tkhd, &tkhd)
  ...
  if (!gst_byte_reader_skip (&tkhd, value_size * 2) ||
    ...
    goto corrupt_file;
  GST_DEBUG_OBJECT (mmtdemux, "MM, parse_trak: tkhd_version:%d, tkhd_flags:%d, trak_id: %d mediaType: %d\n", tkhd_version, tkhd_flags, track_id, mmtdemux->mediaType);
#if 0
	 if( (track_id != 2) && (mmtdemux->n_streams == 2) )
	 {
		 for (n = 0; n < mmtdemux->n_streams; n++) {
			 gst_mmtdemux_stream_free (mmtdemux, mmtdemux->streams[n]);
			 mmtdemux->streams[n] = NULL;
  ...
  if (!mmtdemux->got_moov) {
  //if (!mmtdemux->moov_detected) {
	 //TODO: BY SAFWAN... JUST FOR NOW
	 //if(track_id == 2 /*&& mmtdemux->ismmth*/)
	 //if(track_id == 2 && mmtdemux->ismmth)
	 if(trak_count == 2)
	 {
		 GST_DEBUG_OBJECT (mmtdemux, "ZZZZZZ skip track for Hint Handler");
		 goto skip_track;
	 }
	 if(mmtdemux->mediaType == 1 /*&& mmtdemux->ismmth*/)
		  track_id = 2;
    GST_DEBUG_OBJECT (mmtdemux, "didnt get_moov");
    if (mmtdemux_find_stream (mmtdemux, track_id))
	 {
	 //TODO:// for now commenting this part create creation is getting failed 
	 ...
	 // if(track_id == 2 /*&& mmtdemux->ismmth*/)
	  ...
	  //if(mmtdemux->mediaType == 2 && mmtdemux->ismmth)
	  if(mmtdemux->mediaType == 1)
		  track_id = 2;
	  GST_DEBUG_OBJECT (mmtdemux, " got_moov");
	  stream = mmtdemux_find_stream (mmtdemux, track_id);
	  if (!stream) {
		  GST_WARNING_OBJECT (mmtdemux, "Stream not found, going to ignore it");
		  goto skip_track;
	  }
  ...
  mmtdemux_parse_trex (mmtdemux, stream, &dummy, &dummy, &dummy);

  if ((tkhd_flags & 1) == 0)
    stream->disabled = TRUE;

  GST_DEBUG_OBJECT (mmtdemux, "track[tkhd] version/flags/id: 0x%02x/%06x/%u",
      tkhd_version, tkhd_flags, stream->track_id);

  if (!(mdia = mmtdemux_tree_get_child_by_type (trak, FOURCC_mdia)))
    goto corrupt_file;

  if (!(mdhd = mmtdemux_tree_get_child_by_type (mdia, FOURCC_mdhd))) {
    /* be nice for some crooked mjp2 files that use mhdr for mdhd */
    if (mmtdemux->major_brand != FOURCC_mjp2 ||
        !(mdhd = mmtdemux_tree_get_child_by_type (mdia, FOURCC_mhdr)))
      goto corrupt_file;
  }

  len = QT_UINT32 ((guint8 *) mdhd->data);
  version = QT_UINT32 ((guint8 *) mdhd->data + 8);
  GST_DEBUG_OBJECT (mmtdemux, "track version/flags: %08x", version);
  ...
  if (lang_code < 0x400) {
    mmtdemux_lang_map_qt_code_to_iso (stream->lang_id, lang_code);
  ...
  GST_DEBUG_OBJECT (mmtdemux, "track timescale: %" G_GUINT32_FORMAT,
      stream->timescale);
  GST_DEBUG_OBJECT (mmtdemux, "track duration: %" G_GUINT64_FORMAT,
      stream->duration);
  GST_DEBUG_OBJECT (mmtdemux, "track language code/id: 0x%04x/%s",
      lang_code, stream->lang_id);

  if (G_UNLIKELY (stream->timescale == 0 || mmtdemux->timescale == 0))
    goto corrupt_file;

  if ((tref = mmtdemux_tree_get_child_by_type (trak, FOURCC_tref))) {
    /* chapters track reference */
    GNode *chap = mmtdemux_tree_get_child_by_type (tref, FOURCC_chap);
    if (chap) {
      gsize length = GST_READ_UINT32_BE (chap->data);
      if (mmtdemux->chapters_track_id)
        GST_FIXME_OBJECT (mmtdemux, "Multiple CHAP tracks");

      if (length >= 12) {
        mmtdemux->chapters_track_id =
  ...
  if (!mmtdemux->fragmented &&
      mmtdemux->duration != G_MAXINT64 && stream->duration != G_MAXINT32) {
    guint64 tdur1, tdur2;

    /* don't overflow */
    tdur1 = stream->timescale * (guint64) mmtdemux->duration;
    tdur2 = mmtdemux->timescale * (guint64) stream->duration;
    ...
    if (tdur1 != 0 && (tdur2 * 10 / tdur1) < 2) {
      GST_WARNING_OBJECT (mmtdemux,
          ...
          stream->duration, stream->timescale, mmtdemux->duration,
          mmtdemux->timescale);
      if (new_stream)
        gst_mmtdemux_stream_free (mmtdemux, stream);
      return TRUE;
    }
  }

  if (!(hdlr = mmtdemux_tree_get_child_by_type (mdia, FOURCC_hdlr)))
    goto corrupt_file;

  GST_DEBUG_OBJECT (mmtdemux, "track type: %" GST_FOURCC_FORMAT,
      GST_FOURCC_ARGS (QT_FOURCC ((guint8 *) hdlr->data + 12)));

  len = QT_UINT32 ((guint8 *) hdlr->data);
  if (len >= 20)
    stream->subtype = QT_FOURCC ((guint8 *) hdlr->data + 16);
  GST_DEBUG_OBJECT (mmtdemux, "track subtype: %" GST_FOURCC_FORMAT,
      GST_FOURCC_ARGS (stream->subtype));

  if (!(minf = mmtdemux_tree_get_child_by_type (mdia, FOURCC_minf)))
    goto corrupt_file;

  if (!(stbl = mmtdemux_tree_get_child_by_type (minf, FOURCC_stbl)))
    goto corrupt_file;

  /*parse svmi header if existing */
  svmi = mmtdemux_tree_get_child_by_type (stbl, FOURCC_svmi);
  if (svmi) {
    ...
    if (!version) {
      ...
      /* MPEG-A stereo video */
      if (mmtdemux->major_brand == FOURCC_ss02)
  ...
  if (stream->subtype == FOURCC_vide) {
    ...
    if (!mmtdemux_parse_transformation_matrix (mmtdemux, &tkhd, matrix, "tkhd"))
      goto corrupt_file;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "KKKK w: %u h: %u   ", w, h);

    mmtdemux_inspect_transformation_matrix (mmtdemux, stream, matrix,
        &stream->stream_tags);
  }

  /* parse stsd */
  if (!(stsd = mmtdemux_tree_get_child_by_type (stbl, FOURCC_stsd)))
    goto corrupt_file;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "stsd len:           %d", stsd_len);
  GST_DEBUG_OBJECT (mmtdemux, "stsd entry count:   %u", stsd_entry_count);
  ...
  for (stsd_index = 0; stsd_index < stsd_entry_count; stsd_index++) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "stsd type:          %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (entry->fourcc));
    GST_DEBUG_OBJECT (mmtdemux, "stsd type len:      %d", len);
    ...
    if (fourcc == FOURCC_encv || fourcc == FOURCC_enca) {
       ...
       * with the same type */
      GNode *enc = mmtdemux_tree_get_child_by_type (stsd, fourcc);
      stream->protected = TRUE;
      if (!mmtdemux_parse_protection_scheme_info (mmtdemux, stream, enc, &fourcc))
        GST_ERROR_OBJECT (mmtdemux, "Failed to parse protection scheme info");
    }
    ...
    if (stream->subtype == FOURCC_vide) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "width %d, height %d, bps %d, color table id %d",
      ...
      if (entry->color_table_id) {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "start %d, end %d, palette_count %d",
      ...
      entry->caps =
          mmtdemux_video_caps (mmtdemux, stream, entry, fourcc, stsd_entry_data,
          &codec);
      ...
      if (palette_data) {
        ...
        GST_ELEMENT_WARNING (mmtdemux, STREAM, NOT_IMPLEMENTED,
            (NULL), ("Unsupported palette depth %d", depth));
      }

      GST_DEBUG_OBJECT (mmtdemux, "frame count:   %u",
      ...
      fiel = NULL;
      /* pick 'the' stsd child */
      mp4v = mmtdemux_tree_get_child_by_index (stsd, stsd_index);
      if (!stream->protected) {
      ...
      if (mp4v) {
        esds = mmtdemux_tree_get_child_by_type (mp4v, FOURCC_esds);
        pasp = mmtdemux_tree_get_child_by_type (mp4v, FOURCC_pasp);
        colr = mmtdemux_tree_get_child_by_type (mp4v, FOURCC_colr);
        fiel = mmtdemux_tree_get_child_by_type (mp4v, FOURCC_fiel);
      }
      ...
      if (colr) {
        ...
        if (len == 19 || len == 18) {
          ...
          if (color_type == FOURCC_nclx || color_type == FOURCC_nclc) {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "Unsupported color type");
          }
        } else {
          GST_WARNING_OBJECT (mmtdemux, "Invalid colr atom size");
        }
      }

      if (esds) {
        gst_mmtdemux_handle_esds (mmtdemux, stream, entry, esds,
            stream->stream_tags);
      } else {
        switch (fourcc) {
          case FOURCC_H264:
            ...
            while (len >= 0x8) {
              ...
              switch (QT_FOURCC (avc_data + 0x4)) {
                case FOURCC_avcC:
                {
                  ...
                  GST_DEBUG_OBJECT (mmtdemux, "found avcC codec_data in stsd");
                ...
                case FOURCC_strf:
                {
                  ...
                  GST_DEBUG_OBJECT (mmtdemux, "found strf codec_data in stsd");
            ...
            while (len >= 0x8) {
              ...
              switch (QT_FOURCC (hevc_data + 0x4)) {
                case FOURCC_hvcC:
                {
                  ...
                  GST_DEBUG_OBJECT (mmtdemux, "found hvcC codec_data in stsd");
          ...
          case FOURCC_XVID:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "found %" GST_FOURCC_FORMAT,
                GST_FOURCC_ARGS (fourcc));

            /* codec data might be in glbl extension atom */
            glbl = mp4v ?
                mmtdemux_tree_get_child_by_type (mp4v, FOURCC_glbl) : NULL;
            if (glbl) {
              guint8 *data;
              GstBuffer *buf;
              gint len;

              GST_DEBUG_OBJECT (mmtdemux, "found glbl data in stsd");
              data = glbl->data;
              len = QT_UINT32 (data);
              if (len > 0x8) {
                len -= 0x8;
          ...
          case FOURCC_mjp2:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "found mjp2");
            /* some required atoms */
            mjp2 = mmtdemux_tree_get_child_by_index (stsd, stsd_index);
            if (!mjp2)
              break;
            jp2h = mmtdemux_tree_get_child_by_type (mjp2, FOURCC_jp2h);
            if (!jp2h)
              break;

            /* number of components; redundant with info in codestream, but useful
               to a muxer */
            ihdr = mmtdemux_tree_get_child_by_type (jp2h, FOURCC_ihdr);
            if (!ihdr || QT_UINT32 (ihdr->data) != 22)
              break;
            ncomp = QT_UINT16 (((guint8 *) ihdr->data) + 16);

            colr = mmtdemux_tree_get_child_by_type (jp2h, FOURCC_colr);
            if (!colr)
              break;
            GST_DEBUG_OBJECT (mmtdemux, "found colr");
            ...
            cmap = mmtdemux_tree_get_child_by_type (jp2h, FOURCC_cmap);
            if (cmap) {
              guint32 cmap_len = 0;
              int i;
            ...
            cdef = mmtdemux_tree_get_child_by_type (jp2h, FOURCC_cdef);
            if (cdef) {
              guint32 cdef_len = 0;
              int i;
            ...
            /* some optional atoms */
            field = mmtdemux_tree_get_child_by_type (mjp2, FOURCC_fiel);
            prefix = mmtdemux_tree_get_child_by_type (mjp2, FOURCC_jp2x);
            ...
            if (prefix) {
              ...
              GST_DEBUG_OBJECT (mmtdemux, "found prefix data in stsd");
              data = prefix->data;
              len = QT_UINT32 (data);
              if (len > 0x8) {
                len -= 0x8;
          ...
          case FOURCC_VP31:
          {
            ...
            mmtdemux_parse_svq3_stsd_data (mmtdemux, stsd_entry_data, &gamma_data,
                &seqh);
            if (gamma_data) {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "found codec_data in stsd");
            buf = gst_buffer_new_and_alloc (len);
          ...
          case FOURCC_jpeg:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "found jpeg");

            /* recover information on interlaced/progressive */
            jpeg = mmtdemux_tree_get_child_by_type (stsd, FOURCC_jpeg);
            if (!jpeg)
              break;

            len = QT_UINT32 (jpeg->data);
            GST_DEBUG_OBJECT (mmtdemux, "Found jpeg: len %u, need %d", len,
                min_size);
            if (len >= min_size) {
          ...
          case FOURCC_XiTh:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "found XiTh");
            xith = mmtdemux_tree_get_child_by_index (stsd, stsd_index);
            if (!xith)
              break;

            xdxt = mmtdemux_tree_get_child_by_type (xith, FOURCC_XdxT);
            if (!xdxt)
              break;

            GST_DEBUG_OBJECT (mmtdemux, "found XdxT node");
            /* collect the headers and store them in a stream list so that we can
             * send them out first */
            mmtdemux_parse_theora_extension (mmtdemux, stream, xdxt);
            break;
          }
          case FOURCC_ovc1:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "parse ovc1 header");
            ovc1 = mmtdemux_tree_get_child_by_index (stsd, stsd_index);
            if (!ovc1)
              break;
            ovc1_data = ovc1->data;
            ovc1_len = QT_UINT32 (ovc1_data);
            if (ovc1_len <= 198) {
              GST_WARNING_OBJECT (mmtdemux, "Too small ovc1 header, skipping");
              break;
            }
            ...
            while (len >= 8) {
              ...
              switch (QT_FOURCC (vc1_data + 0x4)) {
                case GST_MAKE_FOURCC ('d', 'v', 'c', '1'):
                {
                  ...
                  GST_DEBUG_OBJECT (mmtdemux, "found dvc1 codec_data in stsd");
                  buf = gst_buffer_new_and_alloc (size);
      ...
      GST_DEBUG_OBJECT (mmtdemux, "version/rev:      %08x", version);
      GST_DEBUG_OBJECT (mmtdemux, "vendor:           %08x",
          QT_UINT32 (stsd_entry_data + offset + 4));
      GST_DEBUG_OBJECT (mmtdemux, "n_channels:       %d", entry->n_channels);
      GST_DEBUG_OBJECT (mmtdemux, "sample_size:      %d", samplesize);
      GST_DEBUG_OBJECT (mmtdemux, "compression_id:   %d", compression_id);
      GST_DEBUG_OBJECT (mmtdemux, "packet size:      %d",
          QT_UINT16 (stsd_entry_data + offset + 14));
      GST_DEBUG_OBJECT (mmtdemux, "sample rate:      %g", entry->rate);
      ...
      if (version == 0x00010000) {
        ...
        switch (fourcc) {
          ...
          default:
          {
            ...
            GST_DEBUG_OBJECT (mmtdemux, "samples/packet:   %d",
                entry->samples_per_packet);
            GST_DEBUG_OBJECT (mmtdemux, "bytes/packet:     %d",
                entry->bytes_per_packet);
            GST_DEBUG_OBJECT (mmtdemux, "bytes/frame:      %d",
                entry->bytes_per_frame);
            GST_DEBUG_OBJECT (mmtdemux, "bytes/sample:     %d",
            ...
            if (!entry->sampled && entry->bytes_per_packet) {
              ...
              GST_DEBUG_OBJECT (mmtdemux, "samples/frame:    %d",
        ...
        GST_DEBUG_OBJECT (mmtdemux, "Sound sample description Version 2");
        GST_DEBUG_OBJECT (mmtdemux, "sample rate:        %g", entry->rate);
        GST_DEBUG_OBJECT (mmtdemux, "n_channels:         %d", entry->n_channels);
        GST_DEBUG_OBJECT (mmtdemux, "bits/channel:       %d",
            QT_UINT32 (stsd_entry_data + offset + 20));
        GST_DEBUG_OBJECT (mmtdemux, "format flags:       %X",
            QT_UINT32 (stsd_entry_data + offset + 24));
        GST_DEBUG_OBJECT (mmtdemux, "bytes/packet:       %d",
            QT_UINT32 (stsd_entry_data + offset + 28));
        GST_DEBUG_OBJECT (mmtdemux, "LPCM frames/packet: %d",
            QT_UINT32 (stsd_entry_data + offset + 32));
      } else if (version != 0x00000) {
        GST_WARNING_OBJECT (mmtdemux, "unknown audio STSD version %08x",
            version);
      }
      ...
      entry->caps = mmtdemux_audio_caps (mmtdemux, stream, entry, fourcc,
      ...
      switch (fourcc) {
        case FOURCC_in24:
        {
          ...
          in24 = mmtdemux_tree_get_child_by_type (stsd, FOURCC_in24);

          enda = mmtdemux_tree_get_child_by_type (in24, FOURCC_enda);
          if (!enda) {
            wave = mmtdemux_tree_get_child_by_type (in24, FOURCC_wave);
            if (wave)
              enda = mmtdemux_tree_get_child_by_type (wave, FOURCC_enda);
          }
          if (enda) {
        ...
        case FOURCC_owma:
        {
          ...
          GST_DEBUG_OBJECT (mmtdemux, "parse owma");
          owma_data = stsd_entry_data;
          owma_len = QT_UINT32 (owma_data);
          if (owma_len <= 54) {
            GST_WARNING_OBJECT (mmtdemux, "Too small owma header, skipping");
            break;
          }
        ...
        case FOURCC_wma_:
        {
          ...
          GST_DEBUG_OBJECT (mmtdemux, "parse wma, looking for wfex");
          ...
          while (len >= 8) {
            ...
            switch (QT_FOURCC (wfex_data + 4)) {
              ...
              {
                GST_DEBUG_OBJECT (mmtdemux, "found wfex in stsd");
                ...
                GST_DEBUG_OBJECT (mmtdemux, "Found wfex box in stsd:");
                GST_DEBUG_OBJECT (mmtdemux, "FormatTag = 0x%04x, Channels = %u, "
                ...
                if (size > wfex.cbSize) {
                  ...
                  GST_WARNING_OBJECT (mmtdemux, "no codec data");
                }
      ...
      mp4a = mmtdemux_tree_get_child_by_index (stsd, stsd_index);
	   GST_DEBUG_OBJECT(mmtdemux,"stsd_index: %u\n", stsd_index);
      if (!stream->protected) {
      } else {
        if (QTDEMUX_TREE_NODE_FOURCC (mp4v) != FOURCC_encv) {
          mp4v = NULL;
      ...
      esds = NULL;
      if (mp4a) {
        wave = mmtdemux_tree_get_child_by_type (mp4a, FOURCC_wave);
        if (wave)
          esds = mmtdemux_tree_get_child_by_type (wave, FOURCC_esds);
        if (!esds)
          esds = mmtdemux_tree_get_child_by_type (mp4a, FOURCC_esds);
      }
      ...
      if ((fourcc & 0xffff) == (('s' << 8) | 'm')) {
        if (len < offset + 20) {
          GST_WARNING_OBJECT (mmtdemux, "No wave atom in MS-style audio");
        } else {
          ...
          if (mmtdemux_parse_node (mmtdemux, wavenode, data, datalen)) {
            const guint8 *waveheader;
            guint32 headerlen;

            waveheadernode = mmtdemux_tree_get_child_by_type (wavenode, fourcc);
            if (waveheadernode) {
              ...
              if (headerlen > 8) {
                ...
                if (gst_riff_parse_strf_auds (GST_ELEMENT_CAST (mmtdemux),
        ...
        switch (fourcc) {
          ...
          case FOURCC_alac:
          {
            ...
            alac = mmtdemux_tree_get_child_by_type (stsd, FOURCC_alac);
            if (alac) {
              /* alac now refers to stsd entry atom */
              wave = mmtdemux_tree_get_child_by_type (alac, FOURCC_wave);
              if (wave)
                alac = mmtdemux_tree_get_child_by_type (wave, FOURCC_alac);
              else
                alac = mmtdemux_tree_get_child_by_type (alac, FOURCC_alac);
            }
            if (alac) {
          ...
          case FOURCC_fLaC:
          {
            ...
            GNode *flac = mmtdemux_tree_get_child_by_type (stsd, FOURCC_fLaC);

            if (flac) {
              /* The 'dfLa' box is added to the sample entry to convey
                 initializing information for the decoder. */
              const GNode *dfla =
                  mmtdemux_tree_get_child_by_type (flac, FOURCC_dfLa);
          ...
          case FOURCC_samr:
          {
            ...
            if (len > 0x24) {
              ...
              if ((len - 0x34) > 8 &&
                  (bitrate = mmtdemux_parse_amr_bitrate (buf, amrwb))) {
                gst_tag_list_add (stream->stream_tags, GST_TAG_MERGE_REPLACE,
      ...
      if (fourcc == FOURCC_rtsp) {
        stream->redirect_uri = mmtdemux_get_rtsp_uri_from_hndl (mmtdemux, minf);
      } else {
        GST_INFO_OBJECT (mmtdemux, "unhandled stream type %" GST_FOURCC_FORMAT,
            GST_FOURCC_ARGS (fourcc));
        goto unknown_stream;
      }
      entry->sampled = TRUE;
      ...
      entry->caps =
          mmtdemux_sub_caps (mmtdemux, stream, entry, fourcc, stsd_entry_data,
          &codec);
      if (codec) {
        gst_tag_list_add (stream->stream_tags, GST_TAG_MERGE_REPLACE,
      ...
      switch (fourcc) {
        case FOURCC_mp4s:
        {
          ...
          mp4s = mmtdemux_tree_get_child_by_type (stsd, FOURCC_mp4s);
          if (mp4s)
            esds = mmtdemux_tree_get_child_by_type (mp4s, FOURCC_esds);
          if (esds == NULL) {
            /* Invalid STSD */
            GST_DEBUG_OBJECT (mmtdemux, "Skipping invalid stsd: no esds child");
            break;
          }
      ...
      entry->caps =
          mmtdemux_generic_caps (mmtdemux, stream, entry, fourcc, stsd_entry_data,
          &codec);
    ...
    remaining_stsd_len -= len;
	 ...
	 GST_DEBUG_OBJECT (mmtdemux, "STBL INIT");
  /* collect sample information */
  if (!mmtdemux_stbl_init (mmtdemux, stream, stbl))
  {
	 GST_DEBUG_OBJECT (mmtdemux, "STBL SAMPLE FAILED");
	  goto samples_failed;
  }

	 GST_DEBUG_OBJECT (mmtdemux, "STBL INIT LATER");
  if (mmtdemux->fragmented) {
    guint64 offset;
	 GST_DEBUG_OBJECT (mmtdemux, "STBL samples:%"G_GUINT32_FORMAT, stream->n_samples);
    ...
    mmtdemux->moof_offset = 0;
    if (stream->n_samples &&
        !mmtdemux_parse_samples (mmtdemux, stream, stream->n_samples - 1)) {
	 GST_DEBUG_OBJECT (mmtdemux, "STBL SAMPLE FAILED");
      mmtdemux->moof_offset = offset;
      goto samples_failed;
    }
    ...
    if (mmtdemux->segment.duration &&
        GST_CLOCK_TIME_IS_VALID (mmtdemux->segment.duration))
      stream->duration =
          GSTTIME_TO_QTSTREAMTIME (stream, mmtdemux->segment.duration);
  }

  /* configure segments */
  if (!mmtdemux_parse_segments (mmtdemux, stream, trak))
  ...
  /* Check for UDTA tags */
  if ((udta = mmtdemux_tree_get_child_by_type (trak, FOURCC_udta))) {
    mmtdemux_parse_udta (mmtdemux, stream->stream_tags, udta);
  }

  /* now we are ready to add the stream */
  if (mmtdemux->n_streams >= GST_QTDEMUX_MAX_STREAMS)
    goto too_many_streams;

    GST_DEBUG_OBJECT (mmtdemux, "TTTTTTT n_streams is now %d", mmtdemux->n_streams);
    mmtdemux->streams[mmtdemux->n_streams] = stream;
  if (!mmtdemux->got_moov) {
  //if (!mmtdemux->moov_detected) {
    mmtdemux->streams[mmtdemux->n_streams] = stream;
    mmtdemux->n_streams++;
    GST_DEBUG_OBJECT (mmtdemux, "n_streams is now %d", mmtdemux->n_streams);
  }
...
skip_track:
  {
    GST_INFO_OBJECT (mmtdemux, "skip disabled track");
    if (new_stream)
      gst_mmtdemux_stream_free (mmtdemux, stream);
    return TRUE;
  }
...
error_encrypted:
  {
    GST_ELEMENT_ERROR (mmtdemux, STREAM, DECRYPT, (NULL), (NULL));
    if (new_stream)
      gst_mmtdemux_stream_free (mmtdemux, stream);
    return FALSE;
  }
...
existing_stream:
  {
    GST_INFO_OBJECT (mmtdemux, "stream with track id %i already exists",
        track_id);
    if (new_stream)
      gst_mmtdemux_stream_free (mmtdemux, stream);
    return TRUE;
...
unknown_stream:
  {
    GST_INFO_OBJECT (mmtdemux, "unknown subtype %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (stream->subtype));
    if (new_stream)
      gst_mmtdemux_stream_free (mmtdemux, stream);
    return TRUE;
...
static void
gst_mmtdemux_guess_bitrate (GstMMTDemux * mmtdemux)
{
  QtDemuxStream *stream = NULL;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "Looking for streams with unknown bitrate");

  if (!gst_pad_peer_query_duration (mmtdemux->sinkpad, GST_FORMAT_BYTES, &size)
      || size <= 0) {
    GST_DEBUG_OBJECT (mmtdemux,
  ...
  GST_DEBUG_OBJECT (mmtdemux, "Total size %" G_GINT64_FORMAT ", header size %u",
      size, mmtdemux->header_size);

  if (size < mmtdemux->header_size)
    return;

  size = size - mmtdemux->header_size;

  if (!gst_mmtdemux_get_duration (mmtdemux, &duration)) {
    GST_DEBUG_OBJECT (mmtdemux, "Stream duration not known - bailing");
    return;
  }

  for (i = 0; i < mmtdemux->n_streams; i++) {
    switch (mmtdemux->streams[i]->subtype) {
      case FOURCC_soun:
      case FOURCC_vide:
        GST_DEBUG_OBJECT (mmtdemux, "checking bitrate for %" GST_PTR_FORMAT,
            CUR_STREAM (mmtdemux->streams[i])->caps);
        /* retrieve bitrate, prefer avg then max */
        bitrate = 0;
        if (mmtdemux->streams[i]->stream_tags) {
          if (gst_tag_list_get_uint (mmtdemux->streams[i]->stream_tags,
                  GST_TAG_MAXIMUM_BITRATE, &bitrate))
            GST_DEBUG_OBJECT (mmtdemux, "max-bitrate: %u", bitrate);
          if (gst_tag_list_get_uint (mmtdemux->streams[i]->stream_tags,
                  GST_TAG_NOMINAL_BITRATE, &bitrate))
            GST_DEBUG_OBJECT (mmtdemux, "nominal-bitrate: %u", bitrate);
          if (gst_tag_list_get_uint (mmtdemux->streams[i]->stream_tags,
                  GST_TAG_BITRATE, &bitrate))
            GST_DEBUG_OBJECT (mmtdemux, "bitrate: %u", bitrate);
        }
        if (bitrate)
          sum_bitrate += bitrate;
        else {
  ...
  if (!stream) {
    GST_DEBUG_OBJECT (mmtdemux, "All stream bitrates are known");
    return;
  }
  ...
  GST_DEBUG_OBJECT (mmtdemux, "System bitrate = %" G_GINT64_FORMAT
...
GstFlowReturn
mmtdemux_prepare_streams (GstMMTDemux * mmtdemux)
{
  gint i;
  GstFlowReturn ret = GST_FLOW_OK;

  GST_DEBUG_OBJECT (mmtdemux, "prepare streams");

  for (i = 0; ret == GST_FLOW_OK && i < mmtdemux->n_streams; i++) {
    QtDemuxStream *stream = mmtdemux->streams[i];
    guint32 sample_num = 0;

    GST_DEBUG_OBJECT (mmtdemux, "stream %d, id %d, fourcc %" GST_FOURCC_FORMAT "no of stream %d",
        i, stream->track_id, GST_FOURCC_ARGS (CUR_STREAM (stream)->fourcc), mmtdemux->n_streams);
#if 0
    if (mmtdemux->fragmented) {
      /* need all moov samples first */
      GST_OBJECT_LOCK (mmtdemux);
      while (stream->n_samples == 0)
        if ((ret = mmtdemux_add_fragmented_samples (mmtdemux)) != GST_FLOW_OK)
		  {
		   GST_DEBUG_OBJECT (mmtdemux, "added fragmented samples from streams");
          break;
		  }
      GST_OBJECT_UNLOCK (mmtdemux);
    } else {
    ...
    if (G_UNLIKELY (mmtdemux->pullbased && !stream->n_samples)) {
      GST_DEBUG_OBJECT (mmtdemux, "no samples for stream; discarding");
      gst_mmtdemux_remove_stream (mmtdemux, i);
      i--;
      continue;
    }
    ...
    while (sample_num == 0 && sample_num < stream->n_samples) {
      if (!mmtdemux_parse_samples (mmtdemux, stream, sample_num))
        break;
      ++sample_num;
    }
    if (stream->n_samples > 0 && stream->stbl_index >= 0) {
      stream->first_duration = stream->samples[0].duration;
      GST_DEBUG_OBJECT (mmtdemux, "stream %d first duration %u",
...
GstFlowReturn
mmtdemux_expose_streams (GstMMTDemux * mmtdemux)
{
  gint i;
  GSList *oldpads = NULL;
  GSList *iter;

  GST_DEBUG_OBJECT (mmtdemux, "exposing streams");
  ...
  for (i = 0; i < mmtdemux->n_streams; i++) {
    ...
    if (!gst_mmtdemux_add_stream (mmtdemux, stream, list))
	 {
	   GST_DEBUG_OBJECT (mmtdemux, "exposed streams Failed");
      return GST_FLOW_ERROR;
	 }
  }

  gst_mmtdemux_guess_bitrate (mmtdemux);

  gst_element_no_more_pads (GST_ELEMENT_CAST (mmtdemux));
  ...
  for (iter = oldpads; iter; iter = g_slist_next (iter)) {
    ...
    gst_element_remove_pad (GST_ELEMENT (mmtdemux), oldpad);
    gst_flow_combiner_remove_pad (mmtdemux->flowcombiner, oldpad);
    gst_object_unref (oldpad);
  }

  /* check if we should post a redirect in case there is a single trak
   * and it is a redirecting trak */
  if (mmtdemux->n_streams == 1 && mmtdemux->streams[0]->redirect_uri != NULL) {
    GstMessage *m;

    GST_INFO_OBJECT (mmtdemux, "Issuing a redirect due to a single track with "
        "an external content");
    m = gst_message_new_element (GST_OBJECT_CAST (mmtdemux),
        gst_structure_new ("redirect",
            "new-location", G_TYPE_STRING, mmtdemux->streams[0]->redirect_uri,
            NULL));
    gst_element_post_message (GST_ELEMENT_CAST (mmtdemux), m);
    mmtdemux->posted_redirect = TRUE;
  }
  ...
  mmtdemux->exposed = TRUE;
  return GST_FLOW_OK;
}

/* check if major or compatible brand is 3GP */
static inline gboolean
mmtdemux_is_brand_3gp (GstMMTDemux * mmtdemux, gboolean major)
{
  if (major) {
    return ((mmtdemux->major_brand & GST_MAKE_FOURCC (255, 255, 0, 0)) ==
        FOURCC_3g__);
  } else if (mmtdemux->comp_brands != NULL) {
    GstMapInfo map;
    guint8 *data;
    gsize size;
    gboolean res = FALSE;

    gst_buffer_map (mmtdemux->comp_brands, &map, GST_MAP_READ);
    data = map.data;
    size = map.size;
    while (size >= 4) {
    ...
    gst_buffer_unmap (mmtdemux->comp_brands, &map);
    return res;
  } else {
    return FALSE;
  }
...
static inline gboolean
mmtdemux_is_string_tag_3gp (GstMMTDemux * mmtdemux, guint32 fourcc)
{
...
static void
mmtdemux_tag_add_location (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  ...
  if (!name) {
    ...
    if (*(data + offset) != 0) {
      GST_DEBUG_OBJECT (mmtdemux, "failed to convert %s tag to UTF-8, "
          "giving up", tag);
    }
  } else {
    gst_tag_list_add (taglist, GST_TAG_MERGE_REPLACE,
...
short_read:
  {
    GST_DEBUG_OBJECT (mmtdemux, "short read parsing 3GP location");
    return;
  }
}
...
static void
mmtdemux_tag_add_year (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  guint16 y;
  GDate *date;
  ...
  if (y == 0) {
    GST_DEBUG_OBJECT (mmtdemux, "year: %u is not a valid year", y);
    return;
  }
  GST_DEBUG_OBJECT (mmtdemux, "year: %u", y);
...
static void
mmtdemux_tag_add_classification (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  int offset;
  char *tag_str = NULL;
  ...
  GST_DEBUG_OBJECT (mmtdemux, "classification info: %s", tag_str);
...
short_read:
  {
    GST_DEBUG_OBJECT (mmtdemux, "short read parsing 3GP classification");
    return;
  }
}

static gboolean
mmtdemux_tag_add_str_full (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  const gchar *env_vars[] = { "GST_QT_TAG_ENCODING", "GST_TAG_ENCODING", NULL };
  GNode *data;
  ...
  data = mmtdemux_tree_get_child_by_type (node, FOURCC_data);
  if (data) {
    ...
    if (type == 0x00000001 && len > 16) {
      s = gst_tag_freeform_string_to_utf8 ((char *) data->data + 16, len - 16,
          env_vars);
      if (s) {
        GST_DEBUG_OBJECT (mmtdemux, "adding tag %s", GST_STR_NULL (s));
        gst_tag_list_add (taglist, GST_TAG_MERGE_REPLACE, tag, s, NULL);
        g_free (s);
      } else {
        GST_DEBUG_OBJECT (mmtdemux, "failed to convert %s tag to UTF-8", tag);
      }
    }
  } else {
    len = QT_UINT32 (node->data);
    ...
    if ((type >> 24) == 0xa9 && len > 8 + 4) {
      ...
      if (len > str_len + 8 + 4) {
        ...
        GST_WARNING_OBJECT (mmtdemux, "Ignoring metadata entry with multiple "
            "text alternatives, reading only first one");
      }

      offset = 12;
      len = MIN (len, str_len + 8 + 4); /* remove trailing strings that we don't use */
      GST_DEBUG_OBJECT (mmtdemux, "found international text tag");

      if (lang_code < 0x800) {  /* MAC encoded string */
        charset = "mac";
      }
    } else if (len > 14 && mmtdemux_is_string_tag_3gp (mmtdemux,
      ...
      if (mmtdemux_is_brand_3gp (mmtdemux, TRUE) ||
          (mmtdemux_is_brand_3gp (mmtdemux, FALSE) &&
              ((type & 0x00FFFFFF) == 0x0) && (type >> 24 <= 0xF))) {
        offset = 14;
        /* 16-bit Language code is ignored here as well */
        GST_DEBUG_OBJECT (mmtdemux, "found 3gpp text tag");
      } else {
        goto normal;
      }
    } else {
    normal:
      offset = 8;
      GST_DEBUG_OBJECT (mmtdemux, "found normal text tag");
    ...
    if (charset) {
      ...
      if (err) {
        GST_DEBUG_OBJECT (mmtdemux, "Failed to convert string from charset %s:"
...
static void
mmtdemux_tag_add_str (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  mmtdemux_tag_add_str_full (mmtdemux, taglist, tag, dummy, node);
}

static void
mmtdemux_tag_add_keywords (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  const gchar *env_vars[] = { "GST_QT_TAG_ENCODING", "GST_TAG_ENCODING", NULL };
  guint8 *data;
  ...
  if (!mmtdemux_is_brand_3gp (mmtdemux, TRUE)) {
    if (!mmtdemux_tag_add_str_full (mmtdemux, taglist, tag, dummy, node)) {
      /* hm, that did not work, maybe 3gpp storage in non-3gpp major brand;
       * let's try it 3gpp way after minor safety check */
      data = node->data;
      if (QT_UINT32 (data) < 15 || !mmtdemux_is_brand_3gp (mmtdemux, FALSE))
        return;
    } else
      return;
  }

  GST_DEBUG_OBJECT (mmtdemux, "found 3gpp keyword tag");
  ...
  for (; count; count--) {
    ...
    s = gst_tag_freeform_string_to_utf8 ((char *) node->data + offset,
        slen, env_vars);
    if (s) {
      GST_DEBUG_OBJECT (mmtdemux, "adding keyword %s", GST_STR_NULL (s));
      if (k) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "failed to convert keyword to UTF-8");
    }
    offset += slen;
  }
  ...
  if (k) {
    GST_DEBUG_OBJECT (mmtdemux, "adding tag %s", GST_STR_NULL (k));
...
short_read:
  {
    GST_DEBUG_OBJECT (mmtdemux, "short read parsing 3GP keywords");
    goto done;
  }
}

static void
mmtdemux_tag_add_num (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag1, const char *tag2, GNode * node)
{
  ...
  if (data) {
    ...
    if (type == 0x00000000 && len >= 22) {
      ...
      if (n1 > 0) {
        GST_DEBUG_OBJECT (mmtdemux, "adding tag %s=%d", tag1, n1);
        gst_tag_list_add (taglist, GST_TAG_MERGE_REPLACE, tag1, n1, NULL);
      }
      if (n2 > 0) {
        GST_DEBUG_OBJECT (mmtdemux, "adding tag %s=%d", tag2, n2);
...
static void
mmtdemux_tag_add_tmpo (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag1, const char *dummy, GNode * node)
{
  ...
  if (data) {
    len = QT_UINT32 (data->data);
    type = QT_UINT32 ((guint8 *) data->data + 8);
    GST_DEBUG_OBJECT (mmtdemux, "have tempo tag, type=%d,len=%d", type, len);
    ...
    if ((type == 0x00000015 || type == 0x0000000f) && len >= 18) {
      ...
      if (n1) {
        /* do not add bpm=0 */
        GST_DEBUG_OBJECT (mmtdemux, "adding tag %d", n1);
...
static void
mmtdemux_tag_add_uint32 (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag1, const char *dummy, GNode * node)
{
  GNode *data;
  int len;
  ...
  if (data) {
    len = QT_UINT32 (data->data);
    type = QT_UINT32 ((guint8 *) data->data + 8);
    GST_DEBUG_OBJECT (mmtdemux, "have %s tag, type=%d,len=%d", tag1, type, len);
    /* some files wrongly have a type 0x0f=15, but it should be 0x15 */
    if ((type == 0x00000015 || type == 0x0000000f) && len >= 20) {
      num = QT_UINT32 ((guint8 *) data->data + 16);
      if (num) {
        /* do not add num=0 */
        GST_DEBUG_OBJECT (mmtdemux, "adding tag %d", num);
...
static void
mmtdemux_tag_add_covr (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag1, const char *dummy, GNode * node)
{
  GNode *data;
  int len;
  ...
  if (data) {
    len = QT_UINT32 (data->data);
    type = QT_UINT32 ((guint8 *) data->data + 8);
    GST_DEBUG_OBJECT (mmtdemux, "have covr tag, type=%d,len=%d", type, len);
    if ((type == 0x0000000d || type == 0x0000000e) && len > 16) {
      ...
      if ((sample =
              gst_tag_image_data_to_image_sample ((guint8 *) data->data + 16,
                  len - 16, image_type))) {
        GST_DEBUG_OBJECT (mmtdemux, "adding tag size %d", len - 16);
...
static void
mmtdemux_tag_add_date (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  GNode *data;
  char *s;
  ...
  if (data) {
    ...
    if (type == 0x00000001 && len > 16) {
      ...
      GST_DEBUG_OBJECT (mmtdemux, "adding date '%s'", s);
      ...
      if (ret >= 1 && y > 1500 && y < 3000) {
        ...
        GST_DEBUG_OBJECT (mmtdemux, "could not parse date string '%s'", s);
      }
      g_free (s);
    }
  }
...
static void
mmtdemux_tag_add_gnre (GstMMTDemux * mmtdemux, GstTagList * taglist,
    const char *tag, const char *dummy, GNode * node)
{
  GNode *data;
  ...
  if (mmtdemux_is_brand_3gp (mmtdemux, TRUE) ||
      (mmtdemux_is_brand_3gp (mmtdemux, FALSE) && !data)) {
    mmtdemux_tag_add_str (mmtdemux, taglist, tag, dummy, node);
    return;
  }
  ...
  if (data) {
    ...
    if (type == 0x00000000 && len >= 18) {
      ...
      if (n > 0) {
        ...
        if (genre != NULL) {
          GST_DEBUG_OBJECT (mmtdemux, "adding %d [%s]", n, genre);
...
static void
mmtdemux_add_double_tag_from_str (GstMMTDemux * demux, GstTagList * taglist,
...
static void
mmtdemux_tag_add_revdns (GstMMTDemux * demux, GstTagList * taglist,
    const char *tag, const char *tag_bis, GNode * node)
{
  ...
  mean = mmtdemux_tree_get_child_by_type (node, FOURCC_mean);
  if (!mean) {
  ...
  name = mmtdemux_tree_get_child_by_type (node, FOURCC_name);
  if (!name) {
    ...
    for (i = 0; i < G_N_ELEMENTS (tags); ++i) {
      if (!g_ascii_strncasecmp (tags[i].name, namestr, namesize)) {
        switch (gst_tag_get_type (tags[i].tag)) {
          case G_TYPE_DOUBLE:
            mmtdemux_add_double_tag_from_str (demux, taglist, tags[i].tag,
                ((guint8 *) data->data) + 16, datasize - 16);
            break;
          case G_TYPE_STRING:
            mmtdemux_tag_add_str (demux, taglist, tags[i].tag, NULL, node);
            break;
          default:
            /* not reached */
            break;
...
static void
mmtdemux_tag_add_id32 (GstMMTDemux * demux, GstTagList * taglist, const char *tag,
    const char *tag_bis, GNode * node)
{
  ...
  GST_DEBUG_OBJECT (demux, "parsing ID32");
  ...
  if (id32_taglist) {
    GST_DEBUG_OBJECT (demux, "parsing ok");
    gst_tag_list_insert (taglist, id32_taglist, GST_TAG_MERGE_KEEP);
    gst_tag_list_unref (id32_taglist);
  } else {
    GST_DEBUG_OBJECT (demux, "parsing failed");
  }

  gst_buffer_unref (buf);
}

typedef void (*GstMMTDemuxAddTagFunc) (GstMMTDemux * demux, GstTagList * taglist,
    const char *tag, const char *tag_bis, GNode * node);
...
static const struct
{
  ...
  const GstMMTDemuxAddTagFunc func;
} add_funcs[] = {
  {
  FOURCC__nam, GST_TAG_TITLE, NULL, mmtdemux_tag_add_str}, {
  FOURCC_titl, GST_TAG_TITLE, NULL, mmtdemux_tag_add_str}, {
  FOURCC__grp, GST_TAG_GROUPING, NULL, mmtdemux_tag_add_str}, {
  FOURCC__wrt, GST_TAG_COMPOSER, NULL, mmtdemux_tag_add_str}, {
  FOURCC__ART, GST_TAG_ARTIST, NULL, mmtdemux_tag_add_str}, {
  FOURCC_aART, GST_TAG_ALBUM_ARTIST, NULL, mmtdemux_tag_add_str}, {
  FOURCC_perf, GST_TAG_ARTIST, NULL, mmtdemux_tag_add_str}, {
  FOURCC_auth, GST_TAG_COMPOSER, NULL, mmtdemux_tag_add_str}, {
  FOURCC__alb, GST_TAG_ALBUM, NULL, mmtdemux_tag_add_str}, {
  FOURCC_albm, GST_TAG_ALBUM, NULL, mmtdemux_tag_add_str}, {
  FOURCC_cprt, GST_TAG_COPYRIGHT, NULL, mmtdemux_tag_add_str}, {
  FOURCC__cpy, GST_TAG_COPYRIGHT, NULL, mmtdemux_tag_add_str}, {
  FOURCC__cmt, GST_TAG_COMMENT, NULL, mmtdemux_tag_add_str}, {
  FOURCC__des, GST_TAG_DESCRIPTION, NULL, mmtdemux_tag_add_str}, {
  FOURCC_desc, GST_TAG_DESCRIPTION, NULL, mmtdemux_tag_add_str}, {
  FOURCC_dscp, GST_TAG_DESCRIPTION, NULL, mmtdemux_tag_add_str}, {
  FOURCC__lyr, GST_TAG_LYRICS, NULL, mmtdemux_tag_add_str}, {
  FOURCC__day, GST_TAG_DATE, NULL, mmtdemux_tag_add_date}, {
  FOURCC_yrrc, GST_TAG_DATE, NULL, mmtdemux_tag_add_year}, {
  FOURCC__too, GST_TAG_ENCODER, NULL, mmtdemux_tag_add_str}, {
  FOURCC__inf, GST_TAG_COMMENT, NULL, mmtdemux_tag_add_str}, {
  FOURCC_trkn, GST_TAG_TRACK_NUMBER, GST_TAG_TRACK_COUNT, mmtdemux_tag_add_num}, {
  FOURCC_disk, GST_TAG_ALBUM_VOLUME_NUMBER, GST_TAG_ALBUM_VOLUME_COUNT,
        mmtdemux_tag_add_num}, {
  FOURCC_disc, GST_TAG_ALBUM_VOLUME_NUMBER, GST_TAG_ALBUM_VOLUME_COUNT,
        mmtdemux_tag_add_num}, {
  FOURCC__gen, GST_TAG_GENRE, NULL, mmtdemux_tag_add_str}, {
  FOURCC_gnre, GST_TAG_GENRE, NULL, mmtdemux_tag_add_gnre}, {
  FOURCC_tmpo, GST_TAG_BEATS_PER_MINUTE, NULL, mmtdemux_tag_add_tmpo}, {
  FOURCC_covr, GST_TAG_IMAGE, NULL, mmtdemux_tag_add_covr}, {
  FOURCC_sonm, GST_TAG_TITLE_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_soal, GST_TAG_ALBUM_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_soar, GST_TAG_ARTIST_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_soaa, GST_TAG_ALBUM_ARTIST_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_soco, GST_TAG_COMPOSER_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_sosn, GST_TAG_SHOW_SORTNAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_tvsh, GST_TAG_SHOW_NAME, NULL, mmtdemux_tag_add_str}, {
  FOURCC_tvsn, GST_TAG_SHOW_SEASON_NUMBER, NULL, mmtdemux_tag_add_uint32}, {
  FOURCC_tves, GST_TAG_SHOW_EPISODE_NUMBER, NULL, mmtdemux_tag_add_uint32}, {
  FOURCC_kywd, GST_TAG_KEYWORDS, NULL, mmtdemux_tag_add_keywords}, {
  FOURCC_keyw, GST_TAG_KEYWORDS, NULL, mmtdemux_tag_add_str}, {
  FOURCC__enc, GST_TAG_ENCODER, NULL, mmtdemux_tag_add_str}, {
  FOURCC_loci, GST_TAG_GEO_LOCATION_NAME, NULL, mmtdemux_tag_add_location}, {
  FOURCC_clsf, GST_QT_DEMUX_CLASSIFICATION_TAG, NULL,
        mmtdemux_tag_add_classification}, {
  FOURCC__mak, GST_TAG_DEVICE_MANUFACTURER, NULL, mmtdemux_tag_add_str}, {
  FOURCC__mod, GST_TAG_DEVICE_MODEL, NULL, mmtdemux_tag_add_str}, {
  FOURCC__swr, GST_TAG_APPLICATION_NAME, NULL, mmtdemux_tag_add_str}, {
     ...
     * bug #614471
     */
  FOURCC_____, "", NULL, mmtdemux_tag_add_revdns}, {
    /* see http://www.mp4ra.org/specs.html for ID32 in meta box */
  FOURCC_ID32, "", NULL, mmtdemux_tag_add_id32}
};
...
static void
mmtdemux_tag_add_blob (GNode * node, GstQtDemuxTagList * mmtdemuxtaglist)
{
  gint len;
  guint8 *data;
  GstBuffer *buf;
  ...
  guint i;
  guint8 ndata[4];
  GstMMTDemux *demux = mmtdemuxtaglist->demux;
  GstTagList *taglist = mmtdemuxtaglist->taglist;
...
static void
mmtdemux_parse_udta (GstMMTDemux * mmtdemux, GstTagList * taglist, GNode * udta)
{
  GNode *meta;
  GNode *ilst;
  GNode *xmp_;
  ...
  demuxtaglist.demux = mmtdemux;
  demuxtaglist.taglist = taglist;

  meta = mmtdemux_tree_get_child_by_type (udta, FOURCC_meta);
  if (meta != NULL) {
    ilst = mmtdemux_tree_get_child_by_type (meta, FOURCC_ilst);
    if (ilst == NULL) {
      GST_DEBUG_OBJECT (mmtdemux, "no ilst");
      return;
    }
  } else {
    ilst = udta;
    GST_DEBUG_OBJECT (mmtdemux, "no meta so using udta itself");
  }

  i = 0;
  while (i < G_N_ELEMENTS (add_funcs)) {
    node = mmtdemux_tree_get_child_by_type (ilst, add_funcs[i].fourcc);
    if (node) {
      gint len;

      len = QT_UINT32 (node->data);
      if (len < 12) {
        GST_DEBUG_OBJECT (mmtdemux, "too small tag atom %" GST_FOURCC_FORMAT,
            GST_FOURCC_ARGS (add_funcs[i].fourcc));
      } else {
        add_funcs[i].func (mmtdemux, taglist, add_funcs[i].gst_tag,
  ...
  g_node_children_foreach (ilst, G_TRAVERSE_ALL,
      (GNodeForeachFunc) mmtdemux_tag_add_blob, &demuxtaglist);

  /* parse up XMP_ node if existing */
  xmp_ = mmtdemux_tree_get_child_by_type (udta, FOURCC_XMP_);
  if (xmp_ != NULL) {
    GstBuffer *buf;
    GstTagList *xmptaglist;
    ...
    mmtdemux_handle_xmp_taglist (mmtdemux, taglist, xmptaglist);
  } else {
    GST_DEBUG_OBJECT (mmtdemux, "No XMP_ node found");
  }
}
...
static gint
mmtdemux_redirects_sort_func (gconstpointer a, gconstpointer b)
{
 ...
 */
static void
mmtdemux_process_redirects (GstMMTDemux * mmtdemux, GList * references)
{
  GstQtReference *best;
  GstStructure *s;
  GstMessage *msg;
  ...
  references = g_list_sort (references, mmtdemux_redirects_sort_func);
  ...
  GST_INFO_OBJECT (mmtdemux, "posting redirect message: %" GST_PTR_FORMAT, s);
  msg = gst_message_new_element (GST_OBJECT_CAST (mmtdemux), s);
  gst_element_post_message (GST_ELEMENT_CAST (mmtdemux), msg);
  mmtdemux->posted_redirect = TRUE;
}
 ...
 */
static gboolean
mmtdemux_parse_redirects (GstMMTDemux * mmtdemux)
{
  GNode *rmra, *rmda, *rdrf;

  rmra = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_rmra);
  if (rmra) {
    GList *redirects = NULL;

    rmda = mmtdemux_tree_get_child_by_type (rmra, FOURCC_rmda);
    while (rmda) {
      GstQtReference ref = { NULL, NULL, 0, 0 };
      GNode *rmdr, *rmvc;

      if ((rmdr = mmtdemux_tree_get_child_by_type (rmda, FOURCC_rmdr))) {
        ref.min_req_bitrate = QT_UINT32 ((guint8 *) rmdr->data + 12);
        GST_DEBUG_OBJECT (mmtdemux, "data rate atom, required bitrate = %u",
            ref.min_req_bitrate);
      }

      if ((rmvc = mmtdemux_tree_get_child_by_type (rmda, FOURCC_rmvc))) {
      ...
      rdrf = mmtdemux_tree_get_child_by_type (rmda, FOURCC_rdrf);
      if (rdrf) {
        guint32 ref_type;
        guint8 *ref_data;
        guint ref_len;
        ...
        if (ref_len > 20) {
          ...
          if (ref_type == FOURCC_alis) {
            ...
            if (ref_len > 70) {
              ...
              GST_WARNING_OBJECT (mmtdemux, "Invalid rdrf/alis size (%u < 70)",
                  ref_len);
            }
          ...
          if (ref.location != NULL) {
            GST_INFO_OBJECT (mmtdemux, "New location: %s", ref.location);
            redirects =
          ...
          GST_WARNING_OBJECT (mmtdemux, "Invalid rdrf size (%u < 20)", ref_len);
        }
      }

      /* look for others */
      rmda = mmtdemux_tree_get_sibling_by_type (rmda, FOURCC_rmda);
    }

    if (redirects != NULL) {
      mmtdemux_process_redirects (mmtdemux, redirects);
    }
  }
  return TRUE;
}
...
static GstTagList *
mmtdemux_add_container_format (GstMMTDemux * mmtdemux, GstTagList * tags)
{
  const gchar *fmt;
  ...
  else if ((mmtdemux->major_brand & 0xffff) == FOURCC_3g__)
    fmt = "3GP";
  else if (mmtdemux->major_brand == FOURCC_qt__)
    fmt = "Quicktime";
  else if (mmtdemux->fragmented)
    fmt = "ISO fMP4";
  else
    fmt = "ISO MP4/M4A";

  GST_DEBUG_OBJECT (mmtdemux, "mapped %" GST_FOURCC_FORMAT " to '%s'",
      GST_FOURCC_ARGS (mmtdemux->major_brand), fmt);
...
mmtdemux_parse_tree (GstMMTDemux * mmtdemux)
{
  ...
  mvhd = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_mvhd);
  if (mvhd == NULL) {
    GST_DEBUG_OBJECT (mmtdemux, "No mvhd node found, looking for redirects.");
    return mmtdemux_parse_redirects (mmtdemux);
  }
  ...
  if (creation_time != 0) {
    ...
    if (creation_time >= QTDEMUX_SECONDS_FROM_1904_TO_1970) {
      ...
      if (now.tv_sec + 24 * 3600 < creation_time) {
        GST_DEBUG_OBJECT (mmtdemux, "discarding bogus future creation time");
      } else {
  ...
  if (datetime) {
    ...
    gst_tag_list_add (mmtdemux->tag_list, GST_TAG_MERGE_KEEP, GST_TAG_DATE_TIME,
        datetime, NULL);
    gst_date_time_unref (datetime);
  }

  GST_INFO_OBJECT (mmtdemux, "timescale: %u", mmtdemux->timescale);
  GST_INFO_OBJECT (mmtdemux, "duration: %" G_GUINT64_FORMAT, mmtdemux->duration);
  ...
  if (mvex) {
    ...
    mmtdemux->fragmented = TRUE;

    /* compensate for total duration */
    mehd = mmtdemux_tree_get_child_by_type_full (mvex, FOURCC_mehd, &mehd_data);
    if (mehd)
      mmtdemux_parse_mehd (mmtdemux, &mehd_data);
  }

  /* set duration in the segment info */
  gst_mmtdemux_get_duration (mmtdemux, &duration);
  if (duration) {
    mmtdemux->segment.duration = duration;
    ...
    mmtdemux->segment.stop = duration;
  }

  /* parse all traks */
  trak = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_trak);
  while (trak) {
    mmtdemux_parse_trak (mmtdemux, trak);
    /* iterate all siblings */
    trak = mmtdemux_tree_get_sibling_by_type (trak, FOURCC_trak);
  }

  mmtdemux->tag_list = gst_tag_list_make_writable (mmtdemux->tag_list);

  /* find tags */
  udta = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_udta);
  if (udta) {
    mmtdemux_parse_udta (mmtdemux, mmtdemux->tag_list, udta);
  } else {
    GST_DEBUG_OBJECT (mmtdemux, "No udta node found.");
  }

  /* maybe also some tags in meta box */
  udta = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_meta);
  if (udta) {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing meta box for tags.");
    mmtdemux_parse_udta (mmtdemux, mmtdemux->tag_list, udta);
  } else {
    GST_DEBUG_OBJECT (mmtdemux, "No meta node found.");
  }

  /* parse any protection system info */
  pssh = mmtdemux_tree_get_child_by_type (mmtdemux->moov_node, FOURCC_pssh);
  while (pssh) {
    GST_DEBUG_OBJECT (mmtdemux, "Parsing pssh box.");
    mmtdemux_parse_pssh (mmtdemux, pssh);
    pssh = mmtdemux_tree_get_sibling_by_type (pssh, FOURCC_pssh);
  }

  mmtdemux->tag_list = mmtdemux_add_container_format (mmtdemux, mmtdemux->tag_list);
  if (mmtdemux->moov_node_compressed) {
	  g_node_destroy (mmtdemux->moov_node_compressed);
	  if (mmtdemux->moov_node)
		  g_free (mmtdemux->moov_node->data);
...
static void
gst_mmtdemux_handle_esds (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    QtDemuxStreamStsdEntry * entry, GNode * esds, GstTagList * list)
{
  ...
  GST_MEMDUMP_OBJECT (mmtdemux, "esds", ptr, len);
  ptr += 8;
  GST_DEBUG_OBJECT (mmtdemux, "version/flags = %08x", QT_UINT32 (ptr));
  ptr += 4;
  while (ptr + 1 < end) {
    tag = QT_UINT8 (ptr);
    GST_DEBUG_OBJECT (mmtdemux, "tag = %02x", tag);
    ptr++;
    len = read_descr_size (ptr, end, &ptr);
    GST_DEBUG_OBJECT (mmtdemux, "len = %d", len);
    ...
    switch (tag) {
      case ES_DESCRIPTOR_TAG:
        GST_DEBUG_OBJECT (mmtdemux, "ID %04x", QT_UINT16 (ptr));
        GST_DEBUG_OBJECT (mmtdemux, "priority %04x", QT_UINT8 (ptr + 2));
        ptr += 3;
        break;
      case DECODER_CONFIG_DESC_TAG:{
        ...
        GST_DEBUG_OBJECT (mmtdemux, "object_type_id %02x", object_type_id);
        GST_DEBUG_OBJECT (mmtdemux, "stream_type %02x", QT_UINT8 (ptr + 1));
        GST_DEBUG_OBJECT (mmtdemux, "buffer_size_db %02x", QT_UINT24 (ptr + 2));
        GST_DEBUG_OBJECT (mmtdemux, "max bitrate %u", max_bitrate);
        GST_DEBUG_OBJECT (mmtdemux, "avg bitrate %u", avg_bitrate);
        if (max_bitrate > 0 && max_bitrate < G_MAXUINT32) {
      ...
      case DECODER_SPECIFIC_INFO_TAG:
        GST_MEMDUMP_OBJECT (mmtdemux, "data", ptr, len);
      ...
      case SL_CONFIG_DESC_TAG:
        GST_DEBUG_OBJECT (mmtdemux, "data %02x", QT_UINT8 (ptr));
        ptr += 1;
        break;
      default:
        GST_DEBUG_OBJECT (mmtdemux, "Unknown/unhandled descriptor tag %02x",
            tag);
        GST_MEMDUMP_OBJECT (mmtdemux, "descriptor data", ptr, len);
        ptr += len;
        break;
    }
  }
  ...
  switch (object_type_id) {
    ...
    case 0x68:                 /* AAC SSR */
      ...
      if (data_ptr != NULL && data_len >= 2) {
        ...
        GST_WARNING_OBJECT (mmtdemux, "No codec_data for AAC available");
  ...
  if (data_ptr) {
    ...
    GST_DEBUG_OBJECT (mmtdemux, "setting codec_data from esds");
    GST_MEMDUMP_OBJECT (mmtdemux, "codec_data from esds", data_ptr, data_len);
...
static GstCaps *
mmtdemux_video_caps (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
...
static GstCaps *
mmtdemux_audio_caps (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    QtDemuxStreamStsdEntry * entry, guint32 fourcc, const guint8 * data,
    int len, gchar ** codec_name)
{
  ...
  GST_DEBUG_OBJECT (mmtdemux, "resolve fourcc 0x%08x", GUINT32_TO_BE (fourcc));
  ...
  if (caps) {
    GstCaps *templ_caps =
        gst_static_pad_template_get_caps (&gst_mmtdemux_audiosrc_template);
...
static GstCaps *
mmtdemux_sub_caps (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    QtDemuxStreamStsdEntry * entry, guint32 fourcc,
    const guint8 * stsd_entry_data, gchar ** codec_name)
{
  GstCaps *caps;
...
static GstCaps *
mmtdemux_generic_caps (GstMMTDemux * mmtdemux, QtDemuxStream * stream,
    QtDemuxStreamStsdEntry * entry, guint32 fourcc,
    const guint8 * stsd_entry_data, gchar ** codec_name)
{
  GstCaps *caps;
  GST_DEBUG_OBJECT (mmtdemux,
      "CCCCCCCCCCAPS:   parsing '%" GST_FOURCC_FORMAT "'",GST_FOURCC_ARGS (fourcc));
...
static void
gst_mmtdemux_append_protection_system_id (GstMMTDemux * mmtdemux,
    const gchar * system_id)
{
  gint i;

  if (!mmtdemux->protection_system_ids)
    mmtdemux->protection_system_ids =
        g_ptr_array_new_with_free_func ((GDestroyNotify) g_free);
  /* Check whether we already have an entry for this system ID. */
  for (i = 0; i < mmtdemux->protection_system_ids->len; ++i) {
    const gchar *id = g_ptr_array_index (mmtdemux->protection_system_ids, i);
    if (g_ascii_strcasecmp (system_id, id) == 0) {
      return;
    }
  }
  GST_DEBUG_OBJECT (mmtdemux, "Adding cenc protection system ID %s", system_id);
  g_ptr_array_add (mmtdemux->protection_system_ids, g_ascii_strdown (system_id,
          -1));
}
